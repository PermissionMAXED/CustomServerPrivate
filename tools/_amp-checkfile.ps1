param(
    [string]$Sid = "",
    [string]$FilePath = "game/UserData/Medusa/medusa.bundle"
)
...
$c = [System.Net.Http.HttpClient]::new(); $c.Timeout = [TimeSpan]::FromMinutes(2)
...
$md5 = [string](J "$rp/FileManagerPlugin/CalculateFileMD5Sum" @{ FilePath = $FilePath } $rs).Result
Write-Host ("HOST {0} md5 = {1}" -f $FilePath, $md5)
