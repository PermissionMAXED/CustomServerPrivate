...
internal static class XrefScanUtilFinder
{
    public static IntPtr FindLastRcxReadAddressBeforeCallTo(IntPtr codeStart, IntPtr callTarget)
    {
        try
        {
            return FindLastRcxReadAddressBeforeCallToCore(codeStart, callTarget);
        }
    ...
    private static IntPtr FindLastRcxReadAddressBeforeCallToCore(IntPtr codeStart, IntPtr callTarget)
    {
        ...
        while (true)
        {
            ...
            if (instruction.Mnemonic == Mnemonic.Call)
            {
                ...
                if (unchecked((IntPtr)target) == callTarget)
                    return lastRcxRead;
            }
    ...
    public static IntPtr FindByteWriteTargetRightAfterCallTo(IntPtr codeStart, IntPtr callTarget)
    {
        try
        {
            return FindByteWriteTargetRightAfterCallToCore(codeStart, callTarget);
        }
        catch (OverflowException)
        {
            return IntPtr.Zero;
    ...
    private static IntPtr FindByteWriteTargetRightAfterCallToCore(IntPtr codeStart, IntPtr callTarget)
    {
        var decoder = XrefScanner.DecoderForAddress(codeStart);
        var seenCall = false;
