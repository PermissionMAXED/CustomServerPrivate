...
public static class XrefScanMethodDb
{
    ...
    internal static void CallMetadataInitForMethod(CachedScanResultsAttribute attribute)
    {
        ...
        if (Marshal.ReadByte(unchecked((IntPtr)(GameAssemblyBase + attribute.MetadataInitFlagRva))) != 0)
            return;
        ...
        if (ourMetadataInitForMethodDelegate == null)
            ourMetadataInitForMethodDelegate =
                Marshal.GetDelegateForFunctionPointer<XrefScanUtil.InitMetadataForMethod>(
                    unchecked((IntPtr)(GameAssemblyBase + XrefScanCache.Header.InitMethodMetadataRva)));

        var token = Marshal.ReadInt32(unchecked((IntPtr)(GameAssemblyBase + attribute.MetadataInitTokenRva)));

        ourMetadataInitForMethodDelegate(token);

        Marshal.WriteByte(unchecked((IntPtr)(GameAssemblyBase + attribute.MetadataInitFlagRva)), 1);
    }
}
