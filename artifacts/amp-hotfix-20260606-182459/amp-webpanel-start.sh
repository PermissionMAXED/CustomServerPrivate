...
export CustomServer__GameServerStartupStallGraceSeconds="${GAME_STARTUP_STALL_GRACE:-90}"
export CustomServer__GameServerStartupStallSeconds="${GAME_STARTUP_STALL:-45}"
...
[ -n "$DEPLOYED_RELEASE" ] || DEPLOYED_RELEASE="bapcustomserver-unknown"
...
if [ "${BAPCUSTOM_PREWARM_UNITY:-1}" != "0" ]; then
  ...
  if [ -f "$PREWARM_MARKER" ]; then
    ...
    PREWARM_ELAPSED=0
    while [ "$PREWARM_ELAPSED" -lt "$PREWARM_TIMEOUT" ] && kill -0 "$PREWARM_PID" 2>/dev/null; do
      ...
      if [ -f "$MELON_LOG" ] && [ "$MELON_LOG" -nt "$PREWARM_SENTINEL" ] && grep -E 'Game bootstrap HTTP listener is active|Started game bootstrap WebServer on 127\.0\.0\.1:' "$MELON_LOG" >/dev/null 2>&1; then
        PREWARM_READY=1
        PREWARM_READY_SOURCE="managed-listener-log"
        break
      fi
      sleep 1
      PREWARM_ELAPSED=$((PREWARM_ELAPSED + 1))
    done
