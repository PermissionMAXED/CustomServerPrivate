...
export CustomServer__GameServerReadyTimeoutSeconds="${BAPCUSTOM_GAME_READY_TIMEOUT:-150}"
export CustomServer__RequireGameServerKcpPort="${REQUIRE_GAME_KCP_PORT:-false}"
export CustomServer__GameServerTcpPortReadyTimeoutSeconds="${BAPCUSTOM_GAME_TCP_READY_TIMEOUT:-75}"
export CustomServer__GameServerKcpPortReadyTimeoutSeconds="${BAPCUSTOM_GAME_KCP_READY_TIMEOUT:-45}"
export CustomServer__GameServerStartAttempts="${BAPCUSTOM_GAME_START_ATTEMPTS:-2}"
export CustomServer__GameServerStartRetryDelaySeconds="${BAPCUSTOM_GAME_START_RETRY_DELAY:-2}"
export CustomServer__GameServerStartupStallGraceSeconds="${BAPCUSTOM_GAME_STARTUP_STALL_GRACE:-45}"
export CustomServer__GameServerStartupStallSeconds="${BAPCUSTOM_GAME_STARTUP_STALL:-25}"
...
[ -n "$DEPLOYED_RELEASE" ] || DEPLOYED_RELEASE="medusa-v1659-masterypreview-quiet"
...
if [ "${BAPCUSTOM_PREWARM_UNITY:-1}" != "0" ]; then
  ...
  if [ -f "$PREWARM_MARKER" ]; then
    ...
    if command -v timeout >/dev/null 2>&1; then
      timeout -k 15s "${PREWARM_TIMEOUT}s" ./start-match.sh ./game/bapbap.exe -batchmode -logFile "$PREWARM_LOG" -httpport="$PREWARM_HTTP" -wsport="$PREWARM_WS" -kcpport="$PREWARM_KCP" -tcpport="$PREWARM_TCP" --melonloader.agfoffline --melonloader.captureplayerlogs --bapcustom-use-proxy=false --bapcustom-show-ui=false --bapcustom-host=127.0.0.1 --bapcustom-server-port="5055" &
    else
      ./start-match.sh ./game/bapbap.exe -batchmode -logFile "$PREWARM_LOG" -httpport="$PREWARM_HTTP" -wsport="$PREWARM_WS" -kcpport="$PREWARM_KCP" -tcpport="$PREWARM_TCP" --melonloader.agfoffline --melonloader.captureplayerlogs --bapcustom-use-proxy=false --bapcustom-show-ui=false --bapcustom-host=127.0.0.1 --bapcustom-server-port="5055" &
    fi
    PREWARM_PID=$!
    PREWARM_READY=0
