Shader "Custom/Toon/ToonMatcap_Amplify" {
	Properties {
		_Centered ("[Centered]", Float) = 0
		_AmplifyProperties ("#Amplify Properties", Float) = 0
		[HideInInspector] _EmissionColor ("Emission Color", Vector) = (1,1,1,1)
		[HideInInspector] _AlphaCutoff ("Alpha Cutoff ", Range(0, 1)) = 0.5
		[Gradient] _GradientTexture ("Gradient Texture", 2D) = "white" {}
		_HitBlinkColor ("HitBlinkColor", Vector) = (1,1,1,0)
		_HitBlinkAmount ("HitBlinkAmount", Range(0, 1)) = 0
		[Toggle(_BLENDBASETEXTURE_ON)] _BlendBaseTexture ("Blend Base Texture", Float) = 1
		[Header(MatCap Parameters)] _MatCap ("MatCap", 2D) = "white" {}
		_MatCapContribution ("MatCap Contribution", Range(0, 1)) = 1
		_OpacityOffset ("Opacity Offset", Range(0, 1)) = 0.5
		_Speed ("Speed", Float) = 1
		_DisplaceAmount ("Displace Amount", Float) = 1
		_DisplaceColor ("Displace Color", Vector) = (0.8396226,0.1003322,0.1003322,1)
		[Toggle(_DISPLACEWITHNOISE_ON)] _DisplaceWithNoise ("Displace With Noise", Float) = 0
		...
		[Header(Fog Of War)] [Toggle] _FOWOccluded ("FOW Occluded", Float) = 0
		[HDR] _BlackBloodTint ("Black & Blood Tint", Vector) = (0,0,0,0)
