Shader "Custom/Toon/ToonFoliageBush_Amplify" {
	Properties {
		_Centered ("[Centered]", Float) = 0
		_AmplifyProperties ("#Amplify Properties", Float) = 0
		[HideInInspector] _EmissionColor ("Emission Color", Vector) = (1,1,1,1)
		[HideInInspector] _AlphaCutoff ("Alpha Cutoff ", Range(0, 1)) = 0.5
		_MatcapTexture ("Matcap Texture", 2D) = "gray" {}
		_Matcap_Intensity ("Matcap_Intensity", Range(0, 1)) = 0
		_Wind_Frequency ("Wind_Frequency", Float) = 1
		[Header(Grass Sway Parameters)] _MoveSpeed ("Move Speed", Range(0, 50)) = 25
		_WorldDistortion ("World Distortion", Range(0, 1)) = 1
		_Rigidness ("Rigidness", Range(0, 50)) = 25
		_Wind_Intensity ("Wind_Intensity", Range(0, 1)) = 1
		_SwayMax ("Sway Max", Range(0, 1)) = 0.02
		_AlphaClipThreshold ("Alpha Clip Threshold", Float) = 1
		_YOffset ("Y Offset", Float) = 0.25
		[Header(Grass Interaction Parameters)] _MaxDisplacementWidth ("Max Displacement Width", Range(0, 1)) = 0.1
		_ShadowClipThreshold ("Shadow Clip Threshold", Float) = 0
		_Radius ("Radius", Range(0, 5)) = 1.25
		[Header(Alpha Occlusion Parameters)] _OcclusionAreaScreenRadius ("Occlusion Area Screen Radius", Range(0, 1)) = 0.4
		_OcclusionMinAlpha ("Occlusion Min Alpha", Range(0, 1)) = 0.2
		_SphereHeight ("Sphere Height", Float) = 1.75
		_PlayerBushSphereIntensity ("Player Bush Sphere Intensity", Float) = 0.6
		[Header(Alpha Occlusion Height Parameters)] _OcclusionHeightStart ("Occlusion Height Start", Float) = 2.5
		_PlayerBushMoveIntensity ("Player Bush Move Intensity", Float) = 0.7
		_OcclusionHeightSoftness ("Occlusion Height Softness", Float) = 2.7
		_OccludeTreshold ("Occlude Treshold", Float) = 0
		_StepSmoothness ("Step Smoothness", Float) = 0
		[Toggle(_MOUSE_OCCLUSION)] _MouseOcclusion ("Mouse Occlusion", Float) = 0
		_MousePosScreenRadius ("_MousePosScreenRadius", Float) = 2.7
		[Toggle(_ALPHAOCCLUDED_ON)] _AlphaOccluded ("AlphaOccluded", Float) = 0
		[Toggle(_DITHERTINTALPHA_ON)] _DitherTintAlpha ("Dither Tint Alpha", Float) = 0
		_NoiseTexture ("Noise Texture", 2D) = "white" {}
		_WorldNoiseFrequency ("World Noise Frequency", Float) = 0
		[Toggle(_ISKELP_ON)] _IsKelp ("IsKelp", Float) = 0
		[Toggle(_VERTICALMASKOFFSET_ON)] _VerticalMaskOffset ("Vertical Mask Offset", Float) = 0
		[KeywordHide(_VERTICALMASKOFFSET_ON, 0)] _FoliageLength ("Foliage Length", Float) = 0
		[KeywordHide(_VERTICALMASKOFFSET_ON, 0)] _MaskStart ("Mask Start", Float) = 0
		[KeywordHide(_VERTICALMASKOFFSET_ON, 0)] _MaskEnd ("Mask End", Float) = 0
		[Toggle(_ISBUSH_ON)] _IsBush ("IsBush", Float) = 0
