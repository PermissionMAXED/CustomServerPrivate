...
[assembly: AssemblyInformationalVersion("1.0.0+d77a0591ae35486baf72176fac6930038e63d4c9")]
...
namespace BapCustomServerMelon
{
	public sealed class CustomServerMod : MelonMod
	{
		...
		private HttpListener? _bootstrapListener;
		...
		private void TryStartDedicatedGameNetwork(int wsPort, int kcpPort, int tcpPort)
		{
			...
			else
			{
				...
				if (!_uiNetworkStartAttempted && type != null)
				{
					Array array = FindLoadedUnityObjects(type);
				...
				if (!_gameNetworkStartAttempted && type2 != null)
				{
					...
					foreach (object item2 in FindGameNetworkManagers(type2))
					{
						...
						try
						{
							...
							InvokeInstance(item2, "PreAwake", wsPort, kcpPort, tcpPort);
		...
		{
			HttpListener? bootstrapListener = _bootstrapListener;
			if (bootstrapListener != null && bootstrapListener.IsListening)
			{
				return;
			}
			try
		...
		private async Task RunManagedBootstrapListenerAsync(HttpListener listener, CancellationToken cancellationToken)
		{
			while (!cancellationToken.IsCancellationRequested && listener.IsListening)
			{
				...
				{
					((MelonBase)this).LoggerInstance.Warning("Managed bootstrap listener accept failed: " + ex3.GetBaseException().Message);
					continue;
				}
				Task.Run(() => HandleBootstrapRequestAsync(context), cancellationToken);
			}
		}

		private async Task HandleBootstrapRequestAsync(HttpListenerContext context)
		{
			...
			try
			{
				if (context.Request.HttpMethod != "POST" || (path != "/setup-game" && path != "/add-teams" && path != "/queue-matched"))
				{
					await WriteBootstrapResponseAsync(context.Response, HttpStatusCode.NotFound, "{\"ok\":false}").ConfigureAwait(continueOnCapturedContext: false);
					return;
				}
				...
				await WriteBootstrapResponseAsync(context.Response, HttpStatusCode.OK, "{\"ok\":true}").ConfigureAwait(continueOnCapturedContext: false);
			...
			catch (Exception ex)
			{
				...
				try
				{
					await WriteBootstrapResponseAsync(context.Response, HttpStatusCode.InternalServerError, "{\"ok\":false}").ConfigureAwait(continueOnCapturedContext: false);
				}
				catch
				{
				}
		...
		private static async Task WriteBootstrapResponseAsync(HttpListenerResponse response, HttpStatusCode statusCode, string body)
		{
		...
		private void TryInstallShopThrottle()
		{
			...
			int num = 0;
			string[] array = new string[4] { "SendShopRequest", "RequestShop", "FetchShop", "Update" };
			foreach (string name in array)
			{
				try
				{
