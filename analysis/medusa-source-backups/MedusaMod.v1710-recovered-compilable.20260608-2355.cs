...
public sealed class MedusaMod : ModBase
{
	...
	public static class GameModeSpawnPlayerCharPatch
	{
		...
		public static void Prefix(PlayerManager playerManager, Vector3 spawnPos)
		{
			...
			if (!((Object)playerManager == (Object)null) && !RestoreRecordedNonMedusaPlayerChar(playerManager, "GameMode.SpawnPlayerChar.prefix"))
			{
	...
	public static class AnimatorSetLayerWeightMedusaGuardPatch
	{
		...
		public static bool Prefix(Animator __instance, int layerIndex)
		{
			...
			try
			{
				...
				if (!IsMedusaRuntimeController(__instance.runtimeAnimatorController))
				{
					return true;
				}
				...
				if (_invalidAnimatorLayerGuardLogCount < 6)
				{
					...
					if (instance != null)
					{
						((ModBase)instance).Log.Info($"[Medusa] suppressed invalid Animator.SetLayerWeight({layerIndex}) for Medusa controller layerCount={layerCount}.");
					}
				}
				return false;
			}
	...
	public override void OnRegistered()
	{
		...
		((ModBase)this).Log.Info("[Medusa] Loaded (v1.6.66). Auto-registers at the lobby; /medusa or F7 to force, /medusa status, /medusa anim.");
		((ModBase)this).Log.Info("[Medusa] v1.6.66: isolates native VFX material caches per prefab, retints native Medusa particles, suppresses green primitive fallbacks, and keeps the v1.6.65 Medusa-only Animator.SetLayerWeight guard. Bundle: UserData\\Medusa\\medusa.bundle.");
		TryLoadBundle();
	...
	private static void RecordNonMedusaSelection(MatchmakingPlayerData playerData, string source)
	{
		try
		{
			...
			if (num2 > 0)
			{
				...
				SetPreMatchSelectionForPlayerId(num2, num, source + ".preserveNonMedusa");
			}
			if (!string.IsNullOrWhiteSpace(text))
			{
