...
public sealed class CustomServerMod : MelonMod
{
    ...
    // Engine indexes this array DIRECTLY by mapId in GameMode.LoadCoroutine (levelNames[levelId]),
    // which calls LevelRuntimeManager.LoadLevel(name, ...) natively BEFORE the custom-map SpawnLevel
    // hook can run. Indices 0-4 are the shipped game maps. Indices 5..40 are custom-map slots and MUST
    // carry a REAL game-map name ("Arena_Map2"): it is the "carrier" the engine can load natively so
    // LoadLevelCoroutine does not NullReference; the dedicated host's SpawnLevel hook then REWRITES it
    // to the actual custom holder by the latched bootstrap mapId. A custom name here (e.g. "CustomFlat")
    // crashes because the engine has no native Resources entry for it. Array length (41) is the hard
    // ceiling on custom mapId: levelNames[mapId] throws IndexOutOfRange above it.
    private static readonly string[] KnownLevelNames = BuildKnownLevelNames();

    private static string[] BuildKnownLevelNames()
    {
        return MelonMapHelpers.BuildKnownLevelNames();
    }
    ...
    // ---- Reflection caches -------------------------------------------------------------------
    // Every scan in this mod funnels through FindLoadedUnityObjects/GetMemberValue/
    // TryInvokeInstanceIfPresent. Uncached, each call re-ran GetMethods()+LINQ+MakeGenericMethod
    // (plus up to three swallowed exceptions) per lookup — multiplying the cost of every periodic
    // scan and causing measurable GC pressure. These caches make repeat lookups near-free.

    /// <summary>Resolved finder for <see cref="FindLoadedUnityObjects"/>: closed MethodInfo plus
    /// its fixed argument template. Cached after the first strategy that structurally works for
    /// the type.</summary>
    private sealed record UnityObjectFinder(MethodInfo Method, object?[]? Args);

    private static readonly ConcurrentDictionary<Type, UnityObjectFinder> s_unityObjectFinderCache = new();

    /// <summary>Per-(type,memberName) cache for instance property/field lookups. Value is a
    /// PropertyInfo, FieldInfo, or null ("member does not exist").</summary>
    private static readonly ConcurrentDictionary<(Type Type, string Name), MemberInfo?> s_instanceMemberCache = new();

    /// <summary>Per-(type,methodName,argCount) cache for instance method lookups used by
    /// InvokeInstance/TryInvokeInstanceIfPresent. Null means "no such method".</summary>
    private static readonly ConcurrentDictionary<(Type Type, string Name, int ArgCount), MethodInfo?> s_instanceMethodCache = new();

    /// <summary>Per-Il2Cpp-array-type cache of (Length property, get_Item method) used by
    /// ConvertEnumerableToArray.</summary>
    private static readonly ConcurrentDictionary<Type, (PropertyInfo? Length, MethodInfo? GetItem)> s_il2cppArrayAccessorCache = new();
    ...
    private bool _mirrorRpcGuardInstalled;
    ...
    private bool _authoritativeCharacterIdsReceived;
    private bool _characterModelUnlockStatePopulated;
    private bool _serverAvailableCharactersLogged;
    private int _charConfigRetryCount;
    private int _lateUpdateCharFixAttempts;
    ...
    private bool _unityWebRequestProxyUrlPatchLogged;
    ...
    private float _nextPatchAt = 35f; // Delay first Harmony patch batch by 35s so Cpp2IL/Support Module fully initializes
    private bool _il2CppInteropReady; // Set true at 35s in OnUpdate (gated by _nextPatchAt), NOT in OnLateInitializeMelon - ALL FindType/Harmony calls must guard behind this
    private int _patchIndex; // Index for staggered Harmony patch installation (3 per frame to avoid Il2Cpp native crash)
    private float _nextBootstrapRepairAt;
    private bool _setupGameFailureLogged;
    private bool _setupGameApplied;
    private int _bootstrapDeferredUntilIl2CppReadyLogCount;
    ...
    private int _stalePrematchDismissLogCount;
    private int _stalePrematchDiagLogCount;
    private int _stalePrematchNotFoundLogCount;
    private int _prematchCleanupArmLogCount;
    private bool _prematchCleanupDisconnectPatchesInstalled;
    private bool _postMatchExitCleanupEligible;
    private bool _postMatchCleanupDismissed;
    private int _augmentDiagLogCount;
    private int _hudProbeLogCount;
    private float _nextHudProbeAt;
    private bool _escToggleRepaired;
    private static int _rpcFinalizerStackLogCount;
    private int _stalePrematchConsecutive;
    private float _nextStalePrematchDismissAt;
    private bool _matchMirrorWasConnected;
    private float _postMatchPrematchCleanupUntil;
    private float _postMatchLobbyReturnDueAt;
    private float _postMatchLobbyReturnUntil;
    private int _postMatchLobbyReturnAttempts;
    private int _postMatchLobbyReturnLogCount;
    private bool _postMatchLobbyReturnLoadInProgress;
    private float _postMatchLobbyReturnSuppressUntil;
    private float _postMatchLoadingClearUntil;
    private int _postMatchLoadingClearLogCount;
    // Cached HasActiveMatchRuntime() probe. The uncached call runs 2-3 FindObjectsOfType scans
    // through the reflection cascade; calling it every frame during a match was the main source
    // of constant in-match micro-stutter (144 scans/second at 144 FPS).
    private float _matchRuntimeProbeCachedAt = float.NegativeInfinity;
    private bool _matchRuntimeProbeCachedValue;
    private float _nextLateUpdateCharFixAt;
    private string _splashDismissSceneName = "";
    private float _splashDismissSceneChangedAt;
    private bool _charConfigHeapScanUsed;
    private int _lastPopulateAllReportedCount = -1;
    private string _lastDirectPopulateSummary = "";
    private float _nextDedicatedGameManagerSweepAt;
    ...
    private float? _autoDisconnectMatchAfterSeconds;
    private bool _autoDisconnectMatchObserved;
    private bool _autoDisconnectInvoked;
    private float _autoDisconnectMatchStartAt;
    ...
    private bool _autoplayQueueMode; // TEST-ONLY: join the public matchmaking queue instead of custom-game ready
    private bool _autoplayQueueJoinSent;
    private bool _autoSelectAugmentsEnabled;
    private int _autoplayState; // 0=waiting-for-login, 1=joining-lobby, 2=set-ready/queue, 3=in-match, 4=queued-waiting
    private float _autoplayNextActionAt;
    private bool _autoplayInMatch;
    private bool _autoplayLockedIn;
    private int _autoplayLockAttempts;
    ...
    public override void OnEarlyInitializeMelon()
    {
        ...
        // DISABLED: EarlyLobbyUiPatchLoop triggered Il2CppInteropRuntime crash when
        // loaded alongside NetworkedCustomChar due to Harmony.FindType resolution.
        // All Harmony patches are installed at ~35s in OnUpdate via _nextPatchAt guard.
        // TryInstallCharacterSelectNullRefPatch and TryInstallUICharactersConfigPatch
        // run inside the OnUpdate patch batch (lines 577-578).
    }
    ...
    public override void OnLateInitializeMelon()
    {
        ...
        // CRITICAL: Do NOT set _il2CppInteropReady here. OnLateInitializeMelon fires
        // BEFORE the Support Module (Il2CppInteropRuntime) finishes loading. Setting
        // _il2CppInteropReady here causes our EarlyLobbyUiPatchLoop coroutine to call
        // FindType/Harmony.Patch on Il2Cpp types immediately, which triggers the
        // UnityVersionHandler..cctor circular dependency crash IN the Support Module.
        // Instead, _il2CppInteropReady is set in OnUpdate when _nextPatchAt (35s) fires.
        LoggerInstance.Msg("Melon runtime initialized. Support Module init pending (waiting 35s for Il2CppInterop startup).");
    }
    ...
    private void InitializeCore(string phase)
    {
        ...
        _nativeUiEntry = _category.CreateEntry("UseNativeGameUi", false, "Use Native Game UI");
        ...
        // Open the lightweight HTTP listener early so AMP's Unity prewarm can observe /status.
        // Actual /setup-game processing is gated in HandleBootstrapTcpClientAsync until
        // Il2CppInterop is ready, so early server POSTs cannot touch the half-initialized runtime.
        ...
        // MINIMAL STARTUP: Only proxy + config + login prefs.
        // Harmony patches on IL2CPP types must NOT run here because the Support Module
        // (Il2Cpp.dll / Il2CppInteropRuntime) has not loaded yet. If we call FindType,
        // typeof, GetTypes, or Harmony.Patch on any Il2Cpp-wrapped type, it triggers
        // Il2CppInterop class init before Start() is called, putting the runtime in
        // an inconsistent state. The Support Module then fails to Start() with:
        // "Il2CppInteropRuntime is not yet initialized."
        // All TryInstall* calls are deferred to the OnUpdate retry loop (gated by
        // _nextPatchAt = 2.0f) which runs after the Support Module is confirmed loaded.
        ...
        // DISABLED: TryFetchServerPolicy triggers FindType which conflicts with NCC during Support Module init.
        // Runs safely at 35s in the OnUpdate patch batch instead.
    ...
    public override void OnSceneWasLoaded(int buildIndex, string sceneName)
    {
        ...
        if (sceneName != null && (sceneName.Contains("Main", StringComparison.OrdinalIgnoreCase) || sceneName.Contains("Lobby", StringComparison.OrdinalIgnoreCase) || sceneName.Contains("Menu", StringComparison.OrdinalIgnoreCase)))
        {
            ...
            _nextLobbyQueueRecoveryAt = Time.realtimeSinceStartup + 5.0f;
            _lobbyQueueInQueueSince = 0f;
            _lobbyQueueIdleProbe = null;
            if (_postMatchLobbyReturnLoadInProgress)
            {
                _postMatchLobbyReturnLoadInProgress = false;
                _postMatchLobbyReturnDueAt = 0f;
                _postMatchLobbyReturnUntil = 0f;
                _postMatchLobbyReturnAttempts = 0;
                _postMatchLoadingClearUntil = Time.realtimeSinceStartup + 35f;
                LoggerInstance.Msg($"[LobbyReturnFallback] MainScene reached after post-match fallback; suppressing stale reconnect/disconnect loops and clearing loading UI.");
            }
        }
    }
    ...
    public override void OnUpdate()
    {
        ...
        // Identity setup must reach the player even if a prior OnGUI exception disabled _uiEnabled,
        // otherwise a single transient IMGUI throw permanently traps a first-start user with no way
        // to enter their name. The native setup panel is mandatory; only cosmetic native UI is gated.
        // Production mode (default ON) FULLY suppresses the cosmetic native UI: the chip is never built
        // and the click handler never runs, so clicking the top-right corner can't open the "Custom Server"
        // overlay regardless of a stale UseNativeGameUi pref. This also stops the per-0.5s RefreshNativeGameUi
        // loop. Identity setup stays exempt (and is auto-generated now, so it won't surface either).
        bool allowNativeUi = (_uiEnabled && (_nativeUiEntry?.Value == true) && _productionModeEntry?.Value != true) || _identitySetupRequired;
        if (allowNativeUi && !_dedicatedProcess && !UnityEngine.Application.isBatchMode)
        {
            EnsureNativeGameUi();
        ...
        if (runScans)
        {
            // Defer ALL Il2Cpp type resolution until OnLateInitializeMelon fires.
            // TryInstall* methods call FindType on Il2Cpp types (Mirror, kcp2k, BAPBAP.UI)
            // which crashes Il2CppInteropRuntime if accessed before initialization.
            if (_il2CppInteropReady)
            {
                ...
                if (!_networkTunerHarmonyInstalled) { _networkTunerHarmonyInstalled = true; }
                ...
                // OnUpdate-based character model population. Only activates after 35s.
                if (Time.realtimeSinceStartup >= 35f && !_dedicatedProcess && !_charConfigPreloadDone)
                {
                    try { TryPopulateCharacterModelsDirect(); } catch { }
                    try { if (!_charConfigPreloadDone && _lastAvailableCharacterIds.Length > 0) PopulateAllUICharactersConfigurations(_lastAvailableCharacterIds); } catch { }
                }
            ...
            } // end _il2CppInteropReady guard
        } // end runScans
        if (Time.realtimeSinceStartup >= _nextLobbyQueueRecoveryAt)
        {
            _nextLobbyQueueRecoveryAt = Time.realtimeSinceStartup + 5.0f;
            try { TryRecoverStaleLobbyQueueUi(); } catch { }
        }
        try { MonitorMatchDisconnectAndCleanPreMatchUi(); } catch { }
        // REMOVED: the per-second TryDismissStalePreMatchCharSelectUi poll. It ran FindObjectsOfType
        // heap walks every second (the [PreMatchCleanup-DIAG] spam), never successfully dismissed
        // anything (its gate is always true during a live prematch — the post-match char-select is a
        // genuinely live prematch, not stale UI), and caused the periodic in-match stutter/standbild.
        // Exit-to-lobby cleanup is now handled by the targeted disconnect monitor above.

        // CRITICAL: Stagger Harmony patch installation to prevent Il2Cpp native crash.
        // Installing 15+ Harmony patches on Il2Cpp types simultaneously at 35s triggers
        // Runtime::UnhandledException in Il2CppInterop. Installing 3 per frame avoids this.
        if (Time.realtimeSinceStartup >= _nextPatchAt && _patchIndex < 10)
        {
            _il2CppInteropReady = true;
            _nextPatchAt = Time.realtimeSinceStartup + 0.05f;

            var patchQueue = new Action[]
            {
                () => { TryInstallNetworkConfigPatches(); TryInstallPreAwakeManagerNetworkPatch(); },
                () => { TryInstallHttpClientHostPatch(); TryInstallControllerManagerNetworkPatch(); },
                () => { TryInstallLobbyNetworkClientPatches(); TryInstallWebSocketClientSelectionPatch(); },
                () => { TryInstallCharacterSelectionTrackerPatches(); TryInstallLifecycleGuardPatches(); TryInstallPrematchCleanupDisconnectPatches(); },
                () => { TryInstallGameModePatches(); TryInstallJoinDiagnosticsPatches(); },
                () => { TryInstallLoginControllerPatches(); TryInstallAnalyticsPatches(); },
                () => { TryInstallCharacterSelectNullRefPatch(); TryInstallLobbyControllerGuardPatches(); },
                () => { TryInstallUICharactersConfigPatch(); TryInstallUnityWebRequestHeaderPatch(); },
                () => { TryFetchServerPolicy(); PatchLoadedNetworkConfigs(); PatchLoadedLobbyNetworkClients(); PatchLoadedWebServers(); TryProtectLobbyUiFromDestruction(); },
                () => { TryInstallMirrorRpcDispatchGuard(); },
            };

            for (int i = 0; i < 3 && _patchIndex < patchQueue.Length; i++, _patchIndex++)
            {
                try { patchQueue[_patchIndex](); } catch (Exception ex) { LoggerInstance.Warning($"Staggered patch #{_patchIndex} failed: {ex.GetBaseException().Message}"); }
            }

            if (_patchIndex >= patchQueue.Length)
                LoggerInstance.Msg($"[PatchStagger] All {patchQueue.Length} patch groups installed over {(int)(_patchIndex / 3 + 1)} frames.");
        }

        if (!_dedicatedProcess &&
            _autoGuestLoginEntry?.Value == true &&
            HasCompleteLocalIdentity() &&
            _lastLoadResponse == null && // PERF: once logged in (LoadResponse arrived) these
                                         // LoginController/LobbyNetworkClient scans are pure
                                         // waste; without this gate they ran every 1.5s forever.
        ...
        if (!_autoJoinAttempted &&
            _patchIndex >= 10 && // never connect to a match before the PreMatch NullRef guard, the
                                 // Mirror RPC-dispatch guard, and the rest of the staggered patches
                                 // are installed; the host's TargetSetupPreMatch RPC runs
                                 // View_PreMatch_CharSelect.InitMap on connect, which throws NRE ->
                                 // Mirror disconnects ("Unknown Error") unless the guard is in place.
            ...
            Time.realtimeSinceStartup >= _nextBootstrapRepairAt)
        {
            ...
            {
                EnsureManagedBootstrapListener(_dedicatedHttpPort.Value);
                if (_bootstrapDeferredUntilIl2CppReadyLogCount < 3)
                {
                    _bootstrapDeferredUntilIl2CppReadyLogCount++;
                    SetBootstrapRepairStatus(
                        "Deferring dedicated bootstrap listener until Il2CppInterop is ready.",
                        logRepeated: false);
                }
                return;
            }
    ...
    public override void OnLateUpdate()
    {
        ...
        // Only start after 35s and when Il2CppInterop is ready. Spread the attempts to one per
        // second - running all 10 on consecutive frames produced a visible freeze burst at t=35s
        // (each attempt runs Resources.FindObjectsOfTypeAll strategies).
        if (_il2CppInteropReady &&
            Time.realtimeSinceStartup >= 35f &&
            !_charConfigPreloadDone &&
            _lateUpdateCharFixAttempts < 10 &&
            Time.realtimeSinceStartup >= _nextLateUpdateCharFixAt)
        {
            _nextLateUpdateCharFixAt = Time.realtimeSinceStartup + 1f;
            _lateUpdateCharFixAttempts++;
    ...
    {
        // Production mode (default): the custom-server overlay/panel is fully removed and unopenable.
        // No code path (F7, stray click, chip) may surface it; only the dev-mode opt-in can.
        if (_productionModeEntry?.Value != false) return;
    ...
    private void HandleNativeUiPointer(Vector3 screenPosition)
    {
        ...
        {
            // Never toggle the panel when the native UI / chip is disabled — otherwise a click that
            // happens to land in the (stale) chip rect, e.g. the Friends tab button, opens the overlay.
            if (_nativeUiEntry?.Value != true)
            {
                return;
            }
    ...
    private void CaptureIdentitySetupKeyboardInput()
    {
        ...
        // When the native uGUI InputField exists it captures keystrokes itself (and
        // CompleteIdentitySetupFromGui reads its .text). Running the manual per-key poller in
        // parallel double-types / garbles the name, so only poll as a fallback for the pure-IMGUI
        // path. Still honor Enter-to-submit in both paths.
        if (_nativeSetupNameInput != null)
        {
    ...
    private void RefreshNativeGameUi(bool syncInputs)
    {
        ...
        // Only assign when changed: every Text.text assignment dirties the uGUI canvas and forces
        // a rebuild, and this refresh runs on a 0.5s timer.
        if (_nativeChipText != null)
        {
            string chip = $"Custom Server\n{endpoint}";
            if (!string.Equals(_nativeChipText.text, chip, StringComparison.Ordinal)) _nativeChipText.text = chip;
        }

        if (_nativeEndpointText != null)
        {
            string endpointLabel = $"Active endpoint: {endpoint}";
            if (!string.Equals(_nativeEndpointText.text, endpointLabel, StringComparison.Ordinal)) _nativeEndpointText.text = endpointLabel;
        }

        if (_nativeStatusText != null)
        {
            string statusLabel = $"Status: {_statusText}";
            if (!string.Equals(_nativeStatusText.text, statusLabel, StringComparison.Ordinal)) _nativeStatusText.text = statusLabel;
        }
    ...
    private void ApplySettings(bool restartProxy, bool save)
    {
        ...
        // DISABLED: PatchLoadedNetworkConfigs calls FindType("BAPBAP.Network.NetworkConfig")
        // which triggers Il2CppInterop class init before Support Module has called Start().
        // Deferred to OnUpdate periodic batch (line 565) where it runs safely.
        // PatchLoadedNetworkConfigs();
    }
    ...
    private void LoadIniSettings()
    {
        ...
        {
            // The file exists but couldn't be read (transient AV/cloud-sync lock, IO error). Flag
            // it so SaveIniSettings backs up the original before any identity-regenerating overwrite,
            // rather than silently clobbering an established account on a transient failure.
            _iniReadFailed = true;
            LoggerInstance.Warning($"Failed to load custom server ini '{_iniPath}': {ex.GetBaseException().Message}. Existing file will be backed up before any overwrite.");
        }
    }
    ...
    private void SaveIniSettings()
    {
        try
        {
            ...
            // If the existing ini couldn't be read at startup (transient lock), preserve a backup
            // before overwriting so a regenerated identity never silently destroys the original.
            if (_iniReadFailed && File.Exists(_iniPath))
            {
                try
                {
                    string backupPath = _iniPath + ".bak";
                    File.Copy(_iniPath, backupPath, overwrite: true);
                    LoggerInstance.Warning($"Backed up unreadable ini to '{backupPath}' before overwriting.");
                }
                catch (Exception backupEx)
                {
                    LoggerInstance.Warning($"Could not back up existing ini before overwrite: {backupEx.GetBaseException().Message}");
                }
                _iniReadFailed = false;
            }
    ...
    private int GetDiscriminatorForHeaders()
    {
        ...
        // String.GetHashCode() is salted per-process on .NET Core/6, so the same accountId would
        // yield a different discriminator every launch. Use a stable FNV-1a hash so the player's
        // Name#discriminator tag stays consistent across sessions.
        return (int)(StableHash(accountId) % 9000) + 1000;
    }
    ...
    private void UpdateIdentitySetupRequirement(string reason)
    {
        ...
        // Auto-generate a complete local identity instead of prompting. The first-start setup window is
        // the ONE overlay surface that bypasses production-mode suppression; filling identity headlessly
        // here means _identitySetupRequired is never true, so the overlay can never appear or be opened.
        if (string.IsNullOrWhiteSpace(_usernameEntry.Value))
        {
            string gen = NormalizePlayerName("Player" + (StableHash(GenerateLocalAccountId()) % 9000 + 1000));
            if (gen.Length < 3) gen = "Player" + (StableHash(reason) % 9000 + 1000);
            _usernameEntry.Value = gen;
        }
        ...
        _setupUsernameText = _usernameEntry.Value.Trim();
        _setupErrorText = "";
        try { MelonPreferences.Save(); SaveIniSettings(); } catch { }
        if (!_identitySetupLogged)
        {
            _identitySetupLogged = true;
            LoggerInstance.Msg($"Custom server identity auto-generated after {reason} (username='{_usernameEntry.Value}'); setup overlay suppressed.");
        }
    }
    ...
    {
        return MelonIdentityHelpers.GenerateLocalAccountId();
    }

    private static string NormalizePlayerName(string value)
    {
        return MelonIdentityHelpers.NormalizePlayerName(value);
    }
    ...
    {
        return MelonIdentityHelpers.NormalizeIniToken(value);
    }
    ...
    private void ApplyCommandLineOverrides(string[] args)
    {
        ...
        foreach (string arg in args)
        {
            ...
            else if (TryGetArgValue(arg, "--bapcustom-auto-disconnect-match-after=", out string? autoDisconnectAfter) &&
                     float.TryParse(autoDisconnectAfter, out float parsedAutoDisconnectAfter) &&
                     parsedAutoDisconnectAfter > 0f)
            {
                _autoDisconnectMatchAfterSeconds = parsedAutoDisconnectAfter;
            }
            else if (arg.Equals("--bapcustom-autoplay", StringComparison.OrdinalIgnoreCase))
            {
                _autoplayEnabled = true;
            }
            else if (arg.Equals("--bapcustom-autoplay-queue", StringComparison.OrdinalIgnoreCase))
            {
                _autoplayEnabled = true;
                _autoplayQueueMode = true;
            }
    ...
    {
        return MelonArgHelpers.IsDedicatedGameServerProcess(args);
    }

    private static bool TryGetArgValue(string arg, string prefix, out string? value)
    {
        return MelonArgHelpers.TryGetArgValue(arg, prefix, out value);
    }

    private static bool TryParseBool(string value, out bool result)
    {
        return MelonArgHelpers.TryParseBool(value, out result);
    }

    private string GetConfiguredApiHost()
    {
        return MelonProxyHelpers.ResolveConfiguredApiHost(
            _localProxyEntry?.Value == true,
            _localPortEntry.Value,
            _hostEntry.Value,
            _serverPortEntry.Value,
            _httpsEntry.Value);
    }
    ...
    {
        return MelonProxyHelpers.NormalizeHost(host);
    }

    private static bool IsLoopbackHost(string host)
    {
        return MelonProxyHelpers.IsLoopbackHost(host);
    }
    ...
    {
        // TEMPORARILY DISABLED: The LobbyNetworkClientNetworkPostfix call chain triggers
        // il2cpp_object_get_class() on a partially-initialized object, causing 0xC0000005.
        // Will re-enable once the Support Module init path is fully stable.
    ...
    private void TryInstallMirrorRpcDispatchGuard()
    {
        if (_mirrorRpcGuardInstalled)
        {
            return;
        }

        // CLIENT-ONLY. The headless dedicated host receives incoming Commands through this same
        // Mirror dispatch path; swallowing there would mask genuine server-side handler errors.
        if (_dedicatedProcess)
        {
            _mirrorRpcGuardInstalled = true;
            return;
        }

        // DURABLE FIX for the in-match "Unknown Error" kick. Every incoming ClientRpc/TargetRpc the
        // host sends during PreMatch (TargetSetupPreMatch, TargetUpdatePreMatch, the spawn-selection
        // TargetRpcs, the countdown ClientRpcs, ...) funnels through Mirror's single dispatch
        // chokepoint before the user handler runs. Mirror's policy is to DISCONNECT the connection
        // whenever an RPC handler throws -- that disconnect is the "Unknown Error" modal. The
        // existing per-UI-method finalizers only cover a couple of handlers; installing the same
        // exception-swallowing finalizer on the central dispatch method keeps the connection alive
        // for ANY RPC handler throw, present or future, instead of playing whack-a-mole per method.
        try
        {
            ...
            // Primary: NetworkIdentity.HandleRemoteCall (void instance) is where every received RPC
            // is invoked. Fallback: RemoteProcedureCalls.Invoke (bool static) sits just below it.
            (string TypeName, string MethodName)[] dispatchTargets =
            {
                ("Mirror.NetworkIdentity", "HandleRemoteCall"),
                ("Mirror.RemoteProcedureCalls", "Invoke"),
            };

            int patched = 0;
            foreach ((string typeName, string methodName) in dispatchTargets)
            {
                Type? dispatchType = FindType(typeName);
                if (dispatchType == null)
                {
                    continue;
                }

                foreach (MethodInfo target in dispatchType.GetMethods(flags)
                ...
                if (patched > 0)
                {
                    // Stop at the first dispatch type that patched; HandleRemoteCall already covers
                    // all RPCs, so also patching Invoke would only double-swallow the same throw.
                    break;
                }
            }

            _mirrorRpcGuardInstalled = patched > 0;
            if (patched > 0)
            {
                LoggerInstance.Msg($"Installed Mirror RPC dispatch guard ({patched} method(s)) (durable fix for in-match 'Unknown Error' RPC-disconnect kick).");
            }
            else
            {
                LoggerInstance.Warning("Mirror RPC dispatch guard NOT installed: dispatch chokepoint method not found.");
            }
        }
        catch (Exception ex)
        {
            LoggerInstance.Warning($"Mirror RPC dispatch guard install failed: {ex.GetBaseException().Message}");
        }
    }
    ...
    private void TryInstallCharacterSelectNullRefPatch()
    {
        ...
        // These patches target lobby/match character-select UI pages (UILobbyCharacterSelectPage,
        // UILobbyMatchCharacterSelectPage, UILobbyCharacterCustomizePage). On the headless dedicated
        // match host (-batchmode -nographics) those pages are never rendered, so patching their Il2Cpp
        // methods there is pure liability: this patch class is known to trigger native Il2Cpp crashes
        // during in-match scene transitions. The host gets its character data from the bootstrap
        // payload, not from any UI page, so skip these entirely on the dedicated host.
        if (_dedicatedProcess)
        {
        ...
        // ROOT-CAUSE FIX for the in-match "Unknown Error" kick: the host sends the
        // TargetSetupPreMatch RPC, whose client handler runs UIPreMatch.PopulatePreMatchUI(qmd) ->
        // View_PreMatch_CharSelect.InitMap(). InitMap throws a NullReferenceException, and Mirror's
        // policy is to DISCONNECT the connection whenever an RPC handler throws -- that disconnect is
        // exactly the "Unknown Error" modal that kicks the player to the lobby right after champ
        // select, for every character. Installing the existing exception-swallowing finalizer on
        // these two methods lets the RPC handler complete instead of throwing, so Mirror keeps the
        // connection alive. Done before the playTabType early-return so it always installs on clients.
        try
        {
            MethodInfo? preMatchFinalizer = typeof(CustomServerMod).GetMethod(
                nameof(CustomServerNullRefFinalizer),
                BindingFlags.Static | BindingFlags.NonPublic);
            if (preMatchFinalizer != null)
            {
                int preMatchPatched = 0;
                (string TypeName, string MethodName)[] preMatchTargets =
                {
                    ("BAPBAP.UI.View_PreMatch_CharSelect", "InitMap"),
                    ("BAPBAP.UI.UIPreMatch", "PopulatePreMatchUI"),
                    ("BAPBAP.UI.UIPreMatch", "UpdatePreMatchUI"),
                    ("BAPBAP.UI.View_PreMatch_CharSelect", "Init"),
                    ("BAPBAP.UI.UIPreMatch", "Init"),
                };
                foreach ((string typeName, string methodName) in preMatchTargets)
                {
                    Type? preMatchType = FindType(typeName);
                    if (preMatchType == null)
                    {
                        continue;
                    }
                    foreach (MethodInfo target in preMatchType.GetMethods(BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic)
                                 .Where(m => m.Name == methodName && !m.ContainsGenericParameters))
                    {
                        if (PatchHarmonyFinalizer(target, preMatchFinalizer))
                        {
                            preMatchPatched++;
                        }
                    }
                }
                if (preMatchPatched > 0)
                {
                    LoggerInstance.Msg($"Installed {preMatchPatched} PreMatch UI NullRef guard(s) (fixes in-match 'Unknown Error' RPC-disconnect kick).");
                }
            }
        }
        catch (Exception ex)
        {
            LoggerInstance.Warning($"PreMatch UI NullRef guard install failed: {ex.GetBaseException().Message}");
        }
        ...
        try
        {
            ...
            if (charSelectType != null)
            {
                ...
                // OpenCharacterSummaryPage skip disabled - was experimental and blocked match entry
                ...
                MethodInfo? unlockDataPrefix = typeof(CustomServerMod).GetMethod(
                    nameof(CharacterPageInstanceHydratePrefix),
                    BindingFlags.Static | BindingFlags.NonPublic);
                if (unlockDataPrefix != null)
                {
                    foreach (string methodName in new[] { "SetCharacterButtonState", "UpdateCharacterIconButtonsState", "UpdateDataUnlockCharacter", "OpenCharSelectPanel", "InitializeCharacterUnlockPanel" })
                    {
                        foreach (MethodInfo target in charSelectType.GetMethods(BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic)
                                     .Where(m => m.Name == methodName))
                        {
                            if (PatchHarmonyPrefix(target, unlockDataPrefix))
                            {
                                patched++;
                            }
                        }
            ...
            if (matchCharSelectType != null)
            {
                ...
                // DISABLED: MatchCharacterSelectHydratePrefix caused native Il2Cpp crashes during
                // in-match scene transitions when mutating CharacterPageModel fields via reflection.
                // The lobby CharacterPageInstanceHydratePrefix still handles lobby char select.
                // MatchCharacterSelectHydratePrefix is still available for manual testing.
                // MethodInfo? matchHydratePrefix = typeof(CustomServerMod).GetMethod(
                //     nameof(MatchCharacterSelectHydratePrefix),
                //     BindingFlags.Static | BindingFlags.NonPublic);
                // if (matchHydratePrefix != null) { ... }

                MethodInfo? matchmakingUnlockedPrefix = typeof(CustomServerMod).GetMethod(
                    nameof(UpdateMatchmakingLockedStatusPrefix),
                    BindingFlags.Static | BindingFlags.NonPublic);
                if (matchmakingUnlockedPrefix != null)
                {
                    foreach (MethodInfo target in matchCharSelectType.GetMethods(BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic)
                                 .Where(m => m.Name == "UpdateMatchmakingLockedStatus"))
                    {
                        if (PatchHarmonyPrefix(target, matchmakingUnlockedPrefix))
                        {
                            patched++;
                        }
                    }
                ...
                {
                    // Skip the cosmetic teammate char-swap animation on the custom server: its
                    // MatchStartPanel slot for the broadcast accountId is never bound (in-match
                    // roster hydration is disabled because reflecting into CharacterPageModel
                    // crashes Il2Cpp), so the native body NREs on every per-player char-change.
                    // Skipping is purely cosmetic — actual char selection flows through
                    // OnCharacterButtonSelect and is authoritative on the host. This turns a
                    // throw-then-swallow into a clean no-op; the finalizer stays as a last resort.
                    if (charChangeAnimPrefix != null && PatchHarmonyPrefix(target, charChangeAnimPrefix))
                    {
                        patched++;
                    }
    ...
    private void TryInstallLobbyControllerGuardPatches()
    {
        ...
        MethodInfo? finalizer = typeof(CustomServerMod).GetMethod(
            nameof(CustomServerNullRefFinalizer),
            BindingFlags.Static | BindingFlags.NonPublic);
        MethodInfo? lobbyReturnPostfix = typeof(CustomServerMod).GetMethod(
            nameof(LobbyReturnMessagePostfix),
            BindingFlags.Static | BindingFlags.NonPublic);
        if (finalizer == null)
        {
            return;
        ...
        try
        {
            ...
            foreach (string targetName in lobbyControllerTargets)
            {
                             ...
                             .Where(m => m.Name == targetName))
                {
                    ...
                    if (lobbyReturnPostfix != null && IsLobbyReturnCleanupMessage(targetName) && PatchHarmonyPostfix(target, lobbyReturnPostfix))
                    {
                        patched++;
                    }
                }
    ...
    private void TryInstallCharacterUnlockPatches()
    {
        ...
        try
        {
            ...
            // SAFE: Only scan specific known types via FindType, NOT all assemblies.
            // Scanning all assemblies with GetTypes() on Il2Cpp types can crash.
            string[] targetTypes = {
                "BAPBAP.UI.CharacterPageModel",
                "BAPBAP.UI.UILobbyCharacterSelectPage",
                "BAPBAP.UI.UILobbyMatchCharacterSelectPage"
            };
            ...
            foreach (string typeName in targetTypes)
            {
                ...
                {
                    if ((method.Name != "CharacterIsUnlocked" &&
                         method.Name != "CharacterIsSelectable" &&
                         method.Name != "CharacterIsInRotation") ||
            ...
            if (_characterUnlockPatchesInstalled)
            {
                LoggerInstance.Msg($"Installed {patched} CharacterIsUnlocked force-unlock patch(es) (safe method).");
            }
        }
        catch (Exception ex)
        {
    ...
    private static bool IsLobbyReturnCleanupMessage(string methodName)
    {
        return methodName.Equals("HandleLeaveLobbyMessage", StringComparison.Ordinal);
    }

    private bool _charConfigViaUiManagerLogged;
    private bool _charConfigDiagLogged;
    private static bool _uimanagerFinalizerLogged;
    ...
    private static int[] GetClientCharacterIds()
    {
        ...
        // Always return all 16 characters (0-15) including Medusa.
        // The server sends all characters regardless, so the client should
        // match. VanillaCharacterCount (15) excluded Medusa and caused
        // desync when the custom char mod DLL was absent.
        return Enumerable.Range(0, MedusaCharacterId + 1).ToArray();
    }

    private static int[] NormalizeCharacterIds(IEnumerable<int> ids)
    {
        return MelonProxyHelpers.NormalizeCharacterIds(ids, MedusaCharacterId);
    }

    private void ApplyAuthoritativeCharacterIds(int[] charIds, string source)
    {
        int[] normalized = NormalizeCharacterIds(charIds);
        if (normalized.Length == 0)
        {
            return;
        }

        bool changed = !_lastAvailableCharacterIds.SequenceEqual(normalized);
        _lastAvailableCharacterIds = normalized;
        _authoritativeCharacterIdsReceived = true;
        PopulateAllUICharactersConfigurations(_lastAvailableCharacterIds);
        try { TryPopulateCharacterModelsDirect(); } catch { }

        if (!_serverAvailableCharactersLogged || changed)
        {
            _serverAvailableCharactersLogged = true;
            LoggerInstance.Msg($"[CharConfig] authoritative availableCharacters from {source}: [{string.Join(",", _lastAvailableCharacterIds)}].");
        }
    }

    private static bool IsCustomCharacterRuntimePresent()
    {
        try
        {
            string[] roots =
            ...
            {
                if (MelonProxyHelpers.HasKnownCustomCharacterRuntime(root))
                {
                    return true;
                }
            }
    ...
    private void TryFixCharactersConfigurationCrash()
    {
        ...
        try
        {
            ...
            // Strategy D: Use Il2Cpp generic FindObjectsOfType<UICharactersConfiguration> via Resources.FindObjectsOfTypeAll(Il2CppSystem.Type)
            if (updated == 0)
            {
                try
                {
                    object? sysType = GetIl2CppType(charsConfigType);
                    if (sysType != null)
                    {
                        MethodInfo? findAll = typeof(Resources).GetMethods(BindingFlags.Static | BindingFlags.Public)
                            .FirstOrDefault(m => m.Name == "FindObjectsOfTypeAll" &&
                                !m.IsGenericMethod &&
                                m.GetParameters().Length == 1 &&
                                (m.GetParameters()[0].ParameterType.FullName == "Il2CppSystem.Type" || m.GetParameters()[0].ParameterType.Name == "Type"));
                        if (findAll != null)
                        {
                            ...
                            if (result != null && result.Length > 0)
                            {
                                ...
                                if (updated > 0)
                                    MelonLogger.Msg($"[CharConfig] Strategy D (assignable) found {result.Length} config(s).");
                            }
                        }
                        else if (!_charConfigDiagLogged)
                        {
                            _charConfigDiagLogged = true;
                            MelonLogger.Msg("[CharConfig] Strategy D: FindObjectsOfTypeAll(Il2CppSystem.Type) overload NOT FOUND via reflection.");
                        }
                    }
                }
                catch (Exception ex)
                {
                    LoggerInstance.Warning($"[CharConfig] Strategy D error: {ex.GetBaseException().Message}");
                }
            }

            if (updated > 0)
            {
                if (_authoritativeCharacterIdsReceived && _characterModelUnlockStatePopulated)
                {
                    _charConfigPreloadDone = true;
                }

                LoggerInstance.Msg($"_lobbyAvailableCharacterIds populated on {updated} configurations (charIds [{string.Join(",", allCharIds)}], authoritative={_authoritativeCharacterIdsReceived}, unlockModels={_characterModelUnlockStatePopulated}).");
            }
        }
        catch (Exception ex)
        {
    ...
    private static void PopulateCharIdsOnInstance(object cfg, Type charsConfigType, int[] allCharIds, ref int updated)
    {
        ...
        try
        {
            ...
            // CRITICAL: Use the NATIVE setter via MethodInfo.Invoke, NOT FieldInfo.SetValue.
            // FieldInfo.SetValue on IL2CPP struct array fields silently fails because the
            // managed wrapper type (int[]) doesn't match the IL2CPP field type (Il2CppStructArray<int>).
            // MethodInfo.Invoke dispatches through the IL2CPP vtable which handles the conversion.
            MethodInfo? setMethod = instType.GetMethods(BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic)
                .FirstOrDefault(m => m.Name.Contains("set_") &&
                                     m.Name.Contains("lobbyAvailableCharacterIds") &&
                                     m.GetParameters().Length == 1);
            ...
            if (setMethod != null)
            {
                ...
                object? il2cppArr;
                if (paramType == typeof(int[]) || paramType.IsAssignableFrom(typeof(int[])))
                {
                    il2cppArr = allCharIds;
                }
                else
                {
                    il2cppArr = CreateIl2CppArray(paramType, allCharIds);
                }
                if (il2cppArr != null)
                {
                    setMethod.Invoke(cfg, new object[] { il2cppArr });
                    updated++;
                }
                else
                {
                    // Fallback: try FieldInfo.SetValue anyway
                    lobbyIdsField.SetValue(cfg, allCharIds);
                    updated++;
                }
            }
            else
            {
                // No native setter found; fallback to FieldInfo.SetValue
                object? newValue;
    ...
    private void TryInstallLifecycleGuardPatches()
    {
        ...
        try
        {
            ...
            if (patched > 0)
            {
                ...
                // Guard the lobby-startup window only. A permanent guard also swallows the user's
                // deliberate "Quit to Desktop" (which routes through Application.Quit), so expire
                // after the startup phase to let intentional quits through.
                _quitGuardExpireTime = Time.realtimeSinceStartup + 120f;
                LoggerInstance.Msg($"Installed lifecycle guard on {patched} Application.Quit overload(s). Guard expires in 120s (lobby-startup window).");
            }
        }
        catch (Exception ex)
        {
    ...
    private void TryInstallPrematchCleanupDisconnectPatches()
    {
        if (_prematchCleanupDisconnectPatchesInstalled || _dedicatedProcess)
        {
            return;
        }

        Type? gameNetworkManagerType = FindType("BAPBAP.Network.GameNetworkManager") ??
                                       FindType("Il2CppBAPBAP.Network.GameNetworkManager");
        Type? gameManagerType = FindType("BAPBAP.Game.GameManager") ??
                                FindType("Il2CppBAPBAP.Game.GameManager");

        MethodInfo? disconnectPostfix = typeof(CustomServerMod).GetMethod(
            nameof(GameNetworkManagerClientDisconnectPostfix),
            BindingFlags.Static | BindingFlags.NonPublic);
        MethodInfo? matchEndPostfix = typeof(CustomServerMod).GetMethod(
            nameof(GameManagerPostMatchSignalPostfix),
            BindingFlags.Static | BindingFlags.NonPublic);
        if (disconnectPostfix == null || matchEndPostfix == null)
        {
            return;
        }
        ...
        try
        {
            ...
            {
                foreach (string methodName in new[] { "OnClientDisconnect", "ClMatchmakingGameEnded" })
                {
                    foreach (MethodInfo target in gameNetworkManagerType.GetMethods(BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic)
                                 .Where(method => method.Name == methodName && !method.ContainsGenericParameters))
                    {
                        if (PatchHarmonyPostfix(target, disconnectPostfix))
                        {
                            patched++;
                        }
                    }
            ...
            if (gameManagerType != null)
            {
                foreach (string methodName in new[] { "TargetTryLeaveGame", "ClOnMatchEnded", "ClMatchmakingEnd", "RpcMatchmakingEnd", "RpcMatchEnd" })
                {
                    foreach (MethodInfo target in gameManagerType.GetMethods(BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic)
                                 .Where(method => method.Name == methodName && !method.ContainsGenericParameters))
                    {
                        if (PatchHarmonyPostfix(target, matchEndPostfix))
                        {
                            patched++;
                        }
                    }
            ...
            foreach ((string TypeName, string MethodName) in new[]
                     {
                         ("BAPBAP.UI.UIGameMode", "TryLeaveGame"),
                         ("Il2CppBAPBAP.UI.UIGameMode", "TryLeaveGame"),
                         ("BAPBAP.UI.UISettingsMenu", "OnExitMatch"),
                         ("Il2CppBAPBAP.UI.UISettingsMenu", "OnExitMatch"),
                     })
            {
                Type? uiType = FindType(TypeName);
                if (uiType == null)
            ...
            if (patched > 0)
            {
                _prematchCleanupDisconnectPatchesInstalled = true;
                LoggerInstance.Msg($"Installed {patched} post-match cleanup/lobby-return hook(s).");
            }
        }
        catch (Exception ex)
        {
            LoggerInstance.Warning($"TryInstallPrematchCleanupDisconnectPatches failed: {ex.GetBaseException().Message}");
        }
    }

    private static void GameNetworkManagerClientDisconnectPostfix()
    {
        try
        {
            s_active?.ArmPostMatchPrematchCleanup("GameNetworkManager client disconnect");
        }
        catch
        {
        }
    ...
    private static void GameManagerPostMatchSignalPostfix(System.Reflection.MethodBase __originalMethod)
    {
        try
        {
            string methodName = __originalMethod?.Name ?? "post-match signal";
            s_active?.ArmPostMatchPrematchCleanup($"GameManager/UI {methodName}", 30f);
        }
        catch
        {
        }
    ...
    private void TryInstallUICharactersConfigPatch()
    {
        ...
        try
        {
            ...
            // DISABLED: Accessing AvailableCharacterIds getter via Harmony DMD on IL2CPP
            // ScriptableObject causes 0xC0000005 (access violation). The UIManager.Awake
            // prefix below handles character config population more safely.
            ...
            //     nameof(EnsureLobbyCharIdsPrefix),
            //     BindingFlags.Static | BindingFlags.NonPublic);
            // if (availIdsGetter != null && availIdsPrefix != null) { PatchHarmonyPrefix(...); ... }
            ...
            if (uiManagerType != null)
            {
                ...
                // FINALIZER catches the UILobby.Build crash inside Awake and populates
                // character config from the UIManager instance BEFORE the lobby dies.
                MethodInfo? populateFinalizer = typeof(CustomServerMod).GetMethod(
                    nameof(UIManagerAwakeFinalizer),
                    BindingFlags.Static | BindingFlags.NonPublic);
                if (awakeMethod != null && populateFinalizer != null && PatchHarmonyFinalizer(awakeMethod, populateFinalizer))
                {
                    LoggerInstance.Msg("Installed finalizer on UIManager.Awake to suppress lobby build crash and populate char config.");
                }
            }
    ...
    // Harmony PREFIX on UIManager.Awake. Uses NO __instance parameter since Harmony
    // does not inject it reliably on IL2CPP. Instead finds UIManager via scene search.
    private static void UIManagerAwakePrefix()
    {
        try
        {
            ...
            {
                if (mgr == null) continue;
                PopulateChartsFromUIManagerInstance(mgr, allCharIds);
            }
        }
        catch { }
    }

    private static void PopulateChartsFromUIManagerInstance(object manager, int[] allCharIds)
    {
        try
        {
            Type uiManagerType = manager.GetType();

            // Dump field names for diagnostics on first run
            if (s_active != null && !s_active._charConfigDiagLogged)
            {
                var fields = uiManagerType.GetFields(BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
                foreach (var f in fields)
                {
                    object? val = f.GetValue(manager);
                    string valStr = val?.ToString() ?? "null";
                    if (valStr.Length > 80) valStr = valStr[..80];
                    s_active?.LoggerInstance.Msg($"[UIMgrField] {f.Name} ({f.FieldType.Name}) = {valStr}");
                }
                s_active._charConfigDiagLogged = true;
            }

            // Direct field access on the UIManager instance
            FieldInfo? charConfigField = uiManagerType.GetField("characterConfig",
                BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
            if (charConfigField == null)
            {
            ...
            object? cfg = charConfigField?.GetValue(manager);
            if (cfg != null)
            {
                int updated = 0;
                PopulateCharIdsOnInstance(cfg, cfg.GetType(), allCharIds, ref updated);
                if (updated > 0)
                {
                    if (s_active != null && s_active._authoritativeCharacterIdsReceived && s_active._characterModelUnlockStatePopulated)
                    {
                        s_active._charConfigPreloadDone = true;
                    }
                    s_active?.LoggerInstance.Msg($"[UIManagerAwakePrefix] Directly populated _lobbyAvailableCharacterIds. filled={allCharIds.Length} authoritative={s_active?._authoritativeCharacterIdsReceived} unlockModels={s_active?._characterModelUnlockStatePopulated}");
                }
                return;
            }

            // Fallback: owner field search
            Type? charsConfigType = FindType("BAPBAP.UI.UICharactersConfiguration");
            if (charsConfigType != null)
            {
                int updated = PopulateCharIdsFromOwnerFields(manager, charsConfigType, allCharIds, "UIManager.Awake fallback");
                if (updated > 0 && s_active != null && s_active._authoritativeCharacterIdsReceived && s_active._characterModelUnlockStatePopulated)
                {
                    s_active._charConfigPreloadDone = true;
                }
            }
    ...
    // Harmony FINALIZER on UIManager.Awake. This runs even when Awake throws an exception
    // (catching the UILobby.Build crash). It populates UICharactersConfiguration._lobbyAvailableCharacterIds
    // from the UIManager's serialized characterConfig field AND returns null to suppress the crash.
    // CRITICAL: NO __instance parameter - Harmony __instance injection is broken on IL2CPP.
    // Instead finds UIManager via FindLoadedUnityObjects.
    private static Exception? UIManagerAwakeFinalizer(Exception? __exception)
    {
        try
        {
            Type? charsConfigType = FindType("BAPBAP.UI.UICharactersConfiguration");
            if (charsConfigType == null) return null; // Suppress even without type - let UI continue

            int[] allCharIds = GetClientCharacterIds();
            int updated = 0;

            // DON'T use __instance - it's null on IL2CPP. Find UIManager via scene search.
            Type? uiManagerType = FindType("BAPBAP.UI.UIManager");
            if (uiManagerType != null)
            {
                Array? managers = FindLoadedUnityObjects(uiManagerType);
                if (managers != null)
                {
                    foreach (object mgr in managers)
                    {
                        if (mgr == null) continue;
                        Type mt = mgr.GetType();
                        FieldInfo? charConfigField = mt.GetField("characterConfig",
                            BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
                        if (charConfigField == null)
                            charConfigField = mt.GetField("_characterConfig",
                                BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
                        object? cfg = charConfigField?.GetValue(mgr);
                        if (cfg != null && charsConfigType.IsInstanceOfType(cfg))
                        {
                            PopulateCharIdsOnInstance(cfg, charsConfigType, allCharIds, ref updated);
                        }
                    }
            ...
            if (updated > 0)
            {
                ...
                s_active?.LoggerInstance.Msg($"[UIManagerFinalizer] _lobbyAvailableCharacterIds populated on {updated} config(s). authoritative={s_active?._authoritativeCharacterIdsReceived} unlockModels={s_active?._characterModelUnlockStatePopulated}");
            }
            else if (__exception != null)
            {
                s_active?.LoggerInstance.Msg($"[UIManagerFinalizer] caught exception but no config found (suppressed): {__exception.GetBaseException().Message}");
                if (!_uimanagerFinalizerLogged)
                {
                    _uimanagerFinalizerLogged = true;
                    s_active?.LoggerInstance.Warning($"[UIManagerFinalizer] Awake crashed (config NOT populated). Suppressing.");
                    // FULL DIAGNOSTIC: if UIManager.Awake throws partway, the in-match HUD sub-views
                    // (uiAbilities/cooldowns, uiMinimap, uiSettings = pause + death Spectate/Leave) are
                    // left unwired -> dead HUD. This stack identifies what's throwing so it can be fixed.
                    try { s_active?.LoggerInstance.Warning($"[UIManagerFinalizerDIAG] full stack:\n{__exception}"); } catch { }
                }
            }

            return null; // ALWAYS suppress exception - let UI continue building
        }
        catch (Exception innerEx)
        {
            s_active?.LoggerInstance.Warning($"[UIManagerFinalizer] Finalizer error (still suppressing): {innerEx.GetBaseException().Message}");
            return null; // Suppress original exception even if finalizer fails
        }
    }
    ...
    // Member cache for EnsureLobbyCharIdsPrefix. The prefix runs on EVERY
    // TryGetLobbyCharConfigByIndex call (per list entry per lobby UI rebuild), so per-call
    // GetField/GetMethods reflection lookups were a hot allocation/CPU source in the lobby.
    private static Type? s_lobbyCharIdsOwnerType;
    private static FieldInfo? s_lobbyCharIdsField;
    private static MethodInfo? s_lobbyCharIdsSetMethod;
    private static PropertyInfo? s_lobbyCharIdsLengthProp;
    private static Type? s_lobbyCharIdsArrayType;
    ...
    private static void EnsureLobbyCharIdsPrefix(object __instance)
    {
        try
        {
            ...
            if (!ReferenceEquals(t, s_lobbyCharIdsOwnerType))
            {
                s_lobbyCharIdsField = t.GetField("_lobbyAvailableCharacterIds",
                    BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
                // CRITICAL: Use the NATIVE setter via MethodInfo.Invoke, NOT FieldInfo.SetValue.
                // FieldInfo.SetValue on IL2CPP struct array fields silently fails because the
                // managed wrapper type (int[]) doesn't match the IL2CPP field type (Il2CppStructArray<int>).
                // MethodInfo.Invoke dispatches through the IL2CPP vtable which handles the conversion.
                s_lobbyCharIdsSetMethod = t.GetMethods(BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic)
                    .FirstOrDefault(m => (m.Name == "set__lobbyAvailableCharacterIds" ||
                                          m.Name.Contains("lobbyAvailableCharacterIds")) &&
                                         m.Name.Contains("set_") &&
                                         m.GetParameters().Length == 1);
                s_lobbyCharIdsLengthProp = null;
                s_lobbyCharIdsArrayType = null;
                s_lobbyCharIdsOwnerType = t;
            }

            FieldInfo? f = s_lobbyCharIdsField;
            if (f == null) return;
            ...
            // Also check Il2CppStructArray Length property via reflection (cached per array type).
            if (!needsFill && current != null)
            {
                Type arrayType = current.GetType();
                if (!ReferenceEquals(arrayType, s_lobbyCharIdsArrayType))
                {
                    s_lobbyCharIdsLengthProp = arrayType.GetProperty("Length");
                    s_lobbyCharIdsArrayType = arrayType;
                }

                if (s_lobbyCharIdsLengthProp != null && s_lobbyCharIdsLengthProp.GetValue(current) is int len && len == 0)
                {
                    needsFill = true;
                }
            }
            ...
            try
            {
                MethodInfo? setMethod = s_lobbyCharIdsSetMethod;
                if (setMethod != null)
                {
                    object? il2cppArr = CreateIl2CppArray(setMethod.GetParameters()[0].ParameterType, all);
                    if (il2cppArr != null)
                    {
                        setMethod.Invoke(__instance, new object[] { il2cppArr });
    ...
    // Harmony prefix that populates CharacterPageModel._data BEFORE the page builds.
    // This bypasses UICharactersConfiguration entirely - we write the character IDs
    // directly onto the data model so the page renders without crashing.
    private static void PopulateCharIdsBeforeBuildPrefix(object __instance)
    {
        try
        {
            if (__instance == null) return;
            int[] allCharIds = GetClientCharacterIds();
            if (allCharIds == null || allCharIds.Length == 0) return;

            Type pageType = __instance.GetType();
            // UILobbyCharacterSelectPage and UILobbyPlayTabPage store CharacterPageModel in _data
            FieldInfo? dataField = pageType.GetField("_data",
                BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
            if (dataField == null)
            {
                // Try alternate names
                dataField = pageType.GetField("data",
                    BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
            }
            if (dataField == null) return;

            object? data = dataField.GetValue(__instance);
            if (data == null) return;

            Type dataType = data.GetType();

            // Method 1: Try native setter for availableCharacters (works with Il2CppStructArray<int>)
            MethodInfo? setAvail = dataType.GetMethods(BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic)
                .FirstOrDefault(m => (m.Name.Contains("set_availableCharacters") || m.Name == "SetAvailableCharacters") && m.GetParameters().Length == 1);
            if (setAvail != null)
            {
                Type paramType = setAvail.GetParameters()[0].ParameterType;
                object? arr = CreateIl2CppArray(paramType, allCharIds);
                if (arr != null) setAvail.Invoke(data, new object[] { arr });
            }
            else
            {
                // Method 2: Direct field write if no setter
                FieldInfo? availField = dataType.GetField("availableCharacters",
                    BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
                if (availField != null)
                {
                    object? arr = CreateIl2CppArray(availField.FieldType, allCharIds) ?? (object)allCharIds;
                    availField.SetValue(data, arr);
                }
            }

            // Also populate charIdsInRotation so rotation-based features work
            FieldInfo? rotationField = dataType.GetField("charIdsInRotation",
                BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
            if (rotationField != null)
            {
                object? arr = CreateIl2CppArray(rotationField.FieldType, allCharIds) ?? (object)allCharIds;
                rotationField.SetValue(data, arr);
            }

            // Log the successful population once
            if (!_populateCharIdsDiagLogged)
            {
                _populateCharIdsDiagLogged = true;
                s_active?.LoggerInstance.Msg($"[PopulateCharIdsBeforeBuild] set availableCharacters=[{string.Join(",", allCharIds)}] on {dataType.FullName}");
            }
        }
        catch (Exception ex)
        {
            s_active?.LoggerInstance.Warning($"[PopulateCharIdsBeforeBuild] failed: {ex.GetBaseException().Message}");
        }
    }
    ...
    private void TryFetchServerPolicy()
    {
        ...
        try
        {
            ...
            int[] availableCharacters = NormalizeCharacterIds(MelonProxyHelpers.ExtractJsonIntArray(body, "availableCharacters"));
            ...
            {
                ApplyAuthoritativeCharacterIds(availableCharacters, "/api/server-config");
            }
            _serverPolicyFetched = true;
            UpdateUguiOverlayText();

            LoggerInstance.Msg(
                $"Server match policy fetched: {policy} (matchmaking={(allowMm ? "allowed" : "blocked")}, custom={(allowCm ? "allowed" : "blocked")}, chars={(_lastAvailableCharacterIds.Length > 0 ? string.Join(",", _lastAvailableCharacterIds) : "fallback")}). " +
    ...
    {
        return MelonProxyHelpers.BuildServerPolicyUrl(
            _localProxyEntry?.Value ?? true,
            _localPortEntry?.Value ?? DefaultLocalProxyPort,
            _hostEntry?.Value ?? "127.0.0.1",
            _serverPortEntry?.Value ?? 5198,
            _httpsEntry?.Value ?? false);
    }
    ...
    private void TryInstallAnalyticsPatches()
    {
        ...
        {
            int patched = 0;
            if (prefix != null && setupAnalytics != null && PatchHarmonyPrefix(setupAnalytics, prefix))
            {
                patched++;
            }

            Type? gameNetworkManagerType = FindType("Il2CppBAPBAP.Network.GameNetworkManager") ??
                                           FindType("BAPBAP.Network.GameNetworkManager");
            if (finalizer != null && gameNetworkManagerType != null)
            {
                foreach (MethodInfo target in gameNetworkManagerType.GetMethods(BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic)
                             .Where(method => method.Name == "OnClientConnect" && !method.ContainsGenericParameters))
                {
                    if (PatchHarmonyFinalizer(target, finalizer))
                    {
                        patched++;
            ...
            if (patched > 0)
            {
                ...
                LoggerInstance.Msg($"Installed {patched} Analytics custom-server guard patch(es).");
            }
        }
        catch (Exception ex)
        {
    ...
    private void RunAutoplayLogic()
    {
        ...
        // Direct-join mode (--bapcustom-join-*): the client is connected straight into the match
        // via ConnectMatchmaking and never touches the lobby, so the join/ready states don't apply.
        // Jump to the in-match state so the PreMatch lock-in driver runs and the client can enter.
        if (_autoJoinAttempted && _autoplayState < 3)
        {
            _autoplayState = 3;
        }
        ...
        try
        {
            switch (_autoplayState)
            {
                case 0: // Waiting for login to complete
                    ...
                    {
                        if (_autoplayQueueMode)
                        {
                            // The game auto-joins the lobby itself (which registers our socket's
                            // accountId server-side — required for GAME_STARTED targeting), so skip
                            // the WS join and just wait for that before POSTing queue/join.
                            _autoplayState = 2;
                            _autoplayNextActionAt = Time.realtimeSinceStartup + 8f;
                            LoggerInstance.Msg("[Autoplay] Login complete (queue mode), will join queue in 8s.");
                        }
                        else
                        {
                            _autoplayState = 1;
                ...
                case 2: // Set ready (custom) OR join public queue (test queue mode)
                    if (_autoplayQueueMode)
                    {
                        if (TryAutoplayJoinQueue())
                        {
                            _autoplayState = 4;
                            _autoplayNextActionAt = Time.realtimeSinceStartup + 2f;
                            LoggerInstance.Msg("[Autoplay] Queue join sent. Waiting for GAME_STARTED.");
                        }
                    }
                    else if (TryAutoplaySetReady())
                    {
                        _autoplayState = 3;
                ...
                case 3: // In match (PreMatch champ-select) - lock in a character, then detect match
                    TryAutoplayDetectMatch();
                    if (!_autoplayLockedIn && _autoplayLockAttempts < 40)
                    {
                        TryAutoplayLockInCharacter();
                    }
                    _autoplayNextActionAt = Time.realtimeSinceStartup + 0.5f;
                    break;

                case 4: // Queued: idle until GAME_STARTED flips _autoJoinAttempted (handled by the
                        // guard at the top of this method, which jumps to state 3). Re-poll slowly.
                    _autoplayNextActionAt = Time.realtimeSinceStartup + 2f;
                    break;
            }
        }
    ...
    // TEST-ONLY (--bapcustom-autoplay-queue): join the public matchmaking queue over HTTP. The WS
    // JOIN_LOBBY (state 1) must have registered this socket's accountId first, and the POST's
    // X-BAP-AccountId MUST match it or the server multicasts GAME_STARTED to a different identity.
    private bool TryAutoplayJoinQueue()
    {
        if (_autoplayQueueJoinSent) return true;
        _autoplayQueueJoinSent = true;

        string apiHost = GetConfiguredApiHost();
        string accountId = _accountIdEntry.Value.Trim();
        string username = _usernameEntry.Value.Trim();
        int charId = _requestedCharacterId.GetValueOrDefault(1);
        ...
        _ = Task.Run(async () =>
        {
            try
            {
                ...
                using var req = new HttpRequestMessage(HttpMethod.Post, $"{apiHost}/api/queue/join");
                if (!string.IsNullOrWhiteSpace(accountId)) req.Headers.Add(AccountHeaderName, accountId);
                if (!string.IsNullOrWhiteSpace(username)) req.Headers.Add(UsernameHeaderName, username);
                req.Content = new StringContent($"{{\"charId\":{charId}}}", Encoding.UTF8, "application/json");
                using HttpResponseMessage resp = await client.SendAsync(req).ConfigureAwait(false);
                string body = await resp.Content.ReadAsStringAsync().ConfigureAwait(false);
                LoggerInstance.Msg($"[Autoplay] POST /api/queue/join -> {(int)resp.StatusCode} acct={accountId} charId={charId} body={body}");
            }
            catch (Exception ex)
            {
                LoggerInstance.Warning($"[Autoplay] queue/join failed: {ex.GetBaseException().Message}");
            }
        });
    ...
    // TEST-ONLY (gated behind --bapcustom-autoplay): drives the game's own PreMatch lock-in
    // RPCs so a headless autoplay client can pass champion-select and enter the live match —
    // the synthetic-input path the lobby UI needs is ignored by this raw-input Unity build, so
    // we invoke the networked Commands directly instead of clicking. Real players are unaffected.
    private void TryAutoplayLockInCharacter()
    {
        _autoplayLockAttempts++;
        object? localPlayer = FindBestLocalPlayerManagerForClient();
        if (localPlayer == null)
        {
            return;
        }

        object? preMatch = GetMemberValueSafe(localPlayer, "playerPreMatch");
        if (preMatch == null)
        {
            return;
        }

        // Already locked on the server? Stop.
        if (GetBoolMember(preMatch, "LockedCharacter") || GetBoolMember(preMatch, "NetworkLockedCharacter"))
        {
            _autoplayLockedIn = true;
            LoggerInstance.Msg("[Autoplay] Character already locked in PreMatch.");
            return;
        }

        int charId = _requestedCharacterId.GetValueOrDefault(0);
        try
        {
            InvokeInstance(preMatch, "CmdTrySelectCharacter", localPlayer, charId);
            InvokeInstance(preMatch, "CmdTryLockCharacter", localPlayer);
            _autoplayLockedIn = true;
            LoggerInstance.Msg($"[Autoplay] Sent CmdTrySelectCharacter({charId}) + CmdTryLockCharacter via PreMatch.");
        }
        catch (Exception ex)
        {
            LoggerInstance.Warning($"[Autoplay] Lock-in attempt {_autoplayLockAttempts} failed: {ex.GetBaseException().Message}");
        }
    }
    ...
    // TEST-ONLY: allows smoke tests to reproduce "leave match -> return to lobby" without
    // manual input or waiting for a full bot round to end. Production clients never set this flag.
    private void TryAutoDisconnectMatchForTest()
    {
        if (_autoDisconnectMatchAfterSeconds is not > 0f || _autoDisconnectInvoked)
        {
            return;
        }

        if (!IsMirrorMatchClientConnected())
        {
            if (_autoDisconnectMatchObserved)
            {
                _autoDisconnectInvoked = true;
                ArmPostMatchPrematchCleanup("test auto-disconnect observed Mirror drop");
            }
            return;
        }
        ...
        if (!_autoDisconnectMatchObserved)
        {
            _autoDisconnectMatchObserved = true;
            _autoDisconnectMatchStartAt = Time.realtimeSinceStartup;
            LoggerInstance.Msg($"[PreMatchCleanupTest] Mirror match connected; auto-disconnect scheduled in {_autoDisconnectMatchAfterSeconds.Value:0.##}s.");
            return;
        }

        if (Time.realtimeSinceStartup - _autoDisconnectMatchStartAt < _autoDisconnectMatchAfterSeconds.Value)
        {
            return;
        }

        _autoDisconnectInvoked = true;
        bool requested = TryDisconnectMatchClientForTest();
        LoggerInstance.Msg($"[PreMatchCleanupTest] Requested test match disconnect requested={requested}.");
        ArmPostMatchPrematchCleanup("test auto-disconnect requested");
    }

    private bool TryDisconnectMatchClientForTest()
    {
        ...
        try
        {
            ...
            if (gameNetworkManagerType != null)
            {
                foreach (object manager in FindGameNetworkManagers(gameNetworkManagerType))
                {
                    ...
                    requested = TryInvokeInstanceIfPresent(manager, "StopClient") || requested;
                    requested = TryInvokeInstanceIfPresent(manager, "Disconnect") || requested;
                    requested = TryInvokeInstanceIfPresent(manager, "StopHost") || requested;
                    if (requested)
                    {
                        return true;
                    }
        ...
        {
            LoggerInstance.Warning($"[PreMatchCleanupTest] GameNetworkManager disconnect failed: {ex.GetBaseException().Message}");
        }

        try
        {
            Type? networkClientType = FindType("Mirror.NetworkClient") ?? FindType("Il2CppMirror.NetworkClient");
            MethodInfo? disconnect = networkClientType?.GetMethod("Disconnect", BindingFlags.Static | BindingFlags.Public | BindingFlags.NonPublic, null, Type.EmptyTypes, null);
            if (disconnect != null)
            {
                disconnect.Invoke(null, Array.Empty<object>());
                requested = true;
            }
        }
        catch (Exception ex)
        {
            LoggerInstance.Warning($"[PreMatchCleanupTest] Mirror.NetworkClient disconnect failed: {ex.GetBaseException().Message}");
        }
        ...
        try
        {
            ...
            while (true)
            {
                ...
                if (!headersParsed)
                {
                    ...
                    MelonBootstrapHelpers.ParseState state = MelonBootstrapHelpers.TryParseHttpHead(
                        pending.GetBuffer(), (int)pending.Length, out method, out path, out contentLength, out headerEnd);
                    if (state == MelonBootstrapHelpers.ParseState.NeedMoreData)
                    {
                        continue;
                    }
                    if (state == MelonBootstrapHelpers.ParseState.BadRequest)
                    {
                        await WriteBootstrapTcpResponseAsync(stream, 400, "Bad Request", "{\"ok\":false}").ConfigureAwait(false);
                        return;
                    }
            ...
            {
                SetBootstrapRepairStatus(
                    "Bootstrap listener is warming up; waiting for Il2CppInterop ready before accepting match payloads.",
                    logRepeated: false);
                await WriteBootstrapTcpResponseAsync(stream, 503, "Service Unavailable", "{\"ok\":false,\"warmingUp\":true}").ConfigureAwait(false);
                return;
            }
    ...
    private void DrainBootstrapPayloads()
    {
        ...
        if (!_il2CppInteropReady)
        {
            if (_retryBootstrapPayloads.Count > 0 && _bootstrapDeferredUntilIl2CppReadyLogCount < 3)
            {
                _bootstrapDeferredUntilIl2CppReadyLogCount++;
                SetBootstrapRepairStatus(
                    "Waiting for Il2CppInterop ready before applying managed bootstrap payload.",
                    logRepeated: false);
            }
            return;
        }
    ...
    private void ForceDismissSplashUiInMatch()
    {
        ...
        try
        {
            ...
            if (!string.Equals(sceneName, _splashDismissSceneName, StringComparison.Ordinal))
            {
                // New scene: give the splash/loading overlay a fresh dismiss window.
                _splashDismissSceneName = sceneName;
                _splashDismissSceneChangedAt = Time.realtimeSinceStartup;
            }
            else if (Time.realtimeSinceStartup - _splashDismissSceneChangedAt > 45f)
            {
                // PERF: a loading overlay only ever needs dismissing shortly after a scene
                // transition. Without this gate the scan ran every second for the entire
                // session (UIManager reflection + object scans) even deep into a match.
                return;
            }

            bool clearingPostMatchLoading = _postMatchLoadingClearUntil > Time.realtimeSinceStartup;
            ...
            bool runtimeLooksLikeMatch = !sceneLooksLikeMatch && HasActiveMatchRuntimeCached(3f);

            if (!sceneLooksLikeMatch && !runtimeLooksLikeMatch && !clearingPostMatchLoading)
            {
                return;
            }

            bool allowTargetedHeapSweep =
                clearingPostMatchLoading ||
                (runtimeLooksLikeMatch &&
                 sceneName.Equals("MainScene", StringComparison.OrdinalIgnoreCase) &&
                 Time.realtimeSinceStartup - _splashDismissSceneChangedAt > 8f);

            int hidden = HideMatchLoadingUi(sceneName, allowTargetedHeapSweep);
            if (hidden > 0 && (!_matchLoadingOverlayHiddenLogged || hidden != _lastMatchLoadingOverlayHiddenCount))
            {
                ...
                LoggerInstance.Msg($"Hid {hidden} match loading/splash overlay object(s). activeScene='{sceneName}' runtimeMatch={runtimeLooksLikeMatch} postMatchClear={clearingPostMatchLoading}");
            }
            else if (clearingPostMatchLoading && _postMatchLoadingClearLogCount < 6)
            {
                _postMatchLoadingClearLogCount++;
                LoggerInstance.Msg($"[LobbyReturnFallback] Post-match loading clear tick activeScene='{sceneName}' hidden={hidden}.");
            }
        }
        catch (Exception ex)
        {
    ...
    private void MonitorMatchDisconnectAndCleanPreMatchUi()
    {
        if (_dedicatedProcess || !_il2CppInteropReady)
        {
            return;
        }

        float now = Time.realtimeSinceStartup;
        bool connected = IsMirrorMatchClientConnected();
        if (connected)
        {
            _matchMirrorWasConnected = true;
            _postMatchExitCleanupEligible = false;
            _postMatchCleanupDismissed = false;
            _postMatchLobbyReturnDueAt = 0f;
            _postMatchLobbyReturnUntil = 0f;
            _postMatchLobbyReturnAttempts = 0;
            _postMatchLobbyReturnLoadInProgress = false;
            _postMatchLobbyReturnSuppressUntil = 0f;
            _postMatchLoadingClearUntil = 0f;
            _postMatchLoadingClearLogCount = 0;
            // PERF: the uncached probe ran 2-3 FindObjectsOfType scans EVERY FRAME for the whole
            // match (the main constant in-match stutter source). The probe only resets two cleanup
            // flags here, so a 3s-cached answer is more than fresh enough.
            if (HasActiveMatchRuntimeCached(3f))
            {
                _postMatchPrematchCleanupUntil = 0f;
                _stalePrematchConsecutive = 0;
            }

            return;
        }

        if (_matchMirrorWasConnected)
        {
            _matchMirrorWasConnected = false;
            ArmPostMatchPrematchCleanup("Mirror match connection dropped");
        }

        TryReturnToLobbyIfMatchSceneStuck(now);

        if (!_postMatchExitCleanupEligible || _postMatchCleanupDismissed)
        {
            return;
        }

        if (_postMatchPrematchCleanupUntil <= 0f || now > _postMatchPrematchCleanupUntil)
        {
            _postMatchExitCleanupEligible = false;
            return;
        }

        if (now < _nextStalePrematchDismissAt)
        {
            return;
        }

        _nextStalePrematchDismissAt = now + 2.0f;
        TryDismissStalePreMatchCharSelectUi();
    }

    private void ArmPostMatchPrematchCleanup(string reason, float seconds = 18f)
    {
        if (_dedicatedProcess)
        {
            return;
        ...
        float now = Time.realtimeSinceStartup;
        if (_postMatchLobbyReturnSuppressUntil > now)
        {
            if (_postMatchLobbyReturnLogCount < 12)
            {
                _postMatchLobbyReturnLogCount++;
                LoggerInstance.Msg($"[LobbyReturnFallback] Ignoring stale post-match signal during lobby return suppression ({reason}).");
            }
            return;
        }

        _postMatchExitCleanupEligible = true;
        _postMatchCleanupDismissed = false;
        _postMatchPrematchCleanupUntil = Math.Max(_postMatchPrematchCleanupUntil, now + seconds);
        _nextStalePrematchDismissAt = now + 0.25f;
        _stalePrematchConsecutive = 0;
        SchedulePostMatchLobbyReturn(reason, seconds);

        if (_prematchCleanupArmLogCount < 20)
        {
            _prematchCleanupArmLogCount++;
            LoggerInstance.Msg($"[PreMatchCleanup-DIAG] Armed stale PreMatch cleanup for {seconds:0}s ({reason}).");
        }
    }

    private void SchedulePostMatchLobbyReturn(string reason, float seconds)
    {
        float now = Time.realtimeSinceStartup;
        _postMatchLobbyReturnDueAt = Math.Max(_postMatchLobbyReturnDueAt, now + 8f);
        _postMatchLobbyReturnUntil = Math.Max(_postMatchLobbyReturnUntil, now + Math.Max(seconds + 30f, 45f));

        if (_postMatchLobbyReturnLogCount < 12)
        {
            _postMatchLobbyReturnLogCount++;
            LoggerInstance.Msg($"[LobbyReturnFallback] Armed fallback after post-match signal ({reason}).");
        }
    }

    private void TryReturnToLobbyIfMatchSceneStuck(float now)
    {
        if (_dedicatedProcess || _postMatchLobbyReturnDueAt <= 0f)
        {
            return;
        }
        ...
        if (now < _postMatchLobbyReturnDueAt)
        {
            return;
        }

        if (_postMatchLobbyReturnUntil > 0f && now > _postMatchLobbyReturnUntil)
        {
            _postMatchLobbyReturnDueAt = 0f;
            _postMatchLobbyReturnUntil = 0f;
            _postMatchLobbyReturnAttempts = 0;
        ...
        string sceneName = GetActiveSceneNameSafe();
        // BAPBAP uses "MainScene" for both the lobby/menu shell and the live match runtime.
        // Treat a lobby-like scene name as safe only after the match objects/state are gone;
        // otherwise the post-match fallback would never fire for MainScene-based matches.
        bool runtimeLooksLikeMatch = HasActiveMatchRuntimeCached(1f) ||
                                     string.Equals(GetPreMatchStateSafe(), "InGame", StringComparison.OrdinalIgnoreCase);
        if (IsLobbyLikeSceneName(sceneName) && !runtimeLooksLikeMatch)
        {
            _postMatchLobbyReturnDueAt = 0f;
            _postMatchLobbyReturnUntil = 0f;
            _postMatchLobbyReturnAttempts = 0;
        ...
        if (IsMirrorMatchClientConnected())
        {
            _postMatchLobbyReturnDueAt = now + 3f;
            return;
        }

        if (_postMatchLobbyReturnAttempts >= 2)
        {
            return;
        }

        _postMatchLobbyReturnAttempts++;
        int stopped = StopMatchNetworkingForLobbyReturn();
        LoggerInstance.Warning(
            $"[LobbyReturnFallback] Client still in non-lobby scene '{sceneName}' after match disconnect/end. " +
            $"Loading MainScene fallback attempt={_postMatchLobbyReturnAttempts} stoppedNetwork={stopped}.");

        if (!TryLoadMainSceneForLobbyReturn(out string loadError))
        {
            LoggerInstance.Warning($"[LobbyReturnFallback] SceneManager.LoadScene(MainScene) failed: {loadError}");
            _postMatchLobbyReturnDueAt = now + 8f;
            return;
        }

        ClearDirectJoinStateForLobbyReturn();
        _postMatchLobbyReturnLoadInProgress = true;
        _postMatchLobbyReturnSuppressUntil = now + 120f;
        _postMatchLoadingClearUntil = now + 45f;
        _postMatchLoadingClearLogCount = 0;
        _postMatchLobbyReturnDueAt = 0f;
        _postMatchLobbyReturnUntil = 0f;
    }

    private void ClearDirectJoinStateForLobbyReturn()
    {
        if (!string.IsNullOrWhiteSpace(_autoJoinGameAuthId) ||
            _autoJoinWsPort.HasValue ||
            _autoJoinKcpPort.HasValue ||
            _autoJoinTcpPort.HasValue)
        {
            LoggerInstance.Msg("[LobbyReturnFallback] Cleared direct match auto-join state after post-match lobby return.");
        }

        _autoJoinGameAuthId = null;
        _autoJoinHost = "127.0.0.1";
        _autoJoinWsPort = null;
        _autoJoinKcpPort = null;
        _autoJoinTcpPort = null;
        _autoJoinAttempted = true;
    }

    private static bool TryLoadMainSceneForLobbyReturn(out string error)
    {
        error = "";
        try
        {
            ...
            {
                error = "SceneManager type not found";
                return false;
            }

            MethodInfo? loadSceneStr = sceneManagerType.GetMethods(BindingFlags.Static | BindingFlags.Public)
                .FirstOrDefault(m => m.Name == "LoadScene" &&
                                     m.GetParameters().Length == 1 &&
                                     m.GetParameters()[0].ParameterType == typeof(string));
            if (loadSceneStr == null)
            {
                error = "LoadScene(string) overload not found";
                return false;
            }

            loadSceneStr.Invoke(null, new object[] { "MainScene" });
            return true;
        }
        catch (Exception ex)
        {
    ...
    private int StopMatchNetworkingForLobbyReturn()
    {
        int stopped = 0;
        try
        {
            ...
            if (gameNetworkManagerType != null)
            {
                foreach (object manager in FindGameNetworkManagers(gameNetworkManagerType))
                {
                    try { if (TryInvokeInstanceIfPresent(manager, "StopClient")) stopped++; } catch { }
                    try { if (TryInvokeInstanceIfPresent(manager, "Disconnect")) stopped++; } catch { }
                }
            }
        ...
        {
            LoggerInstance.Warning($"[LobbyReturnFallback] Stop match networking failed: {ex.GetBaseException().Message}");
        }

        return stopped;
    }

    // After a match ends the game returns to the lobby, but the in-match PreMatch champ-select view
    // (View_PreMatch_CharSelect / UIPreMatch) can survive the transition because lobby canvases are
    // kept alive (DontDestroyOnLoad). Only run after a real Mirror match disconnect. Lobby-owned
    // character-select pages are closed through their own close methods only; never deactivate their
    // GameObjects here because those objects also participate in normal lobby layout/state.
    private void TryDismissStalePreMatchCharSelectUi()
    {
        if (_dedicatedProcess || !_postMatchExitCleanupEligible || _postMatchCleanupDismissed)
        {
            return;
        }
        ...
        try
        {
            ...
            if (!lobbyScene || IsMirrorMatchClientConnected())
            {
                return;
            }
            ...
            int dismissed = 0;
            int hiddenParents = 0;
            int lobbyPageCloseCalls = 0;
            if (IsInMatchPreMatchCharSelectVisible())
            {
                forceClosed = ForceCloseStalePreMatchParents();
                dismissed = DeactivateVisiblePreMatchCharSelectViews();
                hiddenParents = HideActivePreMatchParentObjects();
            }
            else
            {
                lobbyPageCloseCalls = CloseStaleLobbyMatchCharacterSelectPages();
            }

            int total = forceClosed + dismissed + hiddenParents + lobbyPageCloseCalls;
            if (total > 0)
            {
                _postMatchCleanupDismissed = true;
                if (_stalePrematchDismissLogCount < 12)
                {
                    _stalePrematchDismissLogCount++;
                    LoggerInstance.Msg($"[PreMatchCleanup] Dismissed post-match UI on '{sceneName}': uiPreMatchClosed={forceClosed} charViews={dismissed} parents={hiddenParents} lobbyPageCloseCalls={lobbyPageCloseCalls}.");
                }
            ...
            float now = Time.realtimeSinceStartup;
            if (_postMatchPrematchCleanupUntil > now && _stalePrematchNotFoundLogCount < 8)
            {
                _stalePrematchNotFoundLogCount++;
                LoggerInstance.Msg($"[PreMatchCleanup-DIAG] post-match cleanup armed but no dismissable UI yet scene='{sceneName}' state={GetPreMatchStateSafe()}.");
            }
        }
        catch (Exception ex)
        {
            LoggerInstance.Warning($"TryDismissStalePreMatchCharSelectUi failed: {ex.GetBaseException().Message}");
        }
    }

    private static bool IsInMatchPreMatchCharSelectVisible()
    {
        foreach (string typeName in new[]
                 {
                     "BAPBAP.UI.View_PreMatch_CharSelect",
                     "Il2CppBAPBAP.UI.View_PreMatch_CharSelect"
                 })
        {
            Type? viewType = FindType(typeName);
            Array? views = viewType == null ? null : FindLoadedUnityObjects(viewType);
            if (views == null)
            {
                continue;
            }
            ...
            {
                if (view != null && GetMemberValue(view, "gameObject") is GameObject go && go.activeInHierarchy)
                {
                    return true;
                }
            }
        ...
        foreach (string typeName in new[]
                 {
                     "BAPBAP.UI.UIPreMatch",
                     "Il2CppBAPBAP.UI.UIPreMatch"
                 })
        {
            Type? uiType = FindType(typeName);
            Array? uiObjects = uiType == null ? null : FindLoadedUnityObjects(uiType);
            if (uiObjects == null)
            {
                continue;
            }
            ...
            foreach (object? uiObject in uiObjects)
            {
                ...
                if (GetMemberValue(uiObject, "_preMatchParentObject") is GameObject parent && parent.activeInHierarchy)
                {
                    return true;
                }

                object? charSelect = GetMemberValue(uiObject, "CharSelect");
                if (charSelect != null && GetMemberValue(charSelect, "gameObject") is GameObject charSelectGo && charSelectGo.activeInHierarchy)
                {
                    return true;
                }
            }
    ...
    private static int CloseStaleLobbyMatchCharacterSelectPages()
    {
        int actions = 0;

        foreach (string typeName in new[] { "BAPBAP.UI.UILobby", "Il2CppBAPBAP.UI.UILobby" })
        {
            Type? lobbyType = FindType(typeName);
            Array? lobbies = lobbyType == null ? null : FindLoadedUnityObjects(lobbyType);
            if (lobbies == null)
            {
                continue;
            }
            ...
            foreach (object? lobby in lobbies)
            {
                ...
                if (TryInvokeInstanceIfPresent(lobby, "CloseCharacterSelectPage", true) ||
                    TryInvokeInstanceIfPresent(lobby, "CloseCharacterSelectPage"))
                {
                    actions++;
                }
            }
        ...
        string[] closeMethods =
        {
            "ClosePreMatchInterface",
            "CloseCharacterSelect",
            "CloseMatchFoundPanel",
            "CloseMatchStartPanel",
            "CloseSpawnSelect",
            "CloseSkinSelect"
        };
        ...
        foreach (string typeName in new[]
                 {
                     "BAPBAP.UI.UILobbyMatchCharacterSelectPage",
                     "Il2CppBAPBAP.UI.UILobbyMatchCharacterSelectPage"
                 })
        {
            Type? pageType = FindType(typeName);
            Array? pages = pageType == null ? null : FindLoadedUnityObjects(pageType);
            if (pages == null)
            {
                continue;
            }
            ...
            foreach (object? page in pages)
            {
                ...
                if (GetMemberValue(page, "gameObject") is not GameObject pageGo || !pageGo.activeInHierarchy)
                {
                    continue;
                }

                foreach (string methodName in closeMethods)
                {
                    if (TryInvokeInstanceIfPresent(page, methodName))
                    {
                        actions++;
                    }
                }

                if (TryInvokeInstanceIfPresent(page, "OnPageClose"))
                {
                    actions++;
                }

                // Do not deactivate the page GameObject here. In the lobby scene that object also
                // participates in normal lobby layout/state, and disabling it can remove the top UI.
            }
        }
    ...
    private static int DeactivateVisiblePreMatchCharSelectViews()
    {
        int dismissed = 0;
        foreach (string typeName in new[]
                 {
                 ...
                 })
        {
            ...
            foreach (object? view in views)
            {
                ...
                if (TryInvokeInstanceIfPresent(view, "Close", true) ||
                    TryInvokeInstanceIfPresent(view, "Close"))
                {
                    dismissed++;
                }

                if (GetMemberValue(view, "gameObject") is GameObject go && go.activeInHierarchy)
                {
                    go.SetActive(false);
                    dismissed++;
                }
    ...
    private static int HideActivePreMatchParentObjects()
    {
        int hidden = 0;
        foreach (string typeName in new[]
                 {
                 ...
                 })
        {
            ...
            foreach (object? uiObject in uiObjects)
            {
                ...
                foreach (string fieldName in new[] { "_preMatchParentObject", "_timerParentObject", "_bannerParentObject" })
                {
                    if (GetMemberValue(uiObject, fieldName) is GameObject parent && parent.activeInHierarchy)
                    {
    ...
    private static int ForceCloseStalePreMatchParents()
    {
        Type? type = FindType("BAPBAP.UI.UIPreMatch") ?? FindType("Il2CppBAPBAP.UI.UIPreMatch");
        Array? objs = type == null ? null : FindLoadedUnityObjects(type);
        ...
        foreach (object? o in objs)
        {
            ...
            try
            {
                if (TryInvokeInstanceIfPresent(o, "ForceClose"))
                {
                    closed++;
                    continue;
                }

                if (GetMemberValue(o, "gameObject") is GameObject go && go.activeInHierarchy)
                {
                    go.SetActive(false);
                    closed++;
                }
    ...
    private static bool IsLobbyLikeSceneName(string sceneName)
    {
        return sceneName.IndexOf("Main", StringComparison.OrdinalIgnoreCase) >= 0 ||
               sceneName.IndexOf("Lobby", StringComparison.OrdinalIgnoreCase) >= 0 ||
               sceneName.IndexOf("Menu", StringComparison.OrdinalIgnoreCase) >= 0 ||
               sceneName.IndexOf("Login", StringComparison.OrdinalIgnoreCase) >= 0 ||
               sceneName.IndexOf("Bootstrap", StringComparison.OrdinalIgnoreCase) >= 0;
    }

    // Returns the current PreMatchManager.CurrentState as a string ("<none>" if no manager exists).
    // PreMatchState enum: WaitingForPlayers, CharSelect, SpawnSelect, SpawnLock, InGame.
    private static string GetPreMatchStateSafe()
    {
        try
        {
            foreach (string typeName in new[] { "BAPBAP.Game.PreMatchManager", "Il2CppBAPBAP.Game.PreMatchManager" })
            {
                ...
                if (t == null) continue;
                Array? all = FindLoadedUnityObjects(t);
                if (all == null || all.Length == 0) continue;
                foreach (object? pm in all)
                {
                    if (pm == null) continue;
                    object? state = GetMemberValue(pm, "CurrentState") ?? GetMemberValue(pm, "NetworkCurrentState");
                    if (state != null) return state.ToString() ?? "<unknown>";
                }
            }
        }
        catch { }
    ...
    // The match runtime spawns PreMatchManager/View_PreMatch_CharSelect over the client's Mirror
    // connection to the dedicated MATCH host (separate from the lobby WebSocket). A LIVE prematch
    // requires that connection (CharSelect is a SyncVar pushed from the host); after exit-to-lobby the
    // host is killed so isConnected==false even though the orphaned objects linger. Fail-safe: if the
    // Mirror type can't be resolved, return false (presumed stale) so the dismiss gate can never deadlock.
    private static PropertyInfo? s_mirrorIsConnectedProp;
    private static PropertyInfo? s_mirrorActiveProp;
    private static bool s_mirrorPropsResolved;

    private static bool IsMirrorMatchClientConnected()
    {
        try
        {
            // Resolve the static Mirror.NetworkClient properties ONCE - this runs every frame,
            // so per-call GetProperty reflection lookups are pure overhead.
            if (!s_mirrorPropsResolved)
            {
                Type? t = FindType("Mirror.NetworkClient") ?? FindType("Il2CppMirror.NetworkClient");
                if (t == null) return false;
                s_mirrorIsConnectedProp = t.GetProperty("isConnected", BindingFlags.Static | BindingFlags.Public);
                s_mirrorActiveProp = t.GetProperty("active", BindingFlags.Static | BindingFlags.Public);
                s_mirrorPropsResolved = true;
            }

            if (s_mirrorIsConnectedProp != null && s_mirrorIsConnectedProp.GetValue(null) is bool c) return c;
            if (s_mirrorActiveProp != null && s_mirrorActiveProp.GetValue(null) is bool a) return a;
        }
        catch { }
        return false;
    }

    // An "active" prematch state is one where the champ-select/spawn-select UI is LEGITIMATELY up.
    // Anything else (terminal, in-game, or no manager) means a lingering view is stale.
    private static bool IsActivePrematchState(string state)
    {
        return state.IndexOf("CharSelect", StringComparison.OrdinalIgnoreCase) >= 0
            || state.IndexOf("SpawnSelect", StringComparison.OrdinalIgnoreCase) >= 0
            || state.IndexOf("SpawnLock", StringComparison.OrdinalIgnoreCase) >= 0
            || state.IndexOf("WaitingForPlayers", StringComparison.OrdinalIgnoreCase) >= 0;
    }

    /// <summary>
    /// Cached wrapper around <see cref="HasActiveMatchRuntime"/>. The raw probe runs multiple
    /// FindObjectsOfType scans (expensive in IL2CPP); callers on per-frame or per-second paths
    /// must use this wrapper so at most one scan burst runs per <paramref name="maxAgeSeconds"/>.
    /// </summary>
    private bool HasActiveMatchRuntimeCached(float maxAgeSeconds)
    {
        float now = Time.realtimeSinceStartup;
        if (now - _matchRuntimeProbeCachedAt < maxAgeSeconds)
        {
            return _matchRuntimeProbeCachedValue;
        }

        _matchRuntimeProbeCachedValue = HasActiveMatchRuntime();
        _matchRuntimeProbeCachedAt = now;
        return _matchRuntimeProbeCachedValue;
    }
    ...
    private bool HasActiveMatchRuntime()
    {
        try
        {
            // NOTE: deliberately NOT scanning FindLoadedUnityObjects(typeof(GameObject)) here.
            // Enumerating the ENTIRE Il2Cpp GameObject heap every second while the match scene is
            // still instantiating/GC-moving is a dangling-pointer native access violation (an
            // uncatchable crash during the "loading screen" transition). The type-specific lookups
            // below detect an active match just as well and only touch the small per-type tables.
    ...
    private int HideMatchLoadingUi(string sceneName, bool allowTargetedHeapSweep = false)
    {
        int hidden = 0;
        ...
        try
        {
            Type? uiManagerType = FindType("Il2CppBAPBAP.UI.UIManager") ??
                                  FindType("BAPBAP.UI.UIManager");
            if (uiManagerType != null)
            {
        ...
        try
        {
            Type? splashType = FindType("Il2CppBAPBAP.UI.UILobbySplashScreen") ??
                               FindType("BAPBAP.UI.UILobbySplashScreen");
            if (splashType != null)
            {
                Array? splashes = FindLoadedUnityObjects(splashType);
        ...
        if (allowTargetedHeapSweep)
        {
            hidden += HideLoadingOverlayGameObjectSweep();
        }

        // NOTE: a full-heap FindLoadedUnityObjects(typeof(GameObject)) sweep used to run here once
        // per second. During match-scene load (entities instantiating + GC moving objects) that
        // walk hits dangling Il2Cpp pointers and hard-crashes the process natively — the exact AV
        // HasActiveMatchRuntime documents and avoids. The targeted UIManager + UILobbySplashScreen
        // paths above hide the loading overlay without touching the whole heap, so the brute-force
        // sweep (and its nested per-object Component scan) is removed.
        return hidden;
    }

    private int HideLoadingOverlayGameObjectSweep()
    {
        int hidden = 0;
        try
        {
            Array? objects = FindLoadedUnityObjects(typeof(GameObject));
            ...
            {
                if (item is not GameObject go || go == null)
                {
                    continue;
                }
        ...
        {
            if (_postMatchLoadingClearLogCount < 6)
            {
                _postMatchLoadingClearLogCount++;
                LoggerInstance.Warning($"[LobbyReturnFallback] Targeted loading overlay sweep failed: {ex.GetBaseException().Message}");
            }
        }
    ...
    private static bool ShouldHideMatchOverlayObject(GameObject? go)
    {
        ...
        if (!looksLikeOverlay)
        {
            looksLikeOverlay = ContainsLoadingText(go);
        }
        if (!looksLikeOverlay)
        {
            return false;
    ...
    private static bool ContainsLoadingText(GameObject go)
    {
        try
        {
            foreach (Component component in EnumerateComponentsForGameObject(go, includeChildren: true))
            {
                if (component == null)
                {
                    continue;
    ...
    private static IEnumerable<Component> EnumerateComponentsForGameObject(GameObject root, bool includeChildren)
    {
        ...
        Array? components = FindLoadedUnityObjects(typeof(Component));
        if (components == null)
        {
            yield break;
        }
        ...
        {
            if (item is not Component component || component == null)
            {
                continue;
            }
            ...
            bool matches = includeChildren
                ? IsSameOrChildTransform(transform, rootTransform)
            ...
            int id = GetUnityInstanceId(component);
            if (seen.Add(id))
            {
    ...
    private static int HideGameObjectOverlay(GameObject go)
    {
        ...
        try
        {
            foreach (Component component in EnumerateComponentsForGameObject(go, includeChildren: true))
            {
                if (component is not CanvasGroup group)
                {
                    continue;
                }
    ...
    private bool _lobbyUiProtected;
    private void TryProtectLobbyUiFromDestruction()
    {
        if (_lobbyUiProtected) return;
        _lobbyUiProtected = true;
        LoggerInstance.Msg("Skipped broad lobby UI DontDestroyOnLoad protection; stale protected canvases/managers break post-match MainScene reload.");
    }
    ...
    {
        if (path == "/add-teams" || path == "/queue-matched")
        {
            NormalizeDedicatedTeamPayload(path, il2CppPayload);
            return;
        }
    ...
    private void NormalizeDedicatedTeamPayload(string path, object il2CppPayload)
    {
        try
        {
            object? players = GetMemberValueSafe(il2CppPayload, "players");
            int minTeamId = int.MaxValue;
            int maxTeamId = 0;
            int playerCount = 0;
            var playerObjects = new List<object>();
            foreach (object player in EnumerateListLike(players))
            {
                playerObjects.Add(player);
                playerCount++;
                if (TryGetIntMember(player, "teamId", out int teamId))
                {
                    minTeamId = Math.Min(minTeamId, teamId);
                    maxTeamId = Math.Max(maxTeamId, teamId);
                }
            }

            bool shifted = false;
            if (playerObjects.Count > 0 && minTeamId > 0 && minTeamId != int.MaxValue)
            {
                foreach (object player in playerObjects)
                {
                    if (TryGetIntMember(player, "teamId", out int teamId))
                    {
                        int normalized = Math.Max(0, teamId - 1);
                        if (normalized != teamId && SetMemberValueBestEffort(player, "teamId", normalized))
                        {
                            shifted = true;
                        }
                    }
                ...
                maxTeamId = playerObjects
                    .Select(player => TryGetIntMember(player, "teamId", out int teamId) ? Math.Max(0, teamId) : 0)
                    .DefaultIfEmpty(0)
                    .Max();
            }
            ...
            if (path == "/add-teams")
            {
                int[] currentSpawns = ReadIntSequence(GetMemberValueSafe(il2CppPayload, "spawnLocationPerTeam")).ToArray();
                int requiredSlots = Math.Max(1, maxTeamId + 1);
                bool repairedSpawns = false;
                if (currentSpawns.Length < requiredSlots)
                {
                    currentSpawns = Enumerable.Range(0, requiredSlots).ToArray();
                    repairedSpawns = SetIntSequenceMember(il2CppPayload, "spawnLocationPerTeam", currentSpawns);
                }

                LoggerInstance.Msg(
                    "[PrematchTeams] " +
                    $"{path} players={playerCount} teamRange={DescribeTeamRange(playerObjects)} " +
                    $"spawnLocationPerTeam=[{string.Join(",", currentSpawns.Take(24))}] " +
                    $"shifted={shifted} repairedSpawns={repairedSpawns}.");
                return;
            }

            LoggerInstance.Msg(
                "[PrematchTeams] " +
                $"{path} players={playerCount} teamRange={DescribeTeamRange(playerObjects)} shifted={shifted}.");
        }
        catch (Exception ex)
        {
            LoggerInstance.Warning($"Prematch team normalization failed for {path}: {ex.GetBaseException().Message}");
        }
    }

    private static IEnumerable<int> ReadIntSequence(object? value)
    {
        if (value == null)
        {
            yield break;
    ...
    private static bool SetIntSequenceMember(object instance, string name, int[] values)
    {
        const BindingFlags flags = BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic;
        Type type = instance.GetType();
        ...
        else if (IsIl2CppStructArray(memberType) && memberType.GenericTypeArguments[0] == typeof(int))
        {
            converted = Activator.CreateInstance(memberType, new object[] { values });
        }
    ...
    private static string DescribeTeamRange(IEnumerable<object> players)
    {
        var teamIds = players
            .Select(player => TryGetIntMember(player, "teamId", out int teamId) ? teamId : (int?)null)
        ...
        return $"{teamIds.Min()}-{teamIds.Max()}";
    }
    ...
    private static int _lockerCrashLogCount;

    private static System.Exception? LockerCrashFinalizer(MethodBase __originalMethod, System.Exception __exception)
    {
        // Swallow so the locker tab never crashes the lobby UI, but surface WHAT threw: a locker
        // method usually throws because the server reported an owned AssetId the client content DB
        // has no entry for (the 300001/300004/300006 class of bug). Logging the method+message makes
        // an otherwise-invisible empty locker tab diagnosable. Rate-limited so it can't spam.
        if (__exception != null && _lockerCrashLogCount < 20)
        {
            _lockerCrashLogCount++;
            string method = __originalMethod?.Name ?? "?";
            string msg = __exception.GetBaseException().Message;
            if (msg != null && msg.Length > 160) msg = msg[..160];
            s_active?.LoggerInstance.Warning($"[LockerGuard] Swallowed {__exception.GetType().Name} in UILobbyLockerTabPage.{method}: {msg} (likely an owned AssetId missing from the client content DB)");
        }
        return null;
    }
    ...
    private void TryInstallMatchFoundDedupPatch()
    {
        ...
        Type? splashType = FindType("BAPBAP.UI.UILobbySplashScreen");
        Type? uiManagerType = FindType("BAPBAP.UI.UIManager");
        if (lobbyCtrlType == null && matchCtrlType == null && splashType == null && uiManagerType == null) return;
        ...
        MethodInfo? inactiveSplashPrefix = typeof(CustomServerMod).GetMethod(nameof(MatchFoundInactiveSplashPrefix), BindingFlags.Static | BindingFlags.NonPublic);
        if (prefix == null && inactiveSplashPrefix == null) return;
        ...
        foreach (Type? t in new[] { lobbyCtrlType, matchCtrlType })
        {
            ...
            try
            {
                ...
                if (mi != null && prefix != null && PatchHarmonyPrefix(mi, prefix))
                {
                    patched++;
                }
            }
        ...
        foreach (Type? t in new[] { splashType, uiManagerType })
        {
            if (t == null || inactiveSplashPrefix == null) continue;
            try
            {
                foreach (MethodInfo mi in t.GetMethods(BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic)
                             .Where(method => method.Name == "PlayMatchFoundSequence" && method.GetParameters().Length == 1))
                {
                    if (PatchHarmonyPrefix(mi, inactiveSplashPrefix))
                    {
                        patched++;
                    }
                }
    ...
    private static bool MatchFoundInactiveSplashPrefix(object __instance, object[] __args)
    {
        try
        {
            if (__instance == null || IsUnityObjectActiveInHierarchy(__instance))
            {
                return true;
            }

            object? callback = __args != null && __args.Length > 0 ? __args[0] : null;
            bool invoked = TryInvokeMatchFoundCallback(callback);
            s_active?.LoggerInstance.Warning($"[MatchFoundFallback] Skipped inactive {__instance.GetType().Name}.PlayMatchFoundSequence and invoked callback={invoked}.");
            return false;
        }
        catch (Exception ex)
        {
            s_active?.LoggerInstance.Warning($"[MatchFoundFallback] inactive-splash guard failed: {ex.GetBaseException().Message}");
            return true;
        }
    }

    private static bool IsUnityObjectActiveInHierarchy(object instance)
    {
        try
        {
        ...
        try
        {
            object? gameObject = GetMemberValueSafe(instance, "gameObject");
            object? active = GetMemberValueSafe(gameObject, "activeInHierarchy") ??
                             GetMemberValueSafe(gameObject, "activeSelf");
            return active is not bool isActive || isActive;
        }
        catch
        {
            return true;
    ...
    private static bool TryInvokeMatchFoundCallback(object? callback)
    {
        if (callback == null)
        {
            return false;
        ...
        try
        {
            ...
            return TryInvokeInstanceIfPresent(callback, "Invoke");
        }
        catch (Exception ex)
        {
            s_active?.LoggerInstance.Warning($"[MatchFoundFallback] callback invoke failed: {ex.GetBaseException().Message}");
            return false;
        }
    }
    ...
    private void TryRecoverStaleLobbyQueueUi()
    {
        ...
        // Recover quickly after a local leave-queue (the button stays "InQueue" client-side even though
        // the server cleared the queue). The idle probe below still confirms the server queue is empty
        // and HasLivePrematchContext() above guards a real GAME_STARTED handoff, so a short wait is safe
        // and can't reset a legitimately-queued player. Was 8f, which left the button stuck too long.
        ...
        try
        {
            if (_lobbyQueueIdleProbe.Status == TaskStatus.RanToCompletion)
            {
                serverIdle = _lobbyQueueIdleProbe.GetAwaiter().GetResult();
            }
        }
        catch { }
        _lobbyQueueIdleProbe = null;
        ...
        try
        {
            ...
            foreach (string typeName in new[] { "BAPBAP.Game.PreMatchManager", "Il2CppBAPBAP.Game.PreMatchManager" })
            {
                ...
                foreach (object? pm in all)
                {
                    if (pm == null) continue;
                    object? state = GetMemberValue(pm, "CurrentState") ?? GetMemberValue(pm, "NetworkCurrentState");
                    if (state != null && IsActivePrematchState(state.ToString() ?? ""))
                    {
                        return true;
                    }
                }
    ...
    // Extend to 10s so player has time to click an augment without letting a stale panel linger too long.
    ...
    private void TryExtendAugmentSelectTimer()
    {
        ...
        foreach (object inst in all)
        {
            ...
            try
            {
                // augmentSelectWaitDuration is the AUTO-CONFIRM timer (how long before the game picks
                // for you if you don't click). Vanilla ~5s is a bit tight during respawn; 30s was far
                // too long and means an unresolved panel lingers up to 30s. 10s is a sane middle.
                if (SetMemberValue(inst, "augmentSelectWaitDuration", 10f)) extended++;
                SetMemberValue(inst, "augmentRerollWaitDuration", 10f);
            }
            catch { }
        }
        ...
        if (extended > 0)
        {
            ...
            LoggerInstance.Msg($"[AugmentFix] Extended augmentSelectWaitDuration on {extended}/{all.Length} UIAugments instance(s) to 10s.");
        }
    }
    ...
    // ROOT-CAUSE PROBE + SAFE REPAIR for the dead in-match HUD cluster (ESC/pause, minimap,
    // cooldowns, death buttons). The 20-agent investigation found ESC (UISettingsMenu) and minimap
    // (UIMinimap) are [NonSerialized] sub-views on UIManager, wired/activated at runtime: ESC opens
    // only when UISettingsMenu._toggleKeyEnabled==true (set via SetToggleWindowKeyEnabled(true)).
    // Live logs were CLEAN (no swallowed crash), so the views are likely set-but-not-activated.
    // This logs the exact state once in-match, AND re-enables the ESC toggle key if it's off (a
    // targeted, reversible repair — not a blind guess). Bounded logging; runs ~1Hz.
    private void ProbeAndRepairInMatchHud()
    {
        // Only probe once the match is actually live (PreMatchState InGame=4), to avoid touching
        // the legit prematch UI.
        object? primary = FindPrimaryGameManager();
        if (primary == null) return;
        try
        {
            object? pmm = GetMemberValue(primary, "preMatchManager");
            object? st = pmm == null ? null : (GetMemberValue(pmm, "CurrentState") ?? GetMemberValue(pmm, "currentState"));
            if (st != null && Convert.ToInt32(st) < 4) return; // not in-game yet
        }
        catch { return; }
        ...
        {
            if (mgr == null) continue;
            object? uiSettings = GetMemberValueSafe(mgr, "uiSettings");
            object? uiMinimap = GetMemberValueSafe(mgr, "uiMinimap");
            object? uiGameMode = GetMemberValueSafe(mgr, "uiGameMode");
            object? uiAbilities = GetMemberValueSafe(mgr, "uiAbilities");

            string toggleState = "n/a";
            if (uiSettings != null)
            {
                object? tk = GetMemberValueSafe(uiSettings, "_toggleKeyEnabled");
                toggleState = tk?.ToString() ?? "null";

                // SAFE REPAIR: if the pause-menu toggle key is disabled, re-enable it so ESC works.
                if (tk is bool enabled && !enabled && !_escToggleRepaired)
                {
                    if (TryInvokeInstanceIfPresent(uiSettings, "SetToggleWindowKeyEnabled", true))
                    {
                        _escToggleRepaired = true;
                        LoggerInstance.Msg("[HudRepair] UISettingsMenu.SetToggleWindowKeyEnabled(true) invoked — ESC pause re-enabled.");
                    }
                }
            }

            if (_hudProbeLogCount < 20)
            {
                _hudProbeLogCount++;
                LoggerInstance.Msg($"[HudProbe] uiSettings={(uiSettings != null)} (toggleKeyEnabled={toggleState}) " +
                    $"uiMinimap={(uiMinimap != null)} uiGameMode={(uiGameMode != null)} uiAbilities={(uiAbilities != null)}.");
                if (uiGameMode != null)
                {
                    object? go = GetMemberValue(uiGameMode, "gameObject");
                    bool active = go is GameObject g && g.activeInHierarchy;
                    LoggerInstance.Msg($"[HudProbe] uiGameMode.activeInHierarchy={active}.");
                }
            }
        }
    }

    // DIAGNOSTIC (bounded): when an augment panel is up, capture its real state once it transitions
    // to visible. The user reports "pick -> vanishes -> reappears -> can't select anything", and ESC /
    // map become unusable -- the signature of a CanvasGroup left up with blocksRaycasts=true eating all
    // input. This logs the exact panel/selection/raycast state so the fix isn't another blind guess.
    private bool _augmentDiagWasVisible;
    private void LogAugmentPanelDiagnostic()
    {
        object? playerAugments = FindLocalPlayerAugmentsForClient(out string source);
        object? uiAugments = playerAugments != null ? GetMemberValueSafe(playerAugments, "uiAugments") : null;
        if (uiAugments == null)
        {
            Type? uiType = FindType("BAPBAP.UI.UIAugments");
            object[] all = uiType == null ? Array.Empty<object>() : FindLoadedUnityComponents(uiType).ToArray();
            uiAugments = all.Length > 0 ? all[0] : null;
        }
        if (uiAugments == null) { _augmentDiagWasVisible = false; return; }

        bool visible = false;
        try { visible = IsAugmentSelectionVisible(uiAugments); } catch { }

        // Only log on a rising edge or while visible (bounded), to avoid per-frame spam.
        if (!visible) { _augmentDiagWasVisible = false; return; }
        if (_augmentDiagLogCount >= 60) return;
        _augmentDiagLogCount++;

        object? showing = GetMemberValueSafe(uiAugments, "_showingAugments");
        object? animating = GetMemberValueSafe(uiAugments, "animatingSelect");
        object? enabled = GetMemberValueSafe(uiAugments, "augmentsEnabled");
        object? group = GetMemberValueSafe(uiAugments, "augmentsCanvasGroup");
        string raycast = "?", alpha = "?", interact = "?";
        if (group is CanvasGroup cg)
        {
            try { raycast = cg.blocksRaycasts.ToString(); } catch { }
            try { alpha = cg.alpha.ToString("0.00"); } catch { }
            try { interact = cg.interactable.ToString(); } catch { }
        }
        object? selection = playerAugments == null ? null
            : (GetMemberValueSafe(playerAugments, "CurrentSelection") ?? GetMemberValueSafe(playerAugments, "currentSelection"));
        object? selCount = playerAugments == null ? null : GetMemberValueSafe(playerAugments, "SelectionsCount");

        LoggerInstance.Msg($"[AugmentDiag] visible=true source={source} showing={showing} animating={animating} enabled={enabled} " +
            $"canvas(alpha={alpha} blocksRaycasts={raycast} interactable={interact}) currentSelection={(selection != null)} selectionsCount={selCount}.");
        _augmentDiagWasVisible = true;
    }
    ...
    private void TryConfigureDedicatedGameManagers()
    {
        ...
        // PERF: once a GameManager was successfully configured, drop from the 1s cadence to a 10s
        // safety re-sweep (late-spawning managers still get caught). The full sweep does two
        // object scans plus reflection per instance - at 1Hz forever that was a periodic hitch on
        // the HOST, which every connected player perceives as network stutter.
        float nowTime = Time.realtimeSinceStartup;
        if (_dedicatedGameManagerConfigured && nowTime < _nextDedicatedGameManagerSweepAt)
        {
            return;
        }

        _nextDedicatedGameManagerSweepAt = nowTime + (_dedicatedGameManagerConfigured ? 10f : 0f);
    ...
    private static object? GetMemberValue(object instance, string name)
    {
        ...
        // Cached per (type, name): the periodic scans call this for the same members thousands of
        // times per session; uncached GetProperty/GetField lookups were a steady CPU/GC drain.
        MemberInfo? member = s_instanceMemberCache.GetOrAdd((type, name), static key =>
        {
            PropertyInfo? property = key.Type.GetProperty(key.Name, flags);
            if (property != null && property.GetIndexParameters().Length == 0 && property.CanRead)
            {
                return property;
            }

            return (MemberInfo?)key.Type.GetField(key.Name, flags);
        });

        return member switch
        {
            PropertyInfo cachedProperty => cachedProperty.GetValue(instance),
            FieldInfo cachedField => cachedField.GetValue(instance),
            _ => null
        };
    }
    ...
    // Cached per (type, name, argCount): TryInvokeInstanceIfPresent is called in loops over UI
    // objects during post-match cleanup; the uncached GetMethods()+LINQ walk per call was a
    // visible part of the exit-to-lobby hitch.
    private static MethodInfo? ResolveInstanceMethod(Type type, string methodName, int argCount)
    {
        return s_instanceMethodCache.GetOrAdd((type, methodName, argCount), static key =>
        {
            const BindingFlags flags = BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic;
            foreach (MethodInfo candidate in key.Type.GetMethods(flags))
            {
                if (candidate.Name == key.Name && candidate.GetParameters().Length == key.ArgCount)
                {
                    return candidate;
                }
            }
    ...
    {
        MethodInfo? method = ResolveInstanceMethod(instance.GetType(), methodName, args.Length);
    ...
    private static bool CharacterPageUpdateDataGuardPrefix(object __instance, object? data)
    {
        ...
        HydrateCharacterPageModel(data, "UILobbyCharacterSelectPage.UpdateData(data)");
        SetMemberValueBestEffort(__instance, "_data", data);
        HydrateCharacterPageInstance(__instance, "UILobbyCharacterSelectPage.UpdateData(instance)");
        return true;
    }

    private static void CharacterPageInstanceHydratePrefix(object __instance)
    {
        HydrateCharacterPageInstance(__instance, "UILobbyCharacterSelectPage.method");
    }

    private static void MatchCharacterSelectHydratePrefix(object __instance)
    {
        HydrateCharacterPageInstance(__instance, "UILobbyMatchCharacterSelectPage.method");
    }

    private static void UpdateMatchmakingLockedStatusPrefix(ref bool isLocked)
    {
        isLocked = false;
    }

    private static void HydrateCharacterPageInstance(object? instance, string source)
    {
        if (instance == null) return;
        object? data = GetMemberValueSafe(instance, "_data") ?? GetMemberValueSafe(instance, "data");
        if (data != null)
        {
            HydrateCharacterPageModel(data, source + ".data");
        }
    }

    private static void HydrateCharacterPageModel(object? data, string source)
    {
        try
        {
            if (data == null) return;
            ...
            bool changed = false;
            changed |= PopulateAvailableCharactersOnModel(data, ids);
            changed |= PopulateCharIdsInRotationOnModel(data, ids);
            // RE-ENABLED: Populate unlockedCharacters HashSet and mark all listings as purchased.
            // Without this the native CharacterIsUnlocked() reads an empty HashSet and shows locked.
            // Since UnlockAllCharacters=true on the server, ALL chars should be unlocked.
            changed |= PopulateUnlockedCharactersOnPageModel(data, ids);
            int purchased = ForceListingsPurchased(GetMemberValueSafe(data, "charListings"), ids);
            if (changed && s_active != null)
            {
                s_active._characterModelUnlockStatePopulated = true;
                // Mark hydration "done" only when BOTH authoritative char IDs AND the listings are
                // actually present — not merely because IDs arrived (which left the
                // listingsPurchased=0 placeholder state stuck) or because a wall-clock timer fired.
                // Keep the FastPatchUntilTime backstop so the OnUpdate retry loop can't spin forever
                // if listings never bind.
                bool dataReady = s_active._authoritativeCharacterIdsReceived && CharacterPageModelHasListings(data);
                s_active._charConfigPreloadDone = dataReady || Time.realtimeSinceStartup >= FastPatchUntilTime;
                LogCharacterPageDataGuard($"hydrated {source}: chars=[{string.Join(",", ids)}] listingsPurchased={purchased} done={s_active._charConfigPreloadDone}.");
            }
        }
        catch (Exception ex)
        {
            if (_characterPageDataGuardLogCount < 8)
            {
                ...
                s_active?.LoggerInstance.Warning($"[CharacterPageDataGuard] hydrate {source} failed: {ex.GetBaseException().Message}");
            }
        }
    }
    ...
    {
        // BLANKET UNLOCK: Force CharacterIsUnlocked to return true for ALL characters.
        // IL2CPP native CharacterIsUnlocked only checks CharacterPageModel.unlockedCharacters
        // (a HashSet<int> that our server never populates), so without this prefix ALL chars
        // show as locked regardless of server config or listing data.
        __result = true;
        return false;
    }
    ...
    private static int _rpcFinalizerWarnLogCount;

    private static Exception? CustomServerNullRefFinalizer(System.Reflection.MethodBase __originalMethod, Exception? __exception)
    {
        if (__exception == null)
        {
            return null;
        ...
        // Capture ALL exceptions - not just NullRef. The lobby UI crashes on empty
        // UICharactersConfiguration data with various exception types (Argument,
        // IndexOutOfRange, NullRef). Suppressing lets the lobby continue building.
        //
        // PERF: this finalizer sits on Mirror's central RPC dispatch. A repeatedly-throwing RPC
        // handler (e.g. an update RPC several times per second) used to write two unbounded log
        // lines AND kick a full character-model scan per throw - log I/O plus scan cascade at
        // network rate. Warnings are now capped and the scan is left to the 2s OnUpdate ticker.
        if (_rpcFinalizerWarnLogCount < 60)
        {
            _rpcFinalizerWarnLogCount++;
            string where = "?";
            try { where = __originalMethod != null ? $"{__originalMethod.DeclaringType?.FullName}.{__originalMethod.Name}" : "?"; } catch { }
            string msg = __exception.GetBaseException().Message ?? "";
            if (msg.Length > 120) msg = msg[..120];
            s_active?.LoggerInstance.Warning($"Suppressed UI exception in {where}: {__exception.GetType().Name} {msg}");

            // FULL DIAGNOSTIC (bounded): which RPC/UI handler actually threw, with stack. The broad
            // HandleRemoteCall guard swallows ALL RPC throws -> if an in-match HUD/death/return-to-lobby
            // handler throws, the connection survives but the rest of its init chain never runs (dead HUD).
            // This stack identifies the real culprit so the swallow can be narrowed to only the known-bad one.
            if (_rpcFinalizerStackLogCount < 30)
            {
                _rpcFinalizerStackLogCount++;
                try { s_active?.LoggerInstance.Warning($"[RpcGuardDIAG] full stack in {where}:\n{__exception}"); } catch { }
            }
        }
    ...
    private static void LobbyReturnMessagePostfix(System.Reflection.MethodBase __originalMethod)
    {
        try
        {
            string methodName = __originalMethod?.Name ?? "";
            if (IsLobbyReturnCleanupMessage(methodName))
            {
                s_active?.ArmPostMatchPrematchCleanup($"lobby controller {methodName}");
            }
        }
        catch
        {
    ...
    private static void LoginControllerHandleLoadResponsePostfix(object __instance, object response)
    {
        ...
        try
        {
            int[] charIds = NormalizeCharacterIds(ExtractAvailableCharacters(response));
            if (charIds.Length > 0)
            {
                active.ApplyAuthoritativeCharacterIds(charIds, "LoginController.HandleLoadResponse");
            }
            else
            {
                active.PopulateAllUICharactersConfigurations(GetClientCharacterIds());
                try { active.TryPopulateCharacterModelsDirect(); } catch { }
            }
        }
        catch (Exception ex)
        {
    ...
    private static int[] ExtractAvailableCharacters(object response)
    {
        try
        {
            // Use the native GETTER method (get_availableCharacters) to avoid
            // IL2CPP StructArray field casting issues.
            Type respType = response.GetType();
            MethodInfo? getter = respType.GetMethods(BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic)
                .FirstOrDefault(m => m.Name.Contains("get_availableCharacters") || m.Name.Contains("availableCharacters"));
            object? val;
            if (getter != null)
            {
                val = getter.Invoke(response, null);
            }
            else
            {
                FieldInfo? f = respType.GetField("availableCharacters",
                    BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
                if (f == null) return Array.Empty<int>();
                val = f.GetValue(response);
            }
            if (val == null) return Array.Empty<int>();

            // Handle Il2CppStructArray<int> (subclass of System.Array) and plain int[]
            if (val is Array il2cppArr)
            {
                int[] result = new int[il2cppArr.Length];
                for (int i = 0; i < il2cppArr.Length; i++)
                    result[i] = Convert.ToInt32(il2cppArr.GetValue(i));
                return result;
            }
        ...
        {
            s_active?.LoggerInstance?.Warning($"ExtractAvailableCharacters failed: {ex.GetBaseException().Message}");
            return Array.Empty<int>();
        }
    }

    private static HashSet<int> ExtractOwnedCharacterIds(object response)
    {
        ...
        try
        {
            ...
            // Find the assets field/property
            MethodInfo? assetsGetter = respType.GetMethods(BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic)
                .FirstOrDefault(m => m.Name.Contains("get_assets") || m.Name.Contains("get_Assets"));
            FieldInfo? assetsField = assetsGetter == null
                ? respType.GetField("assets", BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic)
                : null;

            object? assetsVal = assetsGetter?.Invoke(response, null) ?? assetsField?.GetValue(response);
            if (assetsVal == null) return owned;

            // Handle Il2CppReferenceArray<Asset> or Array or IEnumerable
            if (assetsVal is System.Collections.IEnumerable enumerable)
            {
                foreach (object? item in enumerable)
                {
                    ...
                    // Try get_assetId / get_AssetId property first
                    MethodInfo? idGetter = itemType.GetMethod("get_assetId", Type.EmptyTypes) ??
                                           itemType.GetMethod("get_AssetId", Type.EmptyTypes);
                    if (idGetter != null)
                        assetId = Convert.ToInt32(idGetter.Invoke(item, null));
                    else
                    {
                        FieldInfo? idField = itemType.GetField("assetId", BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
                        if (idField != null)
                            assetId = Convert.ToInt32(idField.GetValue(item));
                    }

                    // Try get_balance / get_Balance property
                    MethodInfo? balGetter = itemType.GetMethod("get_balance", Type.EmptyTypes) ??
                                            itemType.GetMethod("get_Balance", Type.EmptyTypes);
                    if (balGetter != null)
                        balance = Convert.ToInt32(balGetter.Invoke(item, null));
                    else
                    {
                        FieldInfo? balField = itemType.GetField("balance", BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
                        if (balField != null)
                            balance = Convert.ToInt32(balField.GetValue(item));
                    }

                    // OWNERSHIP DETECTION via unlock assets ONLY.
                    // The raw charId assets (0-15) overlap with currencies
                    // (assetId 0 = Gold, 2 = CharTokens, etc.) so they are NOT reliable.
                    // The unlock assets (100000 + charId) are unambiguous.
                    if (assetId >= 100000 && assetId <= 100015 && balance > 0)
                        owned.Add(assetId - 100000);
                }
            }
        ...
        {
            s_active?.LoggerInstance?.Warning($"ExtractOwnedCharacterIds failed: {ex.GetBaseException().Message}");
            return owned;
        }
    }
    ...
    private void PopulateAllUICharactersConfigurations(int[] charIds)
    {
        ...
        // Try multiple strategies to find existing UICharactersConfiguration instances
        Array? configs = FindLoadedUnityObjects(cfgType);
        if (configs == null || configs.Length == 0)
        {
            try
            {
                MethodInfo? findM = typeof(UnityEngine.Resources).GetMethod("FindObjectsOfTypeAll", new[] { typeof(Type) });
                if (findM != null) configs = findM.Invoke(null, new object[] { cfgType }) as Array;
            }
            catch { }
        }
        if (configs == null || configs.Length == 0)
        {
            try
            {
                MethodInfo? findM = typeof(UnityEngine.Object).GetMethod("FindObjectsOfTypeAll",
                    BindingFlags.Static | BindingFlags.Public, null, new[] { typeof(Type) }, null);
                if (findM != null) configs = findM.Invoke(null, new object[] { cfgType }) as Array;
            }
            catch { }
        }

        // Strategy D: Use Il2CppSystem.Type via Resources.FindObjectsOfTypeAll(Il2CppSystem.Type).
        // This finds ScriptableObjects NOT in Resources but referenced by scene MonoBehaviours.
        // CRITICAL: Use GetMethods + IsAssignableFrom, NOT GetMethod with exact parameter type,
        // because sysType.GetType() returns the CONCRETE Il2Cpp runtime type (e.g. Il2CppSystem.MonoType)
        // which never matches the declared parameter type Il2CppSystem.Type exactly.
        if (configs == null || configs.Length == 0)
        {
            try
            {
                object? sysType = GetIl2CppType(cfgType);
                if (sysType != null)
                {
                    MethodInfo? findAll = typeof(Resources).GetMethods(BindingFlags.Static | BindingFlags.Public)
                        .FirstOrDefault(m => m.Name == "FindObjectsOfTypeAll" &&
                    ...
                    if (findAll != null)
                    {
                        ...
                        {
                            configs = result;
                            MelonLogger.Msg($"[CharConfig] Strategy D (Il2CppType assignable) found {result.Length} config(s). method={findAll.Name}({findAll.GetParameters()[0].ParameterType.Name})");
                        }
                    }
                    else
                    {
        ...
        if (configs == null || configs.Length == 0)
        {
            // Suppress repeated "no instances" log - the DirectModelPopulate already reports this
            return;
        }
        ...
        foreach (object cfg in configs)
        {
            ...
            {
                Type instType = cfg.GetType();
                // Use native setter via MethodInfo.Invoke (bypasses IL2CPP StructArray casting issues)
                MethodInfo? setMethod = instType.GetMethods(BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic)
                    .FirstOrDefault(m => m.Name.Contains("set_") &&
                                         m.Name.Contains("lobbyAvailableCharacterIds") &&
                                         m.GetParameters().Length == 1);
                ...
                if (setMethod != null)
                {
                    ...
                    {
                        ConstructorInfo? ctorArr = paramType.GetConstructor(new[] { typeof(int[]) });
                        if (ctorArr != null) toAssign = ctorArr.Invoke(new object[] { charIds });
                        else
                        {
                            ConstructorInfo? ctorSize = paramType.GetConstructor(new[] { typeof(long) });
                            if (ctorSize != null)
                            {
                                object wrapper = ctorSize.Invoke(new object[] { (long)charIds.Length });
                                PropertyInfo? indexer = paramType.GetProperty("Item");
                                if (indexer != null)
                                {
                                ...
                                else toAssign = wrapper;
                            }
                            else toAssign = CreateIl2CppArray(paramType, charIds);
                        }
                    }

                    if (toAssign != null)
                    {
                        setMethod.Invoke(cfg, new object[] { toAssign });
                        populated++;
                    }
                }
                else
                {
                    // Fallback to FieldInfo.SetValue
                    object? toAssign = CreateIl2CppArray(fieldType, charIds) ?? (object)charIds;
        ...
        if (populated != _lastPopulateAllReportedCount)
        {
            _lastPopulateAllReportedCount = populated;
    ...
    /// <summary>
    /// Directly populates CharacterSelectModel and CharacterPageModel instances
    /// found in the scene. These ARE MonoBehaviours that Unity's FindObjectsOfType
    /// can find, unlike UICharactersConfiguration (ScriptableObject).
    /// </summary>
    private void TryPopulateCharacterModelsDirect()
    {
        if (_charConfigPreloadDone && _characterModelUnlockStatePopulated) return;

        // The lobby character-select models only exist in the lobby scene. Without this gate the
        // 2s populate ticker kept scanning right through live matches whenever the preload never
        // completed (per-match stutter for zero benefit).
        string activeScene = GetActiveSceneNameSafe();
        bool lobbyLikeScene = string.IsNullOrEmpty(activeScene) ||
                              activeScene.Equals("MainScene", StringComparison.OrdinalIgnoreCase) ||
                              activeScene.Equals("Login", StringComparison.OrdinalIgnoreCase) ||
                              activeScene.Equals("Bootstrap", StringComparison.OrdinalIgnoreCase);
        if (!lobbyLikeScene) return;
        ...
        try
        {
            ...
            int unlockUpdates = 0;
            int listingUpdates = 0;
            int modelsSeen = 0;

            // Strategy 1: Find and populate CharacterSelectModel instances
            Type? csModelType = FindType("BAPBAP.UI.CharacterSelectModel");
            if (csModelType != null)
            {
                Array? models = FindLoadedUnityObjects(csModelType);
                if (models != null)
                {
                    foreach (object model in models)
                    {
                        ...
                        modelsSeen++;
                        if (PopulateAvailableCharactersOnModel(model, allCharIds)) availableUpdates++;
                    }
                }
            }

            // Strategy 2: Find and populate CharacterPageModel instances
            Type? cpModelType = FindType("BAPBAP.UI.CharacterPageModel");
            if (cpModelType != null)
            {
                Array? pages = FindLoadedUnityObjects(cpModelType);
                if (pages != null)
                {
                    foreach (object page in pages)
                    {
                        if (page == null) continue;
                        modelsSeen++;
                        if (PopulateAvailableCharactersOnModel(page, allCharIds)) availableUpdates++;
                        if (PopulateCharIdsInRotationOnModel(page, allCharIds)) availableUpdates++;
                        // RE-ENABLED: The unlockedCharacters HashSet on CharacterPageModel must be
                        // populated for the UI to show chars as unlocked. The Harmony
                        // CharacterIsUnlockedPrefix patch alone is not sufficient because some
                        // code paths check the HashSet directly.
                        if (PopulateUnlockedCharactersOnPageModel(page, allCharIds)) unlockUpdates++;
                        listingUpdates += ForceListingsPurchased(GetMemberValueSafe(page, "charListings"), allCharIds);
                    }
                }
            }

            // Strategy 3: Find UILobbyCharacterSelectPage instances and set _data fields
            Type? charSelectPageType = FindType("BAPBAP.UI.UILobbyCharacterSelectPage");
            if (charSelectPageType != null)
            {
                Array? pages = FindLoadedUnityObjects(charSelectPageType);
                if (pages != null)
                {
                    foreach (object page in pages)
                    {
                        if (page == null) continue;
                        object? data = GetMemberValueSafe(page, "_data") ?? GetMemberValueSafe(page, "data");
                        if (data == null) continue;
                        modelsSeen++;
                        if (PopulateAvailableCharactersOnModel(data, allCharIds)) availableUpdates++;
                        if (PopulateCharIdsInRotationOnModel(data, allCharIds)) availableUpdates++;
                        // RE-ENABLED: Same as above — force chars unlocked via HashSet and listings.
                        if (PopulateUnlockedCharactersOnPageModel(data, allCharIds)) unlockUpdates++;
                        listingUpdates += ForceListingsPurchased(GetMemberValueSafe(data, "charListings"), allCharIds);
                    }
                }
            }

            // Strategy 4: When FindLoadedUnityObjects fails for the specific types,
            // search ALL GameObjects and use GetComponent reflection to find matching components.
            // This works when IL2CPP type resolution for FindObjectsOfType fails but the objects
            // exist in the scene hierarchy.
            // SAFETY/PERF: this walks the ENTIRE GameObject heap with a per-object GetComponents
            // call — a multi-millisecond freeze per tick and the exact heap walk the
            // HasActiveMatchRuntime comment documents as native-AV-prone. Run it at most ONCE per
            // session; if it finds nothing the typed strategies keep retrying on the 2s ticker.
            if (modelsSeen == 0 && !_charConfigHeapScanUsed)
            {
                _charConfigHeapScanUsed = true;
                try
                {
                    Array? allGameObjects = FindLoadedUnityObjects(typeof(GameObject));
                    if (allGameObjects != null && allGameObjects.Length > 0)
                    {
                        string[] targetTypeNames = { "CharacterSelectModel", "CharacterPageModel", "UILobbyCharacterSelectPage", "UILobbyPlayTabPage" };
                        foreach (object goObj in allGameObjects)
                        {
                            if (goObj == null) continue;
                            GameObject go;
                            try { go = (GameObject)goObj; }
                            catch { continue; }
                            if (go == null) continue;
                            ...
                            foreach (Component comp in components)
                            {
                                ...
                                string typeName;
                                try { typeName = comp.GetType().Name; }
                                catch { continue; }

                                bool isTarget = false;
                                foreach (string target in targetTypeNames)
                                {
                                    if (typeName.Contains(target, StringComparison.OrdinalIgnoreCase))
                                    {
                                        isTarget = true;
                                        break;
                                    }
                                ...
                                object model = comp;
                                if (PopulateAvailableCharactersOnModel(model, allCharIds)) availableUpdates++;
                                if (PopulateCharIdsInRotationOnModel(model, allCharIds)) availableUpdates++;
                                if (PopulateUnlockedCharactersOnPageModel(model, allCharIds)) unlockUpdates++;
                                listingUpdates += ForceListingsPurchased(GetMemberValueSafe(model, "charListings"), allCharIds);

                                // Also check _data field for nested models
                                object? data = GetMemberValueSafe(model, "_data") ?? GetMemberValueSafe(model, "data");
                                if (data != null)
                                {
                                    if (PopulateAvailableCharactersOnModel(data, allCharIds)) availableUpdates++;
                                    if (PopulateCharIdsInRotationOnModel(data, allCharIds)) availableUpdates++;
                        ...
                        if (modelsSeen > 0)
                        {
                            LoggerInstance.Msg($"[DirectModelPopulate] Strategy 4 (GameObject scan): found {modelsSeen} model(s) via component iteration.");
                        }
                    }
                }
                catch (Exception ex4)
                {
                    LoggerInstance.Warning($"[DirectModelPopulate] Strategy 4 error: {ex4.GetBaseException().Message}");
                }
            }

            if (modelsSeen > 0)
            {
                if (unlockUpdates > 0 || listingUpdates > 0)
                {
                    _characterModelUnlockStatePopulated = true;
                }

                _charConfigPreloadDone = _characterModelUnlockStatePopulated &&
                    (_authoritativeCharacterIdsReceived || Time.realtimeSinceStartup >= FastPatchUntilTime);
                // Only log when the outcome actually changed - this fires on a 2s ticker and used
                // to write an identical multi-value line (with a string.Join over all char ids)
                // every tick until the preload finished.
                string summary = $"[DirectModelPopulate] models={modelsSeen} available={availableUpdates} unlockedSets={unlockUpdates} listingsPurchased={listingUpdates} chars=[{string.Join(",", allCharIds)}] authoritative={_authoritativeCharacterIdsReceived} done={_charConfigPreloadDone}.";
                if (!string.Equals(summary, _lastDirectPopulateSummary, StringComparison.Ordinal))
                {
                    _lastDirectPopulateSummary = summary;
                    LoggerInstance.Msg(summary);
                }
            }
            else if (_charConfigRetryCount < 120)
            {
                _charConfigRetryCount++;
                // Keep _charConfigPreloadDone = false so OnUpdate retries
                if (_charConfigRetryCount % 10 == 1)
                    LoggerInstance.Msg($"[DirectModelPopulate] No models found yet (attempt {_charConfigRetryCount}/120). Will retry.");
            }
        }
        catch (Exception ex)
        {
            LoggerInstance.Warning($"[DirectModelPopulate] Failed: {ex.GetBaseException().Message}");
        }
    }

    private static bool PopulateAvailableCharactersOnModel(object model, int[] charIds)
    {
        return SetIntArrayMember(model, "availableCharacters", charIds);
    }

    private static bool PopulateCharIdsInRotationOnModel(object model, int[] charIds)
    {
        return SetIntArrayMember(model, "charIdsInRotation", charIds);
    }

    private static bool SetIntArrayMember(object target, string memberName, int[] values)
    {
        if (target == null || values.Length == 0) return false;
        Type type = target.GetType();
        MethodInfo? setter = type.GetMethods(BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic)
            .FirstOrDefault(m => m.Name.Contains("set_", StringComparison.Ordinal) &&
                                 m.Name.Contains(memberName, StringComparison.OrdinalIgnoreCase) &&
                                 m.GetParameters().Length == 1);
        if (setter != null)
        {
            object? arr = CreateIl2CppArray(setter.GetParameters()[0].ParameterType, values) ?? values;
            setter.Invoke(target, new[] { arr });
            return true;
        }

        FieldInfo? field = type.GetField(memberName, BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
        if (field == null) return false;
        object? value = CreateIl2CppArray(field.FieldType, values) ?? values;
        field.SetValue(target, value);
        return true;
    }

    private static bool PopulateUnlockedCharactersOnPageModel(object pageModel, int[] charIds)
    {
        if (pageModel == null || charIds.Length == 0) return false;
        FieldInfo? field = pageModel.GetType().GetField("unlockedCharacters", BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
        if (field == null) return false;

        object? set = field.GetValue(pageModel);
        if (set == null)
        {
            set = CreateUnlockedCharacterSet(field.FieldType);
            if (set == null) return false;
            field.SetValue(pageModel, set);
        }

        MethodInfo? clear = set.GetType().GetMethod("Clear", Type.EmptyTypes);
        clear?.Invoke(set, null);

        MethodInfo? add = set.GetType().GetMethod("Add", new[] { typeof(int) }) ??
                          set.GetType().GetMethods(BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic)
                              .FirstOrDefault(m => m.Name == "Add" && m.GetParameters().Length == 1);
        if (add == null) return false;

        foreach (int charId in charIds)
        {
            add.Invoke(set, new object[] { charId });
        }

        return true;
    }

    private static object? CreateUnlockedCharacterSet(Type fieldType)
    {
        try
        {
            if (fieldType == typeof(HashSet<int>) || fieldType.IsAssignableFrom(typeof(HashSet<int>)))
            {
    ...
    private static int ForceListingsPurchased(object? charListingsContainer, int[] allowedCharIds)
    {
        if (charListingsContainer == null) return 0;
        var allowed = new HashSet<int>(allowedCharIds);
        object? entries = GetMemberValueSafe(charListingsContainer, "charListings") ?? charListingsContainer;
        int updated = 0;

        foreach (object? listing in EnumerateListLikePreserveNulls(entries))
        {
            if (listing == null) continue;
            bool hasCharId = TryGetCharacterIdFromListingEntry(listing, out int charId);
            if (hasCharId && !allowed.Contains(charId)) continue;

            if (SetIntMemberValue(listing, "purchases", 1))
            {
                updated++;
            }
        }
    ...
    private static bool SetIntMemberValue(object target, string memberName, int value)
    {
        Type type = target.GetType();
        FieldInfo? field = type.GetField(memberName, BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
        if (field != null)
        {
            try
            {
                field.SetValue(target, Convert.ChangeType(value, field.FieldType, System.Globalization.CultureInfo.InvariantCulture));
                return true;
            }
            catch { }
        }
    ...
    {
        s_active?.RewriteUnityWebRequestProxyUrl(__instance);
    ...
    private void LogUnityWebRequest(object request)
    {
        ...
        object? methodValue = GetMemberValue(request, "method") ?? GetMemberValue(request, "Method");
        string method = methodValue as string ?? "?";

        _unityWebRequestLogCount++;
        // Strip remote host details to avoid leaking server IP in logs; keep local port for proxy diagnostics.
        string sanitized = SanitizeUrlForLog(url);
        LoggerInstance.Msg($"UnityWebRequest #{_unityWebRequestLogCount}: {method} {sanitized}");
    }
    ...
    private static string SanitizeUrlForLog(string url)
    {
        ...
        try
        {
            ...
            if (uri.IsAbsoluteUri)
            {
                ...
                string hostTag = isLocal ? $"local:{uri.Port}" : "remote";
    ...
    private void LogCustomServerRequest(object request)
    {
        ...
        LoggerInstance.Msg($"Custom server HTTP request #{_customServerRequestLogCount}: {method} {SanitizeUrlForLog(url)}");
    ...
    {
        object? urlValue = GetMemberValue(request, "url");
        string? url = urlValue as string;

        if (!MelonProxyHelpers.ShouldRewriteCallbackUrl(
                url,
        ...
        string replacement = _lastAppliedApiHost.TrimEnd('/') + pathAndQuery;
    ...
    private void RewriteUnityWebRequestProxyUrl(object request)
    {
        try
        {
            ...
            int localProxyPort = _localPortEntry?.Value ?? DefaultLocalProxyPort;
            bool knownLocalApiPort =
                uri.Port == DefaultLocalProxyPort ||
                uri.Port == DefaultServerPort ||
                uri.Port == _serverPortEntry.Value ||
                uri.Port == localProxyPort;
            if (!knownLocalApiPort || uri.Port == localProxyPort)
            {
                return;
            }

            string replacement = GetConfiguredApiHost().TrimEnd('/') + uri.PathAndQuery;
            if (string.Equals(url, replacement, StringComparison.OrdinalIgnoreCase))
            {
                return;
            }
            ...
            if (!_unityWebRequestProxyUrlPatchLogged)
            {
                _unityWebRequestProxyUrlPatchLogged = true;
                LoggerInstance.Msg($"Rewrote UnityWebRequest API URL for local proxy: {SanitizeUrlForLog(url)} -> {SanitizeUrlForLog(replacement)}.");
            }
        }
        catch (Exception ex)
        {
            ...
            {
                _unityWebRequestProxyUrlPatchLogged = true;
                LoggerInstance.Warning($"Failed to rewrite UnityWebRequest API URL for local proxy: {ex.GetBaseException().Message}");
            }
        }
    }
    ...
    private static object? TryFindComponentByType(GameObject? gameObject, Type componentType, bool includeChildren)
    {
        ...
        try
        {
            foreach (Component? component in EnumerateComponentsForGameObject(gameObject, includeChildren))
            {
                if (component == null)
                {
                    continue;
    ...
    private static int _getIl2CppTypeDiagCount = 0;
    private static HashSet<string> _reportedNullTypes = new HashSet<string>();

    private static object? GetIl2CppType(Type objectType)
    {
        ...
        bool doDiag = _getIl2CppTypeDiagCount < 10;
        if (doDiag) _getIl2CppTypeDiagCount++;
        ...
        try
        {
            ...
            foreach (var assembly in assemblies)
            {
                ...
                if (asmName != null && asmName.StartsWith("Il2CppInterop.Runtime", StringComparison.OrdinalIgnoreCase))
                {
                    assemblyFound = true;
                    if (doDiag) MelonLogger.Msg($"[GetIl2CppType Diag] Found assembly: {asmName}");
                    Type? runtimeType = assembly.GetType("Il2CppInterop.Runtime.Il2CppType");
                    if (runtimeType != null)
                    {
                        if (doDiag) MelonLogger.Msg($"[GetIl2CppType Diag] Found type: Il2CppInterop.Runtime.Il2CppType");
                        MethodInfo? fromMethod = runtimeType.GetMethod("From", BindingFlags.Static | BindingFlags.Public, null, new[] { typeof(Type) }, null);
                        if (fromMethod != null)
                        {
                            if (doDiag) MelonLogger.Msg($"[GetIl2CppType Diag] Found method From(Type)");
                            var result = fromMethod.Invoke(null, new object[] { objectType });
                            if (result != null)
                            {
                                if (doDiag) MelonLogger.Msg($"[GetIl2CppType Diag] From(Type) returned type for {objectType.Name}");
                                return result;
                            }
                            else
                            {
                                if (doDiag) MelonLogger.Msg($"[GetIl2CppType Diag] From(Type) returned null for {objectType.Name}");
                            }
                        }
                        else
                        {
                            if (doDiag) MelonLogger.Msg($"[GetIl2CppType Diag] Method From(Type) NOT found");
                        }
                    }
                    else
                    {
                        if (doDiag) MelonLogger.Msg($"[GetIl2CppType Diag] Type Il2CppInterop.Runtime.Il2CppType NOT found in assembly");
                    }
                }
            }

            if (!assemblyFound && doDiag)
            {
                MelonLogger.Msg($"[GetIl2CppType Diag] Assembly 'Il2CppInterop.Runtime' NOT found. Available assemblies:");
                foreach (var assembly in assemblies)
                {
                    string? name = assembly.GetName().Name;
                    if (name != null && (name.Contains("Interop", StringComparison.OrdinalIgnoreCase) || name.Contains("Il2Cpp", StringComparison.OrdinalIgnoreCase)))
                    {
        ...
        {
            MelonLogger.Msg($"[GetIl2CppType] Error searching AppDomain for {objectType.Name}: {ex.Message}");
        }

        try
        {
            var il2cppTypeProp = objectType.GetProperty("Il2CppType",
                BindingFlags.Static | BindingFlags.Public | BindingFlags.FlattenHierarchy);
            if (il2cppTypeProp != null)
            {
                var result = il2cppTypeProp.GetValue(null);
    ...
    private static Array? ConvertEnumerableToArray(object? obj)
    {
        if (obj == null) return null;
        if (obj is Array arr) return arr;

        Type type = obj.GetType();

        // Il2Cpp array wrappers implement IEnumerable, which is far cheaper to walk than the old
        // per-element get_Item reflection Invoke (one boxed index array + reflection dispatch per
        // element). Prefer the enumerable path and keep the accessor path only as fallback.
        ...
        if (typeName.Contains("Il2CppInterop.Runtime.InteropTypes.Arrays.") || typeName.Contains("Il2CppArrayBase"))
        {
            try
            {
                (PropertyInfo? lengthProp, MethodInfo? getItemMethod) = s_il2cppArrayAccessorCache.GetOrAdd(
                    type,
                    static t => (t.GetProperty("Length"), t.GetMethod("get_Item", new[] { typeof(int) })));
                if (lengthProp != null && getItemMethod != null)
                {
                    int length = (int)lengthProp.GetValue(obj)!;
                    var list = new System.Collections.ArrayList(length);
                    object[] indexArgs = new object[1];
                    for (int i = 0; i < length; i++)
                    {
                        indexArgs[0] = i;
                        var item = getItemMethod.Invoke(obj, indexArgs);
                        if (item != null) list.Add(item);
                    }
                    return list.ToArray();
                }
            ...
            {
                MelonLogger.Msg($"[ConvertEnumerableToArray] Failed to convert IL2CPP array via reflection: {ex.GetBaseException().Message}");
            }
        }
    ...
    // Open generic/non-generic finder MethodInfos, resolved ONCE per process. Re-running the
    // GetMethods()+LINQ probes on every scan was a large share of each scan's cost.
    private static bool s_openFindersResolved;
    private static MethodInfo? s_openFindObjectsGenericBool;   // Object.FindObjectsOfType<T>(bool)
    private static MethodInfo? s_openFindObjectsGeneric;       // Object.FindObjectsOfType<T>()
    private static MethodInfo? s_openResourcesFindAllGeneric;  // Resources.FindObjectsOfTypeAll<T>()
    private static MethodInfo? s_findObjectsIl2CppType1;       // Object.FindObjectsOfType(Il2CppSystem.Type)
    private static MethodInfo? s_findObjectsIl2CppType2;       // Object.FindObjectsOfType(Il2CppSystem.Type, bool)
    private static MethodInfo? s_resourcesFindAllIl2CppType;   // Resources.FindObjectsOfTypeAll(Il2CppSystem.Type)
    private static MethodInfo? s_resourcesFindAllSystemType;   // Resources.FindObjectsOfTypeAll(System.Type)

    private static void EnsureOpenFindersResolved()
    {
        if (s_openFindersResolved)
        {
            return;
        }
        ...
        try
        {
            MethodInfo[] objectMethods = typeof(Object).GetMethods(BindingFlags.Static | BindingFlags.Public);
            s_openFindObjectsGenericBool = objectMethods.FirstOrDefault(m =>
                m.Name == "FindObjectsOfType" && m.IsGenericMethod &&
                m.GetParameters().Length == 1 && m.GetParameters()[0].ParameterType == typeof(bool));
            s_openFindObjectsGeneric = objectMethods.FirstOrDefault(m =>
                m.Name == "FindObjectsOfType" && m.IsGenericMethod && m.GetParameters().Length == 0);
            s_findObjectsIl2CppType1 = objectMethods.FirstOrDefault(m =>
                m.Name == "FindObjectsOfType" && !m.IsGenericMethod &&
                m.GetParameters().Length == 1 &&
                (m.GetParameters()[0].ParameterType.FullName == "Il2CppSystem.Type" || m.GetParameters()[0].ParameterType.Name == "Type"));
            s_findObjectsIl2CppType2 = objectMethods.FirstOrDefault(m =>
                m.Name == "FindObjectsOfType" && !m.IsGenericMethod &&
                m.GetParameters().Length == 2 &&
                (m.GetParameters()[0].ParameterType.FullName == "Il2CppSystem.Type" || m.GetParameters()[0].ParameterType.Name == "Type") &&
                m.GetParameters()[1].ParameterType == typeof(bool));

            MethodInfo[] resourceMethods = typeof(Resources).GetMethods(BindingFlags.Static | BindingFlags.Public);
            s_openResourcesFindAllGeneric = resourceMethods.FirstOrDefault(m =>
                m.Name == "FindObjectsOfTypeAll" && m.IsGenericMethod && m.GetParameters().Length == 0);
            s_resourcesFindAllIl2CppType = resourceMethods.FirstOrDefault(m =>
                m.Name == "FindObjectsOfTypeAll" && !m.IsGenericMethod &&
                m.GetParameters().Length == 1 &&
                (m.GetParameters()[0].ParameterType.FullName == "Il2CppSystem.Type" || m.GetParameters()[0].ParameterType.Name == "Type"));
            s_resourcesFindAllSystemType = resourceMethods.FirstOrDefault(m =>
                m.Name == "FindObjectsOfTypeAll" && !m.IsGenericMethod &&
                m.GetParameters().Length == 1 && m.GetParameters()[0].ParameterType == typeof(Type));
        }
        catch
        {
            // Leave whatever resolved; the finder cascade tolerates nulls.
        }

        s_openFindersResolved = true;
    }
    ...
    private static Array? FindLoadedUnityObjects(Type objectType)
    {
        ...
        // Fast path: replay the finder that worked for this type before. This turns the repeat
        // cost of every periodic scan into a single cached Invoke instead of a strategy cascade
        // with swallowed exceptions.
        if (s_unityObjectFinderCache.TryGetValue(objectType, out UnityObjectFinder? cachedFinder))
        {
            try
            {
                Array? replay = ConvertEnumerableToArray(cachedFinder.Method.Invoke(null, cachedFinder.Args));
                if (replay != null) return replay;
            }
            catch
            {
                // Cached strategy stopped working (domain reload etc.) - fall through and re-resolve.
            }

            s_unityObjectFinderCache.TryRemove(objectType, out _);
        }

        EnsureOpenFindersResolved();

        // Try generic FindObjects methods first.
        // T[] FindObjectsOfType<T>(bool includeInactive) / T[] FindObjectsOfTypeAll<T>()
        // This avoids invalid cast exceptions in the IL2CPP reflection layer, because the closed generic methods
        // return type (e.g. Canvas[]) is equivalent to the returned Il2CppReferenceArray<Canvas>.

        // 1. Generic Object.FindObjectsOfType<T>(bool includeInactive)
        if (s_openFindObjectsGenericBool != null)
        {
            try
            {
                MethodInfo closedMethod = s_openFindObjectsGenericBool.MakeGenericMethod(objectType);
                object?[] args = { true };
                object? res = closedMethod.Invoke(null, args);
                if (res != null)
                {
                    Array? converted = ConvertEnumerableToArray(res);
                    if (converted != null)
                    {
                        s_unityObjectFinderCache[objectType] = new UnityObjectFinder(closedMethod, args);
                        return converted;
                    }
                }
            }
        ...
        // 2. Generic Object.FindObjectsOfType<T>()
        if (s_openFindObjectsGeneric != null)
        {
            try
            {
                MethodInfo closedMethod = s_openFindObjectsGeneric.MakeGenericMethod(objectType);
                object? res = closedMethod.Invoke(null, null);
                if (res != null)
                {
                    Array? converted = ConvertEnumerableToArray(res);
                    if (converted != null)
                    {
                        s_unityObjectFinderCache[objectType] = new UnityObjectFinder(closedMethod, null);
                        return converted;
                    }
                }
            }
        ...
        // 3. Generic Resources.FindObjectsOfTypeAll<T>()
        if (s_openResourcesFindAllGeneric != null)
        {
            try
            {
                MethodInfo closedMethod = s_openResourcesFindAllGeneric.MakeGenericMethod(objectType);
                object? res = closedMethod.Invoke(null, null);
                if (res != null)
                {
                    Array? converted = ConvertEnumerableToArray(res);
        ...
        // Fallbacks: Non-generic approaches
        object? il2cppType = GetIl2CppType(objectType);
        if (il2cppType != null)
        {
            if (s_findObjectsIl2CppType1 != null)
            {
                try
                {
                    object?[] args = { il2cppType };
                    var res = ConvertEnumerableToArray(s_findObjectsIl2CppType1.Invoke(null, args));
                    if (res != null)
                    {
                        s_unityObjectFinderCache[objectType] = new UnityObjectFinder(s_findObjectsIl2CppType1, args);
                        return res;
                    }
                }
                catch { }
            ...
            if (s_findObjectsIl2CppType2 != null)
            {
                try
                {
                    object?[] args = { il2cppType, true };
                    var res = ConvertEnumerableToArray(s_findObjectsIl2CppType2.Invoke(null, args));
                    if (res != null)
                    {
                        s_unityObjectFinderCache[objectType] = new UnityObjectFinder(s_findObjectsIl2CppType2, args);
                        return res;
                    }
                }
                catch { }
            ...
            if (s_resourcesFindAllIl2CppType != null)
            {
                try
                {
                    object?[] args = { il2cppType };
                    var res = ConvertEnumerableToArray(s_resourcesFindAllIl2CppType.Invoke(null, args));
                    if (res != null)
                    {
                        s_unityObjectFinderCache[objectType] = new UnityObjectFinder(s_resourcesFindAllIl2CppType, args);
                        return res;
                    }
                }
                catch { }
        ...
        // Fallback: TryFindObjectsOfType with System.Type (old approach, safe to fail)
        ...
        if (s_resourcesFindAllSystemType != null)
        {
            try
            {
                return ConvertEnumerableToArray(s_resourcesFindAllSystemType.Invoke(null, new object[] { objectType }));
            }
            catch { }
        }
    ...
    private static Array? TryInvokeUnityObjectFinder(Type ownerType, string methodName, Type objectType, bool includeInactive)
    {
        try
        {
            ...
            return ConvertEnumerableToArray(method.Invoke(null, args));
        }
        catch
        {
            return null;
    ...
    // Negative FindType cache: (name -> assembly count at probe time). Many call sites probe both
    // the "BAPBAP.*" and "Il2CppBAPBAP.*" spelling on periodic scans; without a negative cache the
    // losing spelling re-walked every loaded assembly on every single call. The miss is re-probed
    // only when new assemblies have been loaded since the cached miss.
    private static readonly ConcurrentDictionary<string, int> s_typeMissCache = new();
    ...
    private static Type? FindType(string fullName)
    {
        ...
        if (s_typeMissCache.TryGetValue(fullName, out int missedAtAssemblyCount) &&
            missedAtAssemblyCount == assemblies.Length)
        {
            return null;
        }
            ...
            try
            {
                foreach (string candidate in candidates)
                {
                    ...
                    if (type != null)
                    {
                        ...
                        s_typeMissCache.TryRemove(fullName, out _);
                        return type;
                    }
                }
            }
        ...
        s_typeMissCache[fullName] = assemblies.Length;
        return null;
    }
