...
using Config = Il2CppBAPBAP.Pooling.NetworkPrefabPool.Config;
...
using AugmentSelection = Il2CppBAPBAP.Local.AugmentManager.AugmentSelection;
...
public sealed class MedusaMod : ModBase
{
	...
	public static class ViewPreMatchCharSelectSetDisplayedPatch
	{
		...
		public static void Postfix(CharacterConfiguration character)
		{
			try
			{
				...
				if (IsMedusaId(num) && _viewDisplayMedusaLogCount < 4)
				{
					_viewDisplayMedusaLogCount++;
					MedusaMod instance = _instance;
					if (instance != null)
					{
						((ModBase)instance).Log.Info($"[Medusa] View_PreMatch_CharSelect displayed Medusa; charId={num} autoEnabled={AutoSelectMedusaEnabled()} clicked={_localMedusaExplicitlySelected}.");
					}
				}
			}
			catch (Exception ex)
	...
	public static class GameModeSpawnPlayerCharPatch
	{
		...
		public static void Prefix(PlayerManager playerManager, Vector3 spawnPos)
		{
			...
			if (TryGetPreMatchSelectedChar(playerManager, out var selectedCharId) && IsMedusaId(selectedCharId))
			{
				ClearRecordedNonMedusaSelection(playerManager, "GameMode.SpawnPlayerChar.prefix.selectedMedusa");
				ForcePlayerMedusaChar(playerManager, selectedCharId, "GameMode.SpawnPlayerChar.prefix.selected");
				return;
			}
			if (!RestoreRecordedNonMedusaPlayerChar(playerManager, "GameMode.SpawnPlayerChar.prefix"))
			{
		...
		public static void Postfix(PlayerManager playerManager)
		{
			...
			try
			{
				if (!((Object)playerManager == (Object)null) && TryGetPreMatchSelectedChar(playerManager, out var selectedCharId) && IsMedusaId(selectedCharId))
				{
					ClearRecordedNonMedusaSelection(playerManager, "GameMode.SpawnPlayerChar.postfix.selectedMedusa");
					ForcePlayerMedusaChar(playerManager, selectedCharId, "GameMode.SpawnPlayerChar.postfix.selected");
					EnsureLiveMedusaEntity(playerManager.primaryCharManager, "GameMode.SpawnPlayerChar.postfix.selected");
					return;
				}
				if (!RestoreRecordedNonMedusaPlayerChar(playerManager, "GameMode.SpawnPlayerChar.postfix") && !((Object)playerManager == (Object)null) && IsMedusaId(playerManager.charId))
				{
	...
	[HarmonyPatch(typeof(GameMode), "OnDisable")]
	public static class GameModeOnDisablePatch
	{
		[HarmonyPostfix]
		public static void Postfix(GameMode __instance)
		{
			try
			{
				string text = (Object)__instance == (Object)null ? "<null>" : (((Object)__instance).name ?? __instance.GetType().Name);
				_instance?.ResetRuntimeSceneState("match ended GameMode.OnDisable:" + text + ":main");
			}
			catch
			{
			}
	...
	[HarmonyPatch(typeof(Animator), "SetLayerWeight", new Type[]
	{
		typeof(int),
		typeof(float)
	})]
	public static class AnimatorSetLayerWeightMedusaGuardPatch
	{
		[HarmonyPrefix]
		public static bool Prefix(Animator __instance, int layerIndex)
		{
			//IL_0001: Unknown result type (might be due to invalid IL or missing references)
			//IL_000c: Expected O, but got Unknown
			try
			{
				...
				int layerCount = __instance.layerCount;
				if (layerIndex >= 0 && layerIndex < layerCount)
				{
					return true;
				}
				if (_invalidAnimatorLayerGuardLogCount < 6)
				{
					_invalidAnimatorLayerGuardLogCount++;
					MedusaMod? instance = _instance;
					if (instance != null)
					{
						string text = ObjName((Object?)(object)__instance.runtimeAnimatorController);
						((ModBase)instance).Log.Info($"[Medusa] suppressed invalid Animator.SetLayerWeight({layerIndex}) for controller='{text}' layerCount={layerCount}.");
					}
				}
				return false;
			}
	...
	[HarmonyPatch(typeof(Animator), "CrossFadeInFixedTime", new Type[]
	{
		typeof(int),
		typeof(float),
		typeof(int),
	...
	public static class AnimatorCrossFadeInFixedTimeLayerGuardPatch
	{
		[HarmonyPrefix]
		public static bool Prefix(Animator __instance, int layer)
		{
			//IL_0001: Unknown result type (might be due to invalid IL or missing references)
			//IL_000c: Expected O, but got Unknown
			try
			{
				...
				if (_invalidAnimatorLayerGuardLogCount < 6)
				{
					...
					{
						string text = ObjName((Object?)(object)__instance.runtimeAnimatorController);
						((ModBase)instance).Log.Info($"[Medusa] suppressed invalid Animator.CrossFadeInFixedTime layer={layer} for controller='{text}' layerCount={layerCount}.");
					}
				}
				return false;
			}
	...
	private int _runtimeCleanupLogCount;
	...
	private static readonly HashSet<int> _visualDuplicatePruneRoots = new HashSet<int>();
	...
	private static readonly Dictionary<int, int> _originalNonMedusaCharByPlayerId = new Dictionary<int, int>();

	private static readonly Dictionary<string, int> _originalNonMedusaCharByAccountId = new Dictionary<string, int>(StringComparer.OrdinalIgnoreCase);

	private static int _nonMedusaIntentLogCount;
	...
	private static bool _launchRequestedCharacterIdChecked;

	private static int _launchRequestedCharacterId = -1;
	...
	private static int _invalidAnimatorLayerGuardLogCount;
	...
	private Texture2D? _nativeVfxSoftTexture;
	...
	public override void OnRegistered()
	{
		...
		((ModBase)this).Log.Info("[Medusa] Loaded (v1.6.70). Auto-registers at the lobby; /medusa or F7 to force, /medusa status, /medusa anim.");
		((ModBase)this).Log.Info("[Medusa] v1.6.70: cleans runtime Medusa FX on scene reset and GameMode shutdown, guards invalid animator layer writes/crossfades, restricts input fallback to the local Medusa entity, debounces duplicate cast drivers, and keeps the v1.6.66 native VFX isolation. Bundle: UserData\\Medusa\\medusa.bundle.");
		TryLoadBundle();
	...
	private void ResetRuntimeSceneState(string reason)
	{
		try
		{
			DestroyRuntimeMedusaArtifacts(reason);
			...
			_visualDuplicatePruneRoots.Clear();
			_originalNonMedusaCharByPlayerId.Clear();
			_originalNonMedusaCharByAccountId.Clear();
			_nonMedusaIntentLogCount = 0;
			...
			_lastPrematchCharSelectView = null;
			_lastPrematchMedusaIcon = null;
			_lastPrematchSubmitUtc = DateTime.MinValue;
			_lastPrematchScheduleUtc = DateTime.MinValue;
	...
	private void DestroyRuntimeMedusaArtifacts(string reason)
	{
		try
		{
			int num = 0;
			...
			if (val != null)
			{
				for (int i = 0; i < val.Length; i++)
				{
					...
					if ((Object)val2 == (Object)null || !val2.scene.IsValid())
					{
						continue;
					}
					string text = ((Object)val2).name ?? string.Empty;
					if (text.StartsWith("MedusaFX_", StringComparison.Ordinal))
					{
			...
			if (num > 0 && _runtimeCleanupLogCount < 16)
			{
				_runtimeCleanupLogCount++;
				((ModBase)this).Log.Info($"[Medusa] cleaned {num} runtime Medusa FX object(s) during scene reset: {reason}.");
			}
		}
		catch (Exception ex)
		{
			if (_runtimeCleanupLogCount < 16)
			{
				_runtimeCleanupLogCount++;
				((ModBase)this).Log.Warn("[Medusa] runtime artifact cleanup failed: " + ex.Message);
			}
		}
	}
	...
	private void LogLiveLocalDiagnostics()
	{
		...
		try
		{
			...
			bool flag4 = (Object)worldMedusa != (Object)null && LooksLikeMedusaEntity(worldMedusa) && !IsLikelyForeignBotProxyMedusaEntity(worldMedusa);
		...
		try
		{
			...
			if (val != null)
			{
				for (int num2 = 0; num2 < val.Length; num2++)
				{
					...
					{
						medusaCount++;
						if ((Object)val2 == (Object)null && IsExplicitMedusaRootEntity(entity) && IsLocalOwnedMedusaEntity(entity))
						{
							val2 = entity;
						}
	...
	private void PollLocalInputCastFx()
	{
		...
		try
		{
			...
			if (TryGetLocalMedusaEntityFast(out EntityManager val) && RunAuthoredMedusaAbilityDriver(null, val, num, "InputFallback"))
			{
	...
	private static bool TryGetLocalMedusaEntityFast(out EntityManager entity)
	{
		...
		{
			PlayerManager local = PlayerAPI.Local;
			EntityManager val = (((Object)local != (Object)null) ? local.primaryCharManager : null);
			if ((Object)val != (Object)null && IsMedusaId(SafeIntValue(() => val.charId)))
			{
				entity = val;
				return true;
			}
		...
		try
		{
			...
			if ((Object)authViewCharacter != (Object)null && IsMedusaId(SafeIntValue(() => authViewCharacter.charId)) && IsLocalOwnedMedusaEntity(authViewCharacter))
			{
				entity = authViewCharacter;
				return true;
			}
		}
		catch
	...
	private bool RunAuthoredMedusaAbilityDriver(Ability? ability, EntityManager caster, int slot, string source)
	{
		//IL_0001: Unknown result type (might be due to invalid IL or missing references)
		//IL_000c: Expected O, but got Unknown
	...
	private void ReassertMedusaVisibilityAfterAbilityCast(EntityManager caster, string source)
	{
		...
		{
			Reassert("+now");
			TimerAPI.Once(0.1f, (Action)delegate
			{
				Reassert("+0.10s");
			}, (ModBase)this);
		...
		void Reassert(string suffix)
		{
			...
			try
			{
				if (!((Object)caster == (Object)null))
				{
					...
					if (!((Object)val == (Object)null))
					{
						ForceMedusaVisualRenderable(val, source + ".abilityRenderable" + suffix);
						ForceMedusaVisualMaterialVisibility(val, source + ".abilityMaterial" + suffix);
	...
	private bool ShouldRunAbilityDriver(Ability? ability, EntityManager caster, int slot)
	{
		...
		float num4 = ((Object)ability == (Object)null) ? 0.55f : 0.38f;
		if (_recentAbilityDriver.TryGetValue(key, out var value) && num3 - value < num4)
		{
			return false;
		}
		...
		try
		{
			...
			for (int i = 0; i < val.Length; i++)
			{
				...
				if (!(magnitude < 0.01f))
				{
					...
					float magnitude2 = val5.magnitude;
					if (slot switch
					{
	...
	private bool ApplyAuthoredMedusaHit(EntityManager caster, EntityManager target, int slot, int ownerPlayerId, Vector3 origin, Vector3 dir)
	{
		...
		try
		{
			...
			charHurtbox.ApplyHit(slot switch
			{
				...
				3 => 160, 
				_ => 0, 
			}, array, ownerPlayerId, val2, false, false, true, true, false, val, true, false, (Collider)null);
			return true;
		}
		catch (Exception ex)
		{
	...
	private bool ShouldEmitCastFx(Ability? ability, int slot)
	{
		...
		float num3 = ((Object)ability == (Object)null) ? 0.55f : 0.3f;
		if (_recentCastFx.TryGetValue(key, out var value) && num2 - value < num3)
		{
			return false;
		}
	...
	private bool TrySpawnNativeMedusaCastFx(Transform casterTransform, Vector3 origin, Vector3 dir, int slot)
	{
		...
		try
		{
			...
			Vector3 val = casterTransform.position + Vector3.up * 0.18f;
			bool flag = false;
			switch (slot)
			{
			case 0:
				flag |= SpawnNativeMedusaVfx("VFX_Medusa_Poison_Muzzle", origin + dir * 0.45f, rotation, 1.8f);
				flag |= SpawnNativeMedusaVfx("VFX_Medusa_Poison_Trail", origin + dir * 1.35f, rotation, 1.8f);
				flag |= SpawnNativeMedusaVfx("VFX_Medusa_Poison_Hit", origin + dir * 3.6f, rotation, 1.8f);
				break;
			case 1:
				flag |= SpawnNativeMedusaVfx("VFX_Medusa_Poison_Muzzle", origin + dir * 0.5f, rotation, 1.8f);
				flag |= SpawnNativeMedusaVfx("VFX_Medusa_Poison_Trail", origin + dir * 1.25f, rotation, 1.8f);
				flag |= SpawnNativeMedusaVfx("VFX_Medusa_Poison_Puddle", val + dir * 1.65f, rotation, 3.2f);
				break;
			case 2:
				flag |= SpawnNativeMedusaVfx("VFX_Medusa_Poison_Escape", val, rotation, 2.2f);
				flag |= SpawnNativeMedusaVfx("VFX_Medusa_Poison_Trail", val - dir * 0.85f + Vector3.up * 0.35f, rotation, 1.8f);
				flag |= SpawnNativeMedusaVfx("VFX_Medusa_Poison_Puddle", val, rotation, 3f);
				break;
			case 3:
				flag |= SpawnNativeMedusaVfx("VFX_Medusa_Poison_Muzzle", origin + Vector3.up * 0.22f, rotation, 1.9f);
				flag |= SpawnNativeMedusaVfx("VFX_Medusa_Poison_Wall", casterTransform.position + dir * 2.6f + Vector3.up * 0.18f, rotation, 3f);
				flag |= SpawnNativeMedusaVfx("VFX_Medusa_Poison_Hit", origin + dir * 3.8f, rotation, 1.9f);
				break;
			}
			return flag;
		}
	...
	private bool SpawnNativeMedusaVfx(string prefabName, Vector3 position, Quaternion rotation, float ttl)
	{
		...
		try
		{
			...
			float num = NativeVfxRootScale(prefabName);
			...
			if (_nativeVfxSpawnLogCount < 24)
			{
				...
				((ModBase)this).Log.Info($"[Medusa] native FX spawned: name='{prefabName}' particles={value} renderers={value2} ttl={ttl:F2} scale={num:F2} pos=({position.x:F2},{position.y:F2},{position.z:F2}).");
			}
			return true;
		}
		catch (Exception ex)
			...
			try
			{
				...
				for (int i = 0; i < componentsInChildren.Length; i++)
				{
					...
					if (!((Object)val == (Object)null))
					{
						...
						try
						{
							val.main.startColor = new MinMaxGradient(NativeVfxMaterialColor(prefabName));
						}
						catch
						{
						}
	...
	private Material? CreateNativeVfxReplacementMaterial(Renderer renderer, string prefabName)
	{
		...
		try
		{
			...
			if ((Object)val != (Object)null)
			{
				num = (((Object)val).GetInstanceID() * 397) ^ StringComparer.OrdinalIgnoreCase.GetHashCode(prefabName);
		...
		Color val4 = NativeVfxMaterialColor(prefabName);
		Material val5 = new Material(val2)
		{
			name = "MedusaVfxRuntime_" + prefabName,
			color = val4
		};
		Texture nativeVfxSoftTexture = (Texture)(object)GetNativeVfxSoftTexture();
		if ((Object)nativeVfxSoftTexture != (Object)null)
		{
			string[] array2 = new string[3] { "_BaseMap", "_MainTex", "_Tex" };
			for (int j = 0; j < array2.Length; j++)
			{
				try
				{
					...
					{
						val5.SetTexture(text, nativeVfxSoftTexture);
					}
				}
				catch
				{
		...
		try
		{
			if (val5.HasProperty("_Surface"))
			{
				val5.SetFloat("_Surface", 1f);
			}
			if (val5.HasProperty("_Blend"))
			{
				val5.SetFloat("_Blend", 0f);
			}
			...
			string[] array3 = new string[3] { "_BaseColor", "_Color", "_TintColor" };
			for (int k = 0; k < array3.Length; k++)
			{
				if (val5.HasProperty(array3[k]))
				{
					val5.SetColor(array3[k], val4);
				}
			}
			if (val5.HasProperty("_EmissionColor"))
			{
				val5.SetColor("_EmissionColor", new Color(val4.r * 1.45f, val4.g * 1.45f, val4.b * 1.45f, val4.a));
				val5.EnableKeyword("_EMISSION");
			}
			val5.SetOverrideTag("RenderType", "Transparent");
			val5.EnableKeyword("_SURFACE_TYPE_TRANSPARENT");
		...
		if (num != 0)
		{
			_nativeVfxReplacementMaterials[num] = val5;
		}
		return val5;
	}

	private static Color NativeVfxMaterialColor(string prefabName)
	{
		//IL_0022: Unknown result type (might be due to invalid IL or missing references)
		//IL_004a: Unknown result type (might be due to invalid IL or missing references)
		//IL_0072: Unknown result type (might be due to invalid IL or missing references)
		...
		if (string.Equals(prefabName, "VFX_Medusa_Poison_Wall", StringComparison.OrdinalIgnoreCase))
		{
			return new Color(0.46f, 1f, 0.34f, 0.42f);
		}
		if (string.Equals(prefabName, "VFX_Medusa_Poison_Trail", StringComparison.OrdinalIgnoreCase))
		{
			return new Color(0.22f, 1f, 0.38f, 0.58f);
		}
		if (string.Equals(prefabName, "VFX_Medusa_Poison_Hit", StringComparison.OrdinalIgnoreCase))
		{
			return new Color(0.82f, 1f, 0.44f, 0.68f);
		}
		if (string.Equals(prefabName, "VFX_Medusa_Poison_Muzzle", StringComparison.OrdinalIgnoreCase))
		{
			return new Color(0.28f, 1f, 0.44f, 0.72f);
		}
		Color val = NativeVfxColor(prefabName);
		float num = val.a;
		if (string.Equals(prefabName, "VFX_Medusa_Poison_Wall", StringComparison.OrdinalIgnoreCase))
		{
			num = 0.5f;
		}
		else if (string.Equals(prefabName, "VFX_Medusa_Poison_Puddle", StringComparison.OrdinalIgnoreCase))
		{
			num = 0.62f;
		}
		else if (string.Equals(prefabName, "VFX_Medusa_Poison_Escape", StringComparison.OrdinalIgnoreCase))
		{
			num = 0.72f;
		}
		else if (string.Equals(prefabName, "VFX_Medusa_Poison_Hit", StringComparison.OrdinalIgnoreCase))
		{
			num = 0.78f;
		}
		return new Color(val.r, val.g, val.b, num);
	}

	private Texture2D? GetNativeVfxSoftTexture()
	{
		//IL_0006: Unknown result type (might be due to invalid IL or missing references)
		//IL_0011: Expected O, but got Unknown
		//IL_0021: Unknown result type (might be due to invalid IL or missing references)
		...
		if ((Object)_nativeVfxSoftTexture != (Object)null)
		{
			return _nativeVfxSoftTexture;
		}
		try
		{
			Texture2D val = new Texture2D(64, 64, (TextureFormat)4, false)
			{
				name = "MedusaVfxSoftGlow",
	...
	private static bool ShouldReplaceNativeVfxRendererMaterial(Renderer renderer, string prefabName)
	{
		...
		try
		{
			if (ShouldForceNativeVfxMaterialReplacement(prefabName))
			{
				return true;
			}
	...
	private static bool ShouldForceNativeVfxMaterialReplacement(string prefabName)
	{
		if (!string.Equals(prefabName, "VFX_Medusa_Poison_Wall", StringComparison.OrdinalIgnoreCase) && !string.Equals(prefabName, "VFX_Medusa_Poison_Trail", StringComparison.OrdinalIgnoreCase))
		{
			return string.Equals(prefabName, "VFX_Medusa_Poison_Hit", StringComparison.OrdinalIgnoreCase);
		}
		return true;
	}
	...
	private void FitMedusaVisualToBaseBounds(GameObject visual, bool hasBaseBounds, Bounds baseBounds, string source)
	{
		...
		try
		{
			...
			{
				float num = ClampFloat(baseBounds.size.y * 0.98f / bounds.size.y, 0.02f, 1f);
				...
				if (TryGetSkinnedRendererBounds(visual, includeMedusaVisual: true, out bounds))
				{
					...
					val = new Vector3(baseBounds.center.x - bounds.center.x, baseBounds.min.y - bounds.min.y, baseBounds.center.z - bounds.center.z);
				...
				if (ShouldLogMedusaVisualFit(source))
				{
					((ModBase)this).Log.Info($"[Medusa] visual fit via {source}: skinnedBaseH={baseBounds.size.y:0.###} skinnedVisualH={bounds.size.y:0.###} scaleFactor={num:0.###} localScale={visual.transform.localScale.x:0.###} localPos=({visual.transform.localPosition.x:0.###},{visual.transform.localPosition.y:0.###},{visual.transform.localPosition.z:0.###}).");
				}
			}
			else
			{
		...
		try
		{
			...
			foreach (Transform componentsInChild in root.GetComponentsInChildren<Transform>(true))
			{
				...
				if (gameObject.activeInHierarchy)
				{
					...
					if ((Object)FindMedusaAnimatorUnder(gameObject.transform) != (Object)null)
					{
						return gameObject;
					}
				}
	...
	private void PruneDuplicateMedusaVisuals(GameObject root, GameObject keep, string source)
	{
		//IL_0001: Unknown result type (might be due to invalid IL or missing references)
		//IL_000c: Expected O, but got Unknown
		//IL_000f: Unknown result type (might be due to invalid IL or missing references)
		...
		try
		{
			if ((Object)root == (Object)null || (Object)keep == (Object)null)
			{
				return;
			}
			int unityInstanceId = GetUnityInstanceId((Object?)(object)keep);
			if (unityInstanceId == 0)
			{
				return;
			}
			...
			foreach (Transform componentsInChild in root.GetComponentsInChildren<Transform>(true))
			{
				...
				{
					GameObject gameObject = ((Component)componentsInChild).gameObject;
					if (GetUnityInstanceId((Object?)(object)gameObject) != unityInstanceId)
					{
						list.Add(gameObject);
					}
				}
			...
			int unityInstanceId2 = GetUnityInstanceId((Object?)(object)root);
			if (list.Count > 0 && (unityInstanceId2 == 0 || _visualDuplicatePruneRoots.Add(unityInstanceId2)) && _liveGraftLogCount < 10)
			{
				_liveGraftLogCount++;
				((ModBase)this).Log.Info($"[Medusa] pruned duplicate live visual(s) via {source}: removed={list.Count} keep='{((Object)keep).name}'.");
			}
		}
		catch (Exception ex)
		{
			if (_visualDiagnosticsLogCount < 24)
			{
				...
				((ModBase)this).Log.Warn("[Medusa] duplicate visual prune failed via " + source + ": " + ex.Message);
			}
		}
	}
	...
	private bool IsLiveMedusaVisualStable(GameObject root, GameObject visual, string source)
	{
		...
		try
		{
			...
			if (NeedsMedusaVisualMaterialOpacityRepair(visual))
			{
				return false;
			}
			return true;
	...
	private void EnsureStableLiveMedusaVisualCheap(GameObject root, GameObject visual, string source)
	{
		...
		try
		{
			...
			ForceMedusaVisualMaterialVisibility(visual, source + ".cheapMaterial");
	...
	private void LogMedusaVisualDiagnostics(GameObject root, GameObject visual, string source)
	{
		...
		try
		{
			...
			try
			{
				...
				{
					Vector3 val2 = main.WorldToScreenPoint(flag ? val.center : visual.transform.position);
	...
	private void ForceMedusaVisualMaterialVisibility(GameObject visual, string source)
	{
		...
		try
		{
			...
			{
				ForceMedusaRendererMaterialsOpaque(visual, source + ".prepared");
				{
					foreach (Renderer componentsInChild in visual.GetComponentsInChildren<Renderer>(true))
					{
						if (!((Object)componentsInChild == (Object)null))
			...
			foreach (SkinnedMeshRenderer componentsInChild2 in visual.GetComponentsInChildren<SkinnedMeshRenderer>(true))
			{
				...
				try
				{
					...
					if ((Object)val3 != (Object)null)
					{
						...
						ForceMedusaMaterialOpaque(val3);
	...
	private static bool NeedsMedusaVisualMaterialOpacityRepair(GameObject visual)
	{
		//IL_0001: Unknown result type (might be due to invalid IL or missing references)
		//IL_000c: Expected O, but got Unknown
		//IL_006b: Unknown result type (might be due to invalid IL or missing references)
		...
		try
		{
			...
			foreach (Renderer componentsInChild in visual.GetComponentsInChildren<Renderer>(true))
			{
				...
				string[] array = new string[4] { "_Color", "_BaseColor", "_TintColor", "_MainColor" };
				foreach (string text in array)
				{
					try
					{
						if (sharedMaterial.HasProperty(text) && sharedMaterial.GetColor(text).a < 0.98f)
						{
							return true;
						}
					}
				...
				foreach (string text2 in array)
				{
					try
					{
						if (sharedMaterial.HasProperty(text2) && sharedMaterial.GetFloat(text2) < 0.98f)
						{
							return true;
						}
					}
	...
	private int ForceMedusaRendererMaterialsOpaque(GameObject visual, string source)
	{
		//IL_0003: Unknown result type (might be due to invalid IL or missing references)
		//IL_000e: Expected O, but got Unknown
		//IL_002e: Unknown result type (might be due to invalid IL or missing references)
		...
		try
		{
			...
			foreach (Renderer componentsInChild in visual.GetComponentsInChildren<Renderer>(true))
			{
				...
				try
				{
					foreach (Material item in (Il2CppArrayBase<Material>)(object)componentsInChild.sharedMaterials)
					{
						if (ForceMedusaMaterialOpaque(item))
						{
							num++;
						}
					}
				...
				try
				{
					if (ForceMedusaMaterialOpaque(componentsInChild.material))
					{
						num++;
					}
				}
			...
			if (num > 0 && _materialVisibilityLogCount < 12)
			{
				_materialVisibilityLogCount++;
				((ModBase)this).Log.Info($"[Medusa] forced renderer material opacity via {source}: materials={num}.");
			}
		}
		catch (Exception ex)
		{
			...
			{
				_materialVisibilityLogCount++;
				((ModBase)this).Log.Warn("[Medusa] renderer material opacity repair failed via " + source + ": " + ex.Message);
			}
		}
		return num;
	}

	private static bool ForceMedusaMaterialOpaque(Material material)
	{
		//IL_0001: Unknown result type (might be due to invalid IL or missing references)
		//IL_000c: Expected O, but got Unknown
		//IL_004f: Unknown result type (might be due to invalid IL or missing references)
		...
		try
		{
			...
			foreach (string text2 in array)
			{
				if (material.HasProperty(text2))
				{
					material.SetFloat(text2, 1f);
				}
			}
			array = new string[11]
			{
				"_Cutoff", "_AlphaClip", "_DissolveAmount", "_ErodeAmount", "_IsEroded", "_FoWOccludeOverlay", "_MultiplyFoWToAlpha", "_MultiplyFoWToColor", "_HitAmmount", "_HitAmount",
				"_HitBlinkAmount"
			};
			foreach (string text3 in array)
			{
				if (material.HasProperty(text3))
				{
					material.SetFloat(text3, 0f);
				}
			}
			array = new string[2] { "_Surface", "_Blend" };
			foreach (string text4 in array)
			{
				if (material.HasProperty(text4))
				{
					material.SetFloat(text4, 0f);
				}
			}
			...
			array = new string[7] { "_SURFACE_TYPE_TRANSPARENT", "_ALPHATEST_ON", "_ALPHABLEND_ON", "_ALPHAPREMULTIPLY_ON", "_RENDERING_FADE", "_RENDERING_TRANSPARENT", "_DISSOLVE_ON" };
			foreach (string text5 in array)
			{
				try
				{
					material.DisableKeyword(text5);
				}
				catch
				{
				}
	...
	private void EnsureLiveMedusaVisual(GameObject root, string source)
	{
		...
		try
		{
			...
			{
				PruneDuplicateMedusaVisuals(root, val, source + ".stable");
			...
			if ((Object)FindMedusaAnimatorUnder(root.transform) != (Object)null)
			{
				...
				if ((Object)val != (Object)null)
				{
					PruneDuplicateMedusaVisuals(root, val, source + ".existing");
			...
			PruneDuplicateMedusaVisuals(root, val3, source + ".new.preAnchor");
	...
	private static int LaunchRequestedCharacterId()
	{
		if (_launchRequestedCharacterIdChecked)
		{
			return _launchRequestedCharacterId;
		}
		_launchRequestedCharacterIdChecked = true;
		try
		{
			string[] commandLineArgs = Environment.GetCommandLineArgs();
			for (int i = 0; i < commandLineArgs.Length; i++)
			{
				string text = commandLineArgs[i] ?? string.Empty;
				if (text.StartsWith("--bapcustom-selected-char=", StringComparison.OrdinalIgnoreCase))
				{
					int num = ParseIntOr(text.Substring("--bapcustom-selected-char=".Length), -1);
					if (num >= 0)
					{
						_launchRequestedCharacterId = num;
						break;
					}
				}
				if (text.StartsWith("--bapcustom-character-id=", StringComparison.OrdinalIgnoreCase))
				{
					int num2 = ParseIntOr(text.Substring("--bapcustom-character-id=".Length), -1);
					if (num2 >= 0)
					{
						_launchRequestedCharacterId = num2;
						break;
					}
				}
			}
	...
	private static bool LaunchRequestedNonMedusa(out int requestedCharId)
	{
		requestedCharId = LaunchRequestedCharacterId();
		if (requestedCharId >= 0)
		{
			return !IsMedusaId(requestedCharId);
		}
		return false;
	}
	...
	private static bool IsLocalOwnedMedusaEntity(EntityManager? entity)
	{
		//IL_0019: Unknown result type (might be due to invalid IL or missing references)
		//IL_0024: Expected O, but got Unknown
		//IL_0068: Unknown result type (might be due to invalid IL or missing references)
		...
		try
		{
			...
			NetworkBehaviour val = (NetworkBehaviour)entity;
			try
			{
				if (val.isOwned || val.isLocalPlayer)
				{
					return true;
				}
			}
			...
			if (num <= 0)
			{
				num = PlayerAPI.LocalId;
			}
			int num2 = SafeIntValue(() => entity.ownerPlayerId);
			int num3 = SafeIntValue(() => entity.GetPlayerId());
			return num > 0 && (num2 == num || num3 == num);
		}
		catch
		{
			return false;
	...
	private static bool IsLikelyForeignBotProxyMedusaEntity(EntityManager? entity)
	{
		...
		if (IsExplicitMedusaRootEntity(entity) && IsLocalOwnedMedusaEntity(entity))
		{
			return false;
		}
		try
		{
			if ((((Object)((Component)entity).gameObject).name ?? "").IndexOf("Bot", StringComparison.OrdinalIgnoreCase) < 0)
			{
				return false;
			}
			int localId = PlayerAPI.LocalId;
			int num = SafeIntValue(() => entity.ownerPlayerId);
			int num2 = SafeIntValue(() => entity.GetPlayerId());
			if (localId > 0 && (num == localId || num2 == localId))
			{
				return false;
			}
			return num <= 0 || num2 > 0;
	...
	private static void NormalizeQueueMatchedMedusaSelections(QueueMatchedData? qmd, string source)
	{
		...
		try
		{
			...
			int requestedCharId;
			bool flag4 = LaunchRequestedNonMedusa(out requestedCharId) && !localMedusaExplicitlySelected;
			int num2 = 0;
			int num3 = 0;
			for (int num4 = 0; num4 < count; num4++)
			{
				...
				bool flag5 = PlayerDataMatchesLocal(playerData, num, localAccountId) || (count == 1 && flag3);
				bool flag6 = IsMedusaId(SafeIntValue(() => playerData.charId));
				if (!flag6)
				{
					RecordNonMedusaSelection(playerData, source);
				}
				if (flag5 && flag4)
				{
					if (!flag6)
					...
					try
					{
						playerData.charId = requestedCharId;
					}
					catch
					{
					}
					RecordNonMedusaSelection(playerData, source + ".blockedLocal");
					if (_matchSelectLogCount < 48)
					{
						_matchSelectLogCount++;
						MedusaMod instance = _instance;
						if (instance != null)
						{
							((ModBase)instance).Log.Info($"[Medusa] qmd Medusa override blocked via {source}: local launch requested charId={requestedCharId}; restored qmd char.");
						}
					}
				}
				else
				{
					if (!flag6 && !(flag3 && flag5))
					{
						continue;
					}
					...
					if (instance2 != null)
					{
						((ModBase)instance2).Log.Info($"[Medusa] qmd Medusa normalized via {source}: playerId={num5} account={SafeString(() => playerData.accountId)} localMatch={flag5} old={value} new={CurrentMedusaId()} skin={value2}->-1.");
					}
				}
			}
			if ((num2 > 0 || num3 > 0) && _matchSelectLogCount < 48)
			{
				...
				if (instance3 != null)
				{
					((ModBase)instance3).Log.Info($"[Medusa] qmd Medusa selection summary via {source}: forcedPlayers={num2} prematchSelections={num3} explicitLocal={flag3} launch={flag} autoSelect={flag2} remembered={localMedusaExplicitlySelected} localPlayerId={num}.");
				}
			}
		}
		catch (Exception ex)
		{
			...
			if (instance4 != null)
			{
				((ModBase)instance4).Log.Warn("[Medusa] qmd Medusa normalization failed via " + source + ": " + ex.Message);
			}
		}
	}
	...
	private static void RecordNonMedusaSelection(MatchmakingPlayerData playerData, string source)
	{
		try
		{
			if (playerData == null)
			...
			int num = SafeIntValue(() => playerData.charId);
			if (num < 0 || IsMedusaId(num))
			{
				return;
			}
			int num2 = SafeIntValue(() => playerData.playerId);
			string text = SafeString(() => playerData.accountId);
			if (num2 > 0)
			{
				_originalNonMedusaCharByPlayerId[num2] = num;
				if (!TryGetPreMatchSelectedCharForPlayerId(num2, out _))
				{
					SetPreMatchSelectionForPlayerId(num2, num, source + ".preserveNonMedusa.initial");
				}
			}
			if (!string.IsNullOrWhiteSpace(text))
			{
				_originalNonMedusaCharByAccountId[text] = num;
			}
			if (_nonMedusaIntentLogCount < 16)
			{
				_nonMedusaIntentLogCount++;
				MedusaMod instance = _instance;
				if (instance != null)
				{
					((ModBase)instance).Log.Info($"[Medusa] recorded non-Medusa intent via {source}: playerId={num2} account={text} charId={num}.");
				}
			}
		}
		catch (Exception ex)
		{
			...
			if (instance2 != null)
			{
				((ModBase)instance2).Log.Warn("[Medusa] failed to record non-Medusa intent via " + source + ": " + ex.Message);
			}
		}
	}

	private static bool TryGetRecordedNonMedusaSelection(PlayerManager? player, out int charId)
	{
		//IL_0016: Unknown result type (might be due to invalid IL or missing references)
		//IL_0021: Expected O, but got Unknown
		charId = -1;
		try
		{
			...
			int num = SafeIntValue(() => player.playerId);
			if (num > 0 && _originalNonMedusaCharByPlayerId.TryGetValue(num, out charId))
			{
				return charId >= 0 && !IsMedusaId(charId);
			}
			string text = SafeString(() => player.accountId);
			if (!string.IsNullOrWhiteSpace(text) && _originalNonMedusaCharByAccountId.TryGetValue(text, out charId))
			{
				return charId >= 0 && !IsMedusaId(charId);
			}
		}
	...
	private static bool ShouldRespectRecordedNonMedusaIntent(PlayerManager? player, string source, out int charId)
	{
		charId = -1;
		if (!TryGetRecordedNonMedusaSelection(player, out charId))
		{
			return false;
		}
		bool hasPrematchSelection = TryGetPreMatchSelectedChar(player, out var selectedCharId);
		if (hasPrematchSelection && IsMedusaId(selectedCharId))
		{
			ClearRecordedNonMedusaSelection(player, source + ".selectedMedusa");
			return false;
		}
		if (_localMedusaExplicitlySelected || LaunchRequestedMedusa() || AutoSelectMedusaEnabled())
		{
			return false;
		}
		// The player's live, in-match character selection is authoritative. Prefer the prematch
		// char-select value (what they actually clicked); fall back to the live PlayerManager char.
		int currentChar = hasPrematchSelection ? selectedCharId : SafeIntValue(() => player.charId);
		// If the player currently holds a valid NON-Medusa character, that is a legitimate
		// selection - possibly different from the originally recorded qmd char (e.g. they
		// re-picked in the 20s char-select). Respect it instead of forcing the old recorded
		// char back over it (that overwrite was the "spawn as the wrong character" bug).
		// Track the new selection so later AssignCharacters/SpawnPlayerChar passes stay stable.
		// Only force the recorded char back when the current char is Medusa, which for a
		// non-Medusa player means an unwanted forced conversion that must be undone.
		if (currentChar >= 0 && !IsMedusaId(currentChar))
		{
			if (currentChar != charId)
			{
				UpdateRecordedNonMedusaSelectionValue(player, currentChar, source + ".trackNewSelection");
				charId = currentChar;
			}
			return false;
		}
	...
	private static void UpdateRecordedNonMedusaSelectionValue(PlayerManager? player, int charId, string source)
	{
		//IL_0013: Unknown result type (might be due to invalid IL or missing references)
		//IL_001e: Expected O, but got Unknown
		try
		{
			if ((Object)player == (Object)null || charId < 0 || IsMedusaId(charId))
			{
				return;
			}
			int num = SafeIntValue(() => player.playerId);
			if (num > 0)
			{
				_originalNonMedusaCharByPlayerId[num] = charId;
			}
			string text = SafeString(() => player.accountId);
			if (!string.IsNullOrWhiteSpace(text))
			{
				_originalNonMedusaCharByAccountId[text] = charId;
			}
			if (_nonMedusaIntentLogCount < 48)
			{
				_nonMedusaIntentLogCount++;
				MedusaMod instance = _instance;
				if (instance != null)
				{
					((ModBase)instance).Log.Info($"[Medusa] tracked updated non-Medusa selection via {source}: playerId={num} account={text} charId={charId}.");
				}
			}
		}
		catch
	...
	private static void ClearRecordedNonMedusaSelection(PlayerManager? player, string source)
	{
		//IL_0013: Unknown result type (might be due to invalid IL or missing references)
		//IL_001e: Expected O, but got Unknown
		try
		{
			...
			if (num > 0)
			{
				flag |= _originalNonMedusaCharByPlayerId.Remove(num);
			}
			string text = SafeString(() => player.accountId);
			if (!string.IsNullOrWhiteSpace(text))
			{
				flag |= _originalNonMedusaCharByAccountId.Remove(text);
			}
			if (flag && _nonMedusaIntentLogCount < 48)
			{
				_nonMedusaIntentLogCount++;
				MedusaMod instance = _instance;
				if (instance != null)
				{
					((ModBase)instance).Log.Info($"[Medusa] cleared recorded non-Medusa intent via {source}: playerId={num} account={text}.");
				}
			}
		}
		catch (Exception ex)
		{
			...
			if (instance2 != null)
			{
				((ModBase)instance2).Log.Warn("[Medusa] failed to clear non-Medusa intent via " + source + ": " + ex.Message);
			}
		}
	}

	private static bool RestoreRecordedNonMedusaPlayerChar(PlayerManager? player, string source)
	{
		try
		{
			if (!ShouldRespectRecordedNonMedusaIntent(player, source, out var charId))
			{
				return false;
			}
			int value = SafeIntValue(() => player.charId);
			int num = SafeIntValue(() => player.playerId);
			try
			{
			...
			if (num > 0)
			{
				SetPreMatchSelectionForPlayerId(num, charId, source + ".restoreNonMedusa");
			}
			UpdateMatchmakingPlayerDataForPlayer(player, charId, source + ".restoreNonMedusa", forceMedusaSkin: false);
			if (_nonMedusaIntentLogCount < 32)
			{
				_nonMedusaIntentLogCount++;
				MedusaMod instance = _instance;
				if (instance != null)
				{
					((ModBase)instance).Log.Info($"[Medusa] restored recorded non-Medusa char via {source}: playerId={num} old={value} new={charId}.");
				}
			}
			return true;
		}
		catch (Exception ex)
		{
			...
			if (instance2 != null)
			{
				((ModBase)instance2).Log.Warn("[Medusa] failed to restore non-Medusa intent via " + source + ": " + ex.Message);
			}
		}
		return false;
	}
	...
	private static void TrySubmitMedusaSelection(string source, bool lockAfter)
	{
		...
		try
		{
			...
			if (LaunchRequestedNonMedusa(out var requestedCharId) && !_localMedusaExplicitlySelected && !AutoSelectMedusaEnabled())
			{
				if (_prematchSubmitLogCount < 8)
				{
					_prematchSubmitLogCount++;
					((ModBase)instance).Log.Info($"[Medusa] prematch submit blocked via {source}: launch requested non-Medusa charId={requestedCharId}.");
				}
				return;
			}
			DateTime utcNow = DateTime.UtcNow;
	...
	private static void UpdatePreMatchSelectionState(PlayerManager? player, int charId, string source)
	{
		...
		try
		{
			if ((Object)player == (Object)null || !IsMedusaId(charId) || RestoreRecordedNonMedusaPlayerChar(player, source + ".blockedNonMedusaIntent"))
			{
				return;
			}
			int num = -1;
	...
	private static bool TryGetPreMatchSelectedChar(PlayerManager? player, out int charId)
	{
		...
		try
		{
			...
			return TryGetPreMatchSelectedCharForPlayerId(playerId, out charId);
		}
		catch
		{
		}
	...
	private static bool TryGetPreMatchSelectedCharForPlayerId(int playerId, out int charId)
	{
		//IL_0047: Unknown result type (might be due to invalid IL or missing references)
		//IL_0052: Expected O, but got Unknown
		charId = -1;
	...
	private static void UpdateMatchmakingPlayerDataForPlayer(PlayerManager? player, int charId, string source, bool forceMedusaSkin = true)
	{
		//IL_0001: Unknown result type (might be due to invalid IL or missing references)
		//IL_000c: Expected O, but got Unknown
		//IL_0074: Unknown result type (might be due to invalid IL or missing references)
		...
		try
		{
			if ((Object)player == (Object)null || charId < 0 || (forceMedusaSkin && !IsMedusaId(charId)))
			{
				return;
			}
			int num = -1;
			...
			foreach (GameManager item in val)
			{
				...
				for (int i = 0; i < val2.Count; i++)
				{
					...
					if (_matchSelectLogCount < 42)
					{
						...
						if (instance != null)
						{
							string value3 = (forceMedusaSkin ? $"{value2}->-1" : $"{value2}->unchanged");
							((ModBase)instance).Log.Info($"[Medusa] qmd player char via {source}: playerId={num} account={text} old={value} new={charId} skin={value3}.");
						}
					}
				}
			}
	...
	private static void ForceSelectedMedusaPlayers(PreMatchManager? manager, string source)
	{
		...
		try
		{
			...
			{
				if ((Object)player == (Object)null || RestoreRecordedNonMedusaPlayerChar(player, source + ".preserveNonMedusa"))
				{
					continue;
				}
				int charId = -1;
	...
	private static void RepairLocalMedusaBinding(EntityManager? entity, string source, bool forceWhenLocalPrimaryMissing = false)
	{
		...
		try
		{
			...
			bool flag4 = forceWhenLocalPrimaryMissing && flag && flag3 && IsExplicitMedusaRootEntity(entity) && IsLocalOwnedMedusaEntity(entity);
			if (!num5 && !flag2 && !flag4)
			{
	...
	private static void RepairMedusaCameraTarget(EntityManager? entity, string source)
	{
		...
		try
		{
			...
			int num2 = SafeIntValue(() => entity.ownerPlayerId);
			int num3 = SafeIntValue(() => entity.GetPlayerId());
			bool num4 = num2 == num || num3 == num || (Object)entity.playerManager == (Object)local || (Object)entity.playerObj == (Object)gameObject;
			bool num5 = IsExplicitMedusaRootEntity(entity);
			EntityManager primaryCharManager = local.primaryCharManager;
			bool flag = num2 <= 0 && num3 <= 0;
			bool flag2 = num5 && flag && (Object)primaryCharManager == (Object)null && IsLocalOwnedMedusaEntity(entity);
			if (!num4 && !flag2)
			{
				return;
			}
			...
			if (val != null)
			{
				for (int num8 = 0; num8 < val.Length; num8++)
				{
					CameraController val2 = val[num8];
					if ((Object)val2 == (Object)null)
					{
						continue;
					}
					...
					try
					{
						flag3 = (Object)val2.Target == (Object)transform;
					}
					catch
					{
					}
			...
			int item = ((!((Object)gameObject2 == (Object)null)) ? ((Object)gameObject2).GetInstanceID() : 0);
			if ((num6 > 0 || _cameraTargetRepairIds.Add(item)) && _cameraTargetRepairLogCount < 24)
			{
	...
	private static void EnsureLiveMedusaEntity(EntityManager? entity, string source)
	{
		...
		try
		{
			if ((Object)entity == (Object)null || !LooksLikeMedusaEntity(entity) || IsLikelyForeignBotProxyMedusaEntity(entity))
			{
				return;
			}
	...
	private static void ForceCharacterPageModelUnlocked(CharacterPageModel? data, string source)
	{
		...
		try
		{
			...
			Il2CppSystem.Collections.Generic.HashSet<int> val3 = new Il2CppSystem.Collections.Generic.HashSet<int>();
			for (int j = 0; j < array.Length; j++)
			{
				val3.Add(array[j]);
			}
			...
			foreach (StatusEffectManager item in val)
			{
				...
				for (int i = 0; i < ((Il2CppArrayBase<StatusEffectSO>)(object)statusEffects).Length; i++)
				{
					...
					try
					{
						...
						text = ((il2CppType != null) ? il2CppType.Name : "");
					}
					catch
					{
					}
