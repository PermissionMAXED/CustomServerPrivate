...
# Ownership marker: only ever rm -rf a prefix THIS script created. WINEPREFIX is operator-settable
# (BAPCUSTOM_WINEPREFIX), so without this guard a wrapper-version bump could recursively delete a
# shared/pre-existing prefix the operator pointed us at.
OWNED_MARKER="$WINEPREFIX/.bapcustom-owned"
if [[ ! -f "$OWNED_MARKER" && ( -f "$WINEPREFIX/.bapcustom-wineboot-ok" || -f "$PREFIX_KEY_FILE" ) ]]; then
  echo "[start-match] WARNING: wineprefix '$WINEPREFIX' has bapcustom markers but no ownership marker; skipping destructive reset (not deleting a prefix this script did not create)"
