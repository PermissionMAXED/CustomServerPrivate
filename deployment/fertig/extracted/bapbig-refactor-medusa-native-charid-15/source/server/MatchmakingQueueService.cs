...
public sealed class MatchmakingQueueService
{
    ...
    public QueueJoinResult JoinQueue(string accountId, string username, int discriminator, int charId, int points)
    {
    ...
    public bool LeaveQueue(string accountId)
    {
        ...
        try
        {
            ...
            _logger.LogDebug("Player {AccountId} left the matchmaking queue. Remaining: {Count}.", accountId, _queue.Count);
    ...
    public void RequeuePlayers(IEnumerable<QueueEntry> players, string reason = "failed match start")
    {
        ...
        try
        {
            ...
            if (_queue.Count > 0)
            {
                int retryDelaySeconds = Math.Clamp(_options.FailedStartRetryDelaySeconds, 0, _options.QueueTimerSeconds);
                _queueStartedUtc = now.AddSeconds(-Math.Max(0, _options.QueueTimerSeconds - retryDelaySeconds));
            }

            if (added > 0)
            {
                _logger.LogWarning(
                    "Requeued {Count} matchmaking player(s) after {Reason}. retryDelay={RetryDelay}s failures={Failures}.",
                    added,
                    reason,
                    Math.Clamp(_options.FailedStartRetryDelaySeconds, 0, _options.QueueTimerSeconds),
    ...
    {
        if (_queueStartedUtc is null)
            return _options.QueueTimerSeconds;

        int elapsed = (int)(DateTimeOffset.UtcNow - _queueStartedUtc.Value).TotalSeconds;
        int remaining = _options.QueueTimerSeconds - elapsed;
    ...
    private bool IsReadyInternal(out int secondsElapsed)
    {
        ...
        if (secondsElapsed < _options.QueueTimerSeconds)
            return false;
