...
public sealed class CustomServerOptions
{
    ...
    public int GameServerWrapperOnlyStartupStallGraceSeconds { get; set; } = 60;
    public int GameServerWrapperOnlyStartupStallSeconds { get; set; } = 25;
    ...
    public CharacterUnlockOptions CharacterUnlocks { get; set; } = new();
...
///   - Match start:  <c>[Analytics] Match started: {GameId} mapId={MapId} players={PlayerCount}</c>
...
public sealed class AnalyticsOptions
{
    ...
    public string MatchStartRegex { get; set; } = @"\[Analytics\] Match started: (?<match>\S+)";
...
public sealed class MapPoolOptions
{
    ...
    /// <summary>Custom map (mapId 5). Allowed for EXPLICIT selection by default, but excluded from the
    /// random per-match rotation (see <see cref="BuildRandomMapIds"/>) so normal matches don't land on it
    /// unexpectedly. Set false to forbid it entirely.</summary>
    public bool EnableMapCustomFlat { get; set; } = true;
    ...
    /// <summary>Resolve to canonical map ids that may be SELECTED (explicitly or randomly). Always
    /// returns at least one id. Includes the custom map when enabled so an explicit mapId-5 request
    /// passes the allow-list in PickMapId.</summary>
    public int[] BuildEnabledMapIds()
    {
        ...
        if (EnableMapCustomFlat)                               ids.Add(MapCatalog.CustomFlatId);
        if (ids.Count == 0) ids.Add(MapCatalog.BazaarCityId); // never return empty
        return ids.ToArray();
    }

    /// <summary>The subset of <see cref="BuildEnabledMapIds"/> eligible for RANDOM per-match rotation —
    /// excludes the custom map so normal matches never land on it unless explicitly requested.</summary>
    public int[] BuildRandomMapIds()
    {
        int[] all = BuildEnabledMapIds();
        var rnd = all.Where(id => !MapCatalog.IsCustomMapId(id)).ToArray();
        return rnd.Length > 0 ? rnd : all;
    }
...
/// </summary>
public sealed class CharacterUnlockOptions
{
    /// <summary>Compatibility bypass. When true, every roster character is treated as owned.</summary>
    public bool UnlockAllCharacters { get; set; } = true;

    /// <summary>Char-token balance assigned to new economy profiles.</summary>
    public int StartingCharTokens { get; set; } = 10;

    /// <summary>Currency asset id used by the native client for character tokens.</summary>
    public int CharTokenAssetId { get; set; } = 2;

    /// <summary>Fallback price for any character not listed in Prices.</summary>
    public int DefaultPrice { get; set; } = 1;

    /// <summary>Characters every account owns even when UnlockAllCharacters is false.</summary>
    public int[] InitiallyUnlockedCharacterIds { get; set; } = [0];

    /// <summary>Comma-separated alternative to InitiallyUnlockedCharacterIds.</summary>
    public string InitiallyUnlockedCharacterIdsCsv { get; set; } = "";

    /// <summary>Per-character price table.</summary>
    public CharacterPriceEntry[] Prices { get; set; } = CharacterCatalog.AllIds
        .Select(id => new CharacterPriceEntry { CharId = id, Price = id == 0 ? 0 : 1 })
        .ToArray();

    /// <summary>Comma-separated price overrides, e.g. "0:0,1:1,15:5".</summary>
    public string PricesCsv { get; set; } = "";
}

public sealed class CharacterPriceEntry
{
...
public sealed class MatchDefaults
{
    ...
    [
        ...
        new MapMappingEntry { UnityGameModeId = 2, MapIds = [1] },
        new MapMappingEntry { UnityGameModeId = 3, MapIds = [1, 2, 3, 4] }
    ];
    ...
    public string EnabledGameModeIdsCsv { get; set; } = "0,3,4,5,9,10";
    public bool RandomizeMapPerMatch { get; set; } = true;

    /// <summary>
    /// Per-game-mode overrides for player limit and bots. Empty = current behavior
    /// (team size derived from the mode, bots from the matchmaking queue defaults).
    /// Each entry targets a matchmaking game mode id (0..10).
    /// </summary>
    public PerModeSettingsEntry[] PerModeSettings { get; set; } = [];

    /// <summary>Optional JSON override for PerModeSettings (mergeable from AMP as a single string).</summary>
    public string PerModeSettingsJson { get; set; } = "";
}
...
/// </summary>
/// <summary>
/// Per-mode override for player limit and bots, keyed by matchmaking game mode id (0..10).
/// Mirrors the MapMapping pattern: empty array = no override (derive team size, use queue bot defaults).
/// </summary>
public sealed class PerModeSettingsEntry
{
    /// <summary>Matchmaking game mode id this entry targets (0=Warmup,3=Solos,4=Duos,5=Trios,9=FFA,10=Custom).</summary>
    public int MatchmakingGameModeId { get; set; }

    /// <summary>Optional friendly name for display only.</summary>
    public string GameModeName { get; set; } = "";

    /// <summary>Total player slots for this mode (bots fill the remainder). Default 8.</summary>
    public int MaxPlayers { get; set; } = 8;

    /// <summary>Players per team. 0 = derive from the mode (Duos->2, Trios->3, else 1).</summary>
    public int TeamSize { get; set; }

    /// <summary>Bots to add. -1 = use the matchmaking queue default.</summary>
    public int BotCount { get; set; } = -1;

    /// <summary>Bot difficulty. -1 = use the matchmaking queue default.</summary>
    public int BotDifficulty { get; set; } = -1;
}
...
public static class MapCatalog
{
    ...
    /// <summary>First mapId reserved for custom maps. Any mapId &gt;= this is a custom map slot,
    /// loaded client-side/host-side by the mod from a UserData/CustomMaps/*.json (see CustomMapLoader),
    /// which already supports an arbitrary number of custom maps keyed by their JSON "mapId". Shipped
    /// maps occupy 1..4; custom maps occupy 5.. so multiple custom maps can coexist (5, 6, 7, ...).</summary>
    public const int FirstCustomMapId = 5;

    /// <summary>Back-compat alias for the first/default custom map slot. Equals <see cref="FirstCustomMapId"/>.</summary>
    public const int CustomFlatId = FirstCustomMapId;

    /// <summary>Highest mapId the dedicated host can load. The mod's KnownLevelNames carrier array
    /// is sized 41 (indices 0..40), so any mapId above this would index out of bounds and crash
    /// the match host (GameMode level-load). Custom map slots occupy <see cref="FirstCustomMapId"/>..40.</summary>
    public const int MaxCustomMapId = 40;

    /// <summary>True if the mapId is a usable custom-map slot (<see cref="FirstCustomMapId"/>..<see cref="MaxCustomMapId"/>).
    /// Custom maps are explicit-select only and excluded from random rotation. Bounded at the top so an
    /// out-of-range request is treated as invalid (falls back) instead of crashing the host carrier array.</summary>
    public static bool IsCustomMapId(int mapId) => mapId >= FirstCustomMapId && mapId <= MaxCustomMapId;
    ...
    public static readonly Dictionary<string, int> NameToId = new(StringComparer.OrdinalIgnoreCase)
    {
        ...
        // NOTE: dictionary is OrdinalIgnoreCase, so "Bazaarcity" already resolves via the
        // "BazaarCity" entry above. A literal second key here collides and throws in the
        // static initializer (crashes any name-based map resolution).
        ...
        { "CustomFlat", CustomFlatId },
        { "Custom Flat", CustomFlatId },
        { "Custom", CustomFlatId },
    };
