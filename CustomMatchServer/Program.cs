...
builder.Services.PostConfigure<EconomyOptions>(options =>
{
    int? startingCharTokens = builder.Configuration.GetValue<int?>("CustomServer:CharacterUnlocks:StartingCharTokens");
    if (startingCharTokens.HasValue)
    {
        options.StartingCharTokens = Math.Max(0, startingCharTokens.Value);
    }
});
...
builder.Services.Configure<PlayerOverridesOptions>(builder.Configuration.GetSection("CustomServer:PlayerOverrides"));
builder.Services.AddSingleton<PlayerOverridesService>();
builder.Services.AddSingleton<CharacterUnlockService>();
builder.Services.PostConfigure<CustomServerOptions>(options =>
{
    ...
    PerModeSettingsEntry[] perModeFromJson = ParsePerModeSettingsJson(options.MatchDefaults.PerModeSettingsJson);
    if (perModeFromJson.Length > 0)
    {
        // JSON entries override defaults: replace the entry for any MatchmakingGameModeId in JSON.
        var byMode = options.MatchDefaults.PerModeSettings.ToDictionary(m => m.MatchmakingGameModeId, m => m);
        foreach (PerModeSettingsEntry entry in perModeFromJson)
        {
            byMode[entry.MatchmakingGameModeId] = entry;
        }
        options.MatchDefaults.PerModeSettings = byMode.Values.ToArray();
    }
    options.MatchDefaults.PerModeSettings = options.MatchDefaults.PerModeSettings
        .DistinctBy(entry => entry.MatchmakingGameModeId)
        .ToArray();
    ...
    options.CharacterUnlocks.InitiallyUnlockedCharacterIds = options.CharacterUnlocks.InitiallyUnlockedCharacterIds
        .Concat(ParseIntCsv(options.CharacterUnlocks.InitiallyUnlockedCharacterIdsCsv))
        .Where(CharacterCatalog.IsKnownId)
        .Distinct()
        .OrderBy(id => id)
        .DefaultIfEmpty(0)
        .ToArray();
    CharacterPriceEntry[] csvPrices = ParseCharacterPricesCsv(options.CharacterUnlocks.PricesCsv);
    options.CharacterUnlocks.Prices = options.CharacterUnlocks.Prices
        .Concat(csvPrices)
        .Where(p => CharacterCatalog.IsKnownId(p.CharId))
        .GroupBy(p => p.CharId)
        .Select(g => g.Last())
        .OrderBy(p => p.CharId)
        .ToArray();
    options.CharacterUnlocks.DefaultPrice = Math.Max(0, options.CharacterUnlocks.DefaultPrice);
    options.CharacterUnlocks.StartingCharTokens = Math.Max(0, options.CharacterUnlocks.StartingCharTokens);
    options.CharacterUnlocks.CharTokenAssetId = options.CharacterUnlocks.CharTokenAssetId > 0
        ? options.CharacterUnlocks.CharTokenAssetId
        : options.Unlocks.CharTokenAssetId;
});
...
// Applied via a global limiter partitioned by path so every /admin and /api/admin route is
// covered without per-endpoint opt-in; all other paths get an unlimited partition.
...
app.MapGet("/", (LobbyService lobbyService, IOptions<CustomServerOptions> options) =>
{
    return Results.Json(new
    {
        ...
        // Public endpoint: accountId-free snapshot (WS lobby admin trusts X-BAP-AccountId, so
        // accountIds are effectively credentials). Full detail lives on /admin/lobbies.
        lobbies = lobbyService.GetPublicLobbySnapshots()
...
app.MapGet("/api/diagnostics/runtime", (HttpContext ctx, IOptions<AdminOptions> adminOptions) =>
{
...
app.MapGet("/api/diagnostics/game-logs", (HttpContext ctx, IOptions<AdminOptions> adminOptions, IOptions<CustomServerOptions> options, int? tailLines, int? files) =>
{
    if (!AdminAuth.IsAuthorized(ctx, adminOptions.Value)) return Results.Unauthorized();
    return Results.Json(
...
app.MapGet("/api/diagnostics/server-log", (HttpContext ctx, IOptions<AdminOptions> adminOptions, DiagnosticsLogBuffer buffer, int? tail) =>
{
    if (!AdminAuth.IsAuthorized(ctx, adminOptions.Value)) return Results.Unauthorized();
    return Results.Json(new
    {
...
app.MapGet("/api/chars/listing", (HttpContext context, EconomyService economyService, CharacterUnlockService characterUnlocks, ILogger<Program> logger) =>
{
    var identity = ResolveIdentity(context);
    economyService.GetOrCreatePlayer(identity.AccountId, identity.Username);
    object[] listings = BuildCharacterListings(identity.AccountId, economyService, characterUnlocks);
    int owned = listings.Count(l => (int)l.GetType().GetProperty("purchases")!.GetValue(l)! > 0);
    logger.LogInformation("Character listing for {AccountId}: tokens={Tokens} owned={Owned} locked={Locked} medusaPrice={MedusaPrice}.",
        identity.AccountId,
        economyService.GetCharTokens(identity.AccountId),
        owned,
        listings.Length - owned,
        characterUnlocks.GetPrice(CharacterCatalog.MedusaCharId));
    return Results.Json(new { charListings = listings }, JsonContract.Options);
});
app.MapPost("/api/chars/listing", (HttpContext context, EconomyService economyService, CharacterUnlockService characterUnlocks, ILogger<Program> logger) =>
{
    var identity = ResolveIdentity(context);
    economyService.GetOrCreatePlayer(identity.AccountId, identity.Username);
    object[] listings = BuildCharacterListings(identity.AccountId, economyService, characterUnlocks);
...
app.MapGet("/api/server-config", (HttpContext context, IOptions<CustomServerOptions> options, CharacterUnlockService characterUnlocks) =>
{
    var opts = options.Value;
    var identity = ResolveIdentity(context);
    int[] playerChars = characterUnlocks.GetRosterCharacterIds(identity.AccountId);
    return Results.Json(new
    {
        ...
        availableCharacters = playerChars,
        availableMaps = opts.MapPool.BuildEnabledMapIds(),
        medusa = new
        {
            charId = CharacterCatalog.MedusaCharId,
            rosterEnabled = opts.Roster.EnableMedusa,
            available = Array.IndexOf(playerChars, CharacterCatalog.MedusaCharId) >= 0
        },
    }, JsonContract.Options);
});
...
app.MapPost("/api/chars/listing/purchase", async (HttpContext context, EconomyService economyService, CharacterUnlockService characterUnlocks, ILogger<Program> logger) =>
{
    var identity = ResolveIdentity(context);
    string? listingId = await TryReadBodyFieldAsync(context, "listingId");
    int? bodyCharId = await TryReadBodyIntAsync(context, "charId", "characterId");
    string? recipientId = await TryReadBodyFieldAsync(context, "recipientId");

    if (!string.IsNullOrWhiteSpace(recipientId) &&
        !string.Equals(recipientId, identity.AccountId, StringComparison.OrdinalIgnoreCase))
    {
        return Results.Json(CharacterPurchaseResponse(false, "Cannot purchase a character for another account.", listingId ?? "", 0, characterUnlocks.CharTokenAssetId, 0, economyService.GetCharTokens(identity.AccountId), []), JsonContract.Options);
    }

    int charId;
    if (!characterUnlocks.TryParseListingId(listingId, out charId))
    {
        charId = bodyCharId ?? -1;
    }
    ...
    {
        return Results.Json(CharacterPurchaseResponse(false, "Valid listingId or charId required.", listingId ?? "", 0, characterUnlocks.CharTokenAssetId, 0, economyService.GetCharTokens(identity.AccountId), []), JsonContract.Options);
    }

    listingId = CharacterUnlockService.ListingIdFor(charId);
    economyService.GetOrCreatePlayer(identity.AccountId, identity.Username);
    int price = characterUnlocks.GetPrice(charId);
    int[] unlockAssetIds = CharacterUnlockService.UnlockAssetIdsFor(charId);

    CharacterPurchaseResult result;
    if (characterUnlocks.IsCharacterOwned(identity.AccountId, charId, economyService))
    {
        result = economyService.PurchaseCharacter(identity.AccountId, identity.Username, charId, 0, unlockAssetIds, "character purchase idempotent-owned");
    }
    else
    {
        result = economyService.PurchaseCharacter(identity.AccountId, identity.Username, charId, price, unlockAssetIds, "character purchase");
    }

    logger.LogInformation("Character purchase for {AccountId}: charId={CharId} price={Price} ok={Ok} balance={Balance} alreadyOwned={AlreadyOwned} message={Message}",
        identity.AccountId, charId, price, result.Ok, result.NewCharTokenBalance, result.AlreadyOwned, result.Message);

    return Results.Json(CharacterPurchaseResponse(result.Ok, result.Message, listingId, charId, characterUnlocks.CharTokenAssetId, price, result.NewCharTokenBalance, result.GrantedAssetIds), JsonContract.Options);
});
...
app.MapPost("/api/friends/add", async (HttpContext context, FriendsService friendsService, PlayerStorageService playerStorage, IOptions<FriendsOptions> friendsOpt) =>
{
    ...
    {
        // The game client sends { username, discriminator }, not an accountId. Resolve it.
        string? username = await TryReadBodyFieldAsync(context, "username");
        if (!string.IsNullOrWhiteSpace(username))
        {
            toAccountId = playerStorage.LookupAccountId(username);
        }
    }
    if (string.IsNullOrWhiteSpace(toAccountId))
        return Results.Json(new { ok = false, message = "Player not found." }, JsonContract.Options);
...
app.MapPost("/api/friends/invite", async (HttpContext context, FriendsService friendsService, LobbyService lobbyService, IOptions<FriendsOptions> friendsOpt) =>
{
    ...
    if (result.Ok)
    {
        await lobbyService.NotifyLobbyInviteAsync(toAccountId, identity.AccountId, lobbyId);
    }
    return Results.Json(new { ok = result.Ok, message = result.Message }, JsonContract.Options);
});
...
// Internal economy endpoints. /api/internal/gold and /api/internal/reset-metagame are no-ops;
// /api/internal/xp grants character XP and is loopback-guarded to match game-callback endpoints.
...
{
    if (!AdminAuth.IsLoopback(context)) return Results.Unauthorized();
    var identity = ResolveIdentity(context);
...
// IMPORTANT: these admin endpoints must inject IOptions<T>, NOT IOptionsMonitor<T>.
// IOptions<T>.Value and IOptionsMonitor<T>.CurrentValue are two DIFFERENT cached instances;
// LobbyService/MatchmakingQueueService bind IOptions<T>.Value in their constructors. Mutating
// the monitor instance silently updated an object nobody reads — new lobbies and matchmaking
// defaults never saw the change (GET reflected it back, which hid the bug).
app.MapGet("/api/admin/bots", (HttpContext ctx, ServerAdminService adminService, IOptions<AdminOptions> adminOptions, IOptions<CustomServerOptions> options, IOptions<MatchmakingQueueOptions> queueOptions) =>
{
    if (!AdminAuth.IsAuthorized(ctx, adminOptions.Value)) return Results.Unauthorized();
    var opts = options.Value;
...
app.MapPost("/api/admin/bots", async (HttpContext ctx, ServerAdminService adminService, IOptions<AdminOptions> adminOptions, IOptions<CustomServerOptions> options, IOptions<MatchmakingQueueOptions> queueOptions, LobbyService lobbyService) =>
{
    if (!AdminAuth.IsAuthorized(ctx, adminOptions.Value)) return Results.Unauthorized();
    ...
    // Same-instance mutation as the services (see note on GET /api/admin/bots).
    var opts = options.Value;
    var qOpts = queueOptions.Value;
    if (botCount >= 0)
    {
    ...
    // The config fallback (MatchDefaults.BotDifficulty) is not clamped anywhere, so guard the
    // array index to avoid an IndexOutOfRange 500 on a misconfigured difficulty value.
    int safeDiff = Math.Clamp(finalDiff, 0, diffNames.Length - 1);
    ...
    {
        ok = true,
        message = $"Bots updated: count={finalCount}, difficulty={finalDiff}({diffNames[safeDiff]}). {lobbiesUpdated} active lobbies updated.",
        botCount = finalCount,
        botDifficulty = finalDiff,
        difficultyName = diffNames[safeDiff],
        lobbiesUpdated
    }, JsonContract.Options);
});
...
app.MapGet("/api/admin/modifiers", (HttpContext ctx, ServerAdminService adminService, IOptions<AdminOptions> adminOptions, IOptions<CustomServerOptions> options) =>
{
    if (!AdminAuth.IsAuthorized(ctx, adminOptions.Value)) return Results.Unauthorized();
    var opts = options.Value;
...
app.MapPost("/api/admin/modifiers", async (HttpContext ctx, ServerAdminService adminService, IOptions<AdminOptions> adminOptions, IOptions<CustomServerOptions> options, LobbyService lobbyService) =>
{
    if (!AdminAuth.IsAuthorized(ctx, adminOptions.Value)) return Results.Unauthorized();
    string body = await new StreamReader(ctx.Request.Body).ReadToEndAsync();
    if (string.IsNullOrWhiteSpace(body)) body = "{}";
...
app.MapGet("/api/shop", (HttpContext ctx, ShopService shopService, EconomyService eco, PlayerOverridesService overrides) => {
    var id = ResolveIdentity(ctx);
    return Results.Json(shopService.BuildShopResponse(id.AccountId, eco, overrides.GetShopOverride(id.AccountId)), JsonContract.Options);
});
app.MapPost("/api/shop", (HttpContext ctx, ShopService shopService, EconomyService eco, PlayerOverridesService overrides) => {
    var id = ResolveIdentity(ctx);
    return Results.Json(shopService.BuildShopResponse(id.AccountId, eco, overrides.GetShopOverride(id.AccountId)), JsonContract.Options);
});

// Matchmaking Queue endpoints
app.MapPost("/api/queue/join", async (HttpContext context, MatchmakingQueueService queueService, RankedService rankedService, IOptions<CustomServerOptions> serverOptions, EconomyService economyService, CharacterUnlockService characterUnlocks, LobbyService lobbyService) =>
{
    ...
    var identity = ResolveIdentity(context);
    // Post-match auto-requeue suppression also covers this HTTP path (the WS ready path is guarded in
    // SwitchReadyAsync). If the account just returned from a match, ignore this auto queue-join so the
    // player stays in the lobby; a later deliberate Play finds no flag and queues normally.
    if (lobbyService.TryConsumePostMatchRequeueSuppression(identity.AccountId))
    {
        return Results.Json(new { ok = false, message = "Returned to lobby. Press Play to queue.", position = 0, secondsRemaining = 0, charId = 0 }, JsonContract.Options);
    }
    ...
    economyService.GetOrCreatePlayer(identity.AccountId, identity.Username);
    charId = characterUnlocks.ResolveSelectableCharacter(identity.AccountId, charId, null, economyService);

    int gameModeId = 3;
    string? requestedMode = await TryReadBodyFieldAsync(context, "gameModeId");
    if (!string.IsNullOrWhiteSpace(requestedMode) && int.TryParse(requestedMode, out int parsedMode) && parsedMode >= 0 && parsedMode <= 10)
    {
        gameModeId = parsedMode;
    }
    else
    {
        // The unmodified client doesn't send gameModeId on queue join, so carry the mode the player
        // selected in their lobby instead of always defaulting to Solos.
        int? lobbyMode = lobbyService.GetAccountLobbyMode(identity.AccountId);
        if (lobbyMode is >= 0 and <= 10)
        {
            gameModeId = lobbyMode.Value;
        }
    }

    var result = queueService.JoinQueue(identity.AccountId, identity.Username, identity.Discriminator, charId, ranked.Points, gameModeId);
    return Results.Json(new { ok = result.Ok, message = result.Message, position = result.QueuePosition, secondsRemaining = result.SecondsRemaining, charId }, JsonContract.Options);
});
...
app.MapPost("/api/internal/game-ping", (HttpContext context, GamePing ping, LobbyService lobbyService) =>
{
    if (!AdminAuth.IsLoopback(context)) return Results.Unauthorized();
    lobbyService.RecordGamePing(ping);
    return Results.Ok(new { ok = true });
});
app.MapPost("/game-ping", (HttpContext context, GamePing ping, LobbyService lobbyService) =>
{
    if (!AdminAuth.IsLoopback(context)) return Results.Unauthorized();
    lobbyService.RecordGamePing(ping);
    return Results.Ok(new { ok = true });
...
app.MapPost("/api/internal/game-ended", (HttpContext context, GameEndedPayload payload, LobbyService lobbyService) =>
{
    if (!AdminAuth.IsLoopback(context)) return Results.Unauthorized();
    lobbyService.RecordGameEnded(payload);
    return Results.Ok(new { ok = true });
});
app.MapPost("/api/internal/team-ended", (HttpContext context, JsonElement payload) =>
{
    if (!AdminAuth.IsLoopback(context)) return Results.Unauthorized();
    return Results.Ok(new { ok = true });
});
app.MapPost("/game-ended", (HttpContext context, GameEndedPayload payload, LobbyService lobbyService) =>
{
    if (!AdminAuth.IsLoopback(context)) return Results.Unauthorized();
    lobbyService.RecordGameEnded(payload);
    return Results.Ok(new { ok = true });
});
app.MapPost("/team-ended", (HttpContext context, JsonElement payload) =>
{
    if (!AdminAuth.IsLoopback(context)) return Results.Unauthorized();
    return Results.Ok(new { ok = true });
});
    ...
    CancellationToken cancellationToken) =>
{
    ...
    AdminCommandResult result = normalizedCommand switch
    {
        ...
        "set-modifiers" => AdminSetModifiers(normalizedCommand, command, actor, adminService, app.Services.GetRequiredService<IOptions<CustomServerOptions>>()),
        "list-modifiers" => AdminListModifiers(normalizedCommand, app.Services.GetRequiredService<IOptions<CustomServerOptions>>()),
...
static void MapMetagameBootstrapEndpoints(WebApplication app)
{
    ...
    {
        iapListings = CharacterCatalog.AllIds.Select(charId => new
        {
            listingId = $"iap-char-{charId}",
            productId = $"char_{charId}",
            price = 0m,
            currency = "token",
            purchased = true,
            rewards = new[]
            {
                new { assetId = charId, amount = 1 }
            }
        }).ToArray()
    });
    ...
    // /api/chars/listing is now a per-player endpoint registered above in the main routes.
    // This MapClientJson call is intentionally removed — see the /api/chars/listing MapGet
    // that uses PlayerOverridesService for per-player character lists.
...
{
    int[] allCharIds = CharacterCatalog.AllIds;
    return new
    {
        passId = 1,
        passLevels = Enumerable.Range(1, 100)
            .Select(level => MinimalPassLevel(level, (level - 1) * 100,
                level <= allCharIds.Length ? $"custom-char-{allCharIds[level - 1]}" : null))
            .ToArray()
    };
}
...
static object[] BuildCharacterListings(string accountId, EconomyService economyService, CharacterUnlockService characterUnlocks)
{
    return characterUnlocks.GetRosterCharacterIds(accountId)
        .Select(charId => BuildCharacterListing(characterUnlocks.BuildListingState(accountId, charId, economyService)))
        .ToArray();
}

static object BuildCharacterListing(CharacterListingState listing)
{
    object[] costs = listing.Owned
        ? Array.Empty<object>()
        : [new { assetId = listing.CharTokenAssetId, amount = listing.Price }];

    object[] rewards = listing.UnlockAssetIds
        .Concat([listing.DefaultSkinAssetId])
        .Distinct()
        .Select(assetId => new { assetId, amount = 1 })
        .Cast<object>()
        .ToArray();
    ...
    return new
    {
        ...
        charId = listing.CharId,
        levelRequirement = 0,
        costs,
        rewards,
        purchases = listing.Owned ? 1 : 0
    };
}

static object CharacterPurchaseResponse(bool ok, string message, string listingId, int charId, int charTokenAssetId, int price, int balance, int[] rewardAssetIds)
{
    return new
    {
        ok,
        ...
        charId,
        purchases = ok ? 1 : 0,
        cost = new { assetId = charTokenAssetId, amount = price, balance },
        rewards = rewardAssetIds
            .Distinct()
            .Select(assetId => new { assetId, amount = 1, balance = 1 })
            .ToArray()
    };
}
...
static IResult BuildLoadResult(HttpContext context, ServerAdminService adminService, IOptions<UnlockOptions> unlockOptions, CharacterUnlockService characterUnlocks)
{
    LoadResponse response = BuildLoadResponse(context, adminService, unlockOptions.Value, characterUnlocks);
    AttachSessionCookie(context, response.AccountId);
    return Results.Json(response, JsonContract.Options);
}

static IResult BuildCompleteResult(HttpContext context, ServerAdminService adminService, IOptions<UnlockOptions> unlockOptions, CharacterUnlockService characterUnlocks)
{
    LoadResponse response = BuildLoadResponse(context, adminService, unlockOptions.Value, characterUnlocks);
    AttachSessionCookie(context, response.AccountId);
    return Results.Json(new
...
static IResult LoadoutEquipResult(HttpContext context, ServerAdminService adminService, IOptions<UnlockOptions> unlockOptions, CharacterUnlockService characterUnlocks)
{
...
static LoadResponse BuildLoadResponse(HttpContext context, ServerAdminService adminService, UnlockOptions unlockOptions, CharacterUnlockService characterUnlocks)
{
    ...
    // Resolve per-player overrides
    var overrides = context.RequestServices.GetRequiredService<PlayerOverridesService>();
    var playerOverride = overrides.GetEffective(identity.AccountId);

    // Apply username override if set
    string username = playerOverride.Username ?? identity.Username;
    identity = identity with { Username = username };

    var ownedAssets = BuildOwnedAssets(unlockOptions, playerOverride).ToList();

    void UpsertAsset(int assetId, int balance)
    {
        LoadAsset? existing = ownedAssets.FirstOrDefault(a => a.AssetId == assetId);
        if (existing is not null) existing.Balance = balance;
        else ownedAssets.Add(new LoadAsset { AssetId = assetId, Balance = balance });
    }

    void AddOwnedAsset(int assetId)
    {
        if (ownedAssets.All(a => a.AssetId != assetId))
            ownedAssets.Add(new LoadAsset { AssetId = assetId, Balance = 1 });
    }

    void EnsureOwnedAssetBalanceAtLeast(int assetId, int minimumBalance = 1)
    {
        LoadAsset? existing = ownedAssets.FirstOrDefault(a => a.AssetId == assetId);
        if (existing is not null)
        {
            existing.Balance = Math.Max(existing.Balance, minimumBalance);
        }
        else
        {
            ownedAssets.Add(new LoadAsset { AssetId = assetId, Balance = minimumBalance });
        }
    }
    ...
    {
        economyService.GetOrCreatePlayer(identity.AccountId, username);
        int[] economyAssets = economyService.GetOwnedAssets(identity.AccountId);
        foreach (int ecoAssetId in economyAssets)
        {
            AddOwnedAsset(ecoAssetId);
        }

        // Add character unlock assets according to the same ownership rules used by
        // /api/chars/listing and lobby character switching. This keeps every server
        // surface in agreement, including player-overrides UnlockEverything=true.
        ownedCharacterIds = characterUnlocks.GetRosterCharacterIds(identity.AccountId)
            .Where(charId => characterUnlocks.IsCharacterOwned(identity.AccountId, charId, economyService))
            .ToArray();
        foreach (int charId in ownedCharacterIds)
        {
            foreach (int assetId in CharacterUnlockService.UnlockAssetIdsFor(charId))
            {
                AddOwnedAsset(assetId);
            }
        }

        // Update gold balance from player override, or fall through to economy
        int gold = playerOverride.Gold ?? economyService.GetGold(identity.AccountId);
        UpsertAsset(unlockOptions.GoldAssetId, gold);

        // Update char-token balance from player override, or fall through to economy.
        int charTokens = playerOverride.CharTokens ?? economyService.GetCharTokens(identity.AccountId);
        UpsertAsset(characterUnlocks.CharTokenAssetId, charTokens);

        // Character unlock compatibility assets overlap with low currency ids
        // (notably charId 2 and charTokens assetId 2). Keep owned character
        // markers truthy after currency balances are upserted so the native
        // client never interprets an owned roster slot as locked.
        foreach (int charId in ownedCharacterIds)
        {
            foreach (int assetId in CharacterUnlockService.UnlockAssetIdsFor(charId))
            {
                EnsureOwnedAssetBalanceAtLeast(assetId);
            }
        }
    }

    // Apply per-player extra owned assets
    int[] extraAssets = overrides.GetExtraOwnedAssetIds(identity.AccountId);
    foreach (int extraId in extraAssets)
    {
        AddOwnedAsset(extraId);
    }
...
static LoadAsset[] BuildOwnedAssets(UnlockOptions options, PlayerOverrideEntry? playerOverride = null)
{
    var assets = new List<LoadAsset>(1024);

    int[] defaultSkins = CharacterCatalog.DefaultSkinAssetIds;

    // Determine effective unlock flags: player override > server config > default
    bool unlockAll = playerOverride?.UnlockEverything ?? options.UnlockEverything;
    bool unlockSkins = playerOverride?.UnlockAllSkins ?? (unlockAll || options.EffectiveUnlockSkins);
    bool unlockEmotes = playerOverride?.UnlockAllEmotes ?? (unlockAll || options.EffectiveUnlockEmotes);
    bool unlockBanners = playerOverride?.UnlockAllBanners ?? (unlockAll || options.EffectiveUnlockBanners);
    bool unlockMasteryBadges = playerOverride?.UnlockAllMasteryBadges ?? (unlockAll || options.EffectiveUnlockMasteryBadges);
    bool unlockTombstones = playerOverride?.UnlockAllTombstones ?? (unlockAll || options.EffectiveUnlockTombstones);
    int? goldOverride = playerOverride?.Gold;
    int? charTokensOverride = playerOverride?.CharTokens;

    if (!unlockAll && !unlockSkins && !unlockEmotes && !unlockBanners && !unlockMasteryBadges && !unlockTombstones)
    {
    ...
    // Always add default skins
    foreach (int defaultSkin in defaultSkins.Distinct())
    {
        if (assets.All(a => a.AssetId != defaultSkin))
            assets.Add(new LoadAsset { AssetId = defaultSkin, Balance = 1 });
    }

    if (unlockSkins || unlockAll)
    {
        int skinStart = options.SkinAssetIdStart > 0 ? options.SkinAssetIdStart : 300000;
        int skinEnd = options.SkinAssetIdEnd > 0 ? options.SkinAssetIdEnd : 301000;
        AppendRange(assets, skinStart, skinEnd, balance: 1);
    }
    ...
    if (unlockEmotes || unlockAll)
    {
        int es = options.EmoteAssetIdStart > 0 ? options.EmoteAssetIdStart : 400000;
        int ee = options.EmoteAssetIdEnd > 0 ? options.EmoteAssetIdEnd : 401000;
        AppendRange(assets, es, ee, balance: 1);
    }
    if (unlockBanners || unlockAll)
    {
        int bs = options.BannerAssetIdStart > 0 ? options.BannerAssetIdStart : 500000;
        int be = options.BannerAssetIdEnd > 0 ? options.BannerAssetIdEnd : 501000;
        AppendRange(assets, bs, be, balance: 1);
    }
    if (unlockMasteryBadges || unlockAll)
    {
        int ms = options.MasteryBadgeAssetIdStart > 0 ? options.MasteryBadgeAssetIdStart : 600000;
        int me = options.MasteryBadgeAssetIdEnd > 0 ? options.MasteryBadgeAssetIdEnd : 601000;
        AppendRange(assets, ms, me, balance: 1);
    }
    if (unlockTombstones || unlockAll)
    {
        int ts = options.TombstoneAssetIdStart > 0 ? options.TombstoneAssetIdStart : 700000;
        int te = options.TombstoneAssetIdEnd > 0 ? options.TombstoneAssetIdEnd : 701000;
        AppendRange(assets, ts, te, balance: 1);
    }

    foreach (int currencyAssetId in options.CurrencyAssetIds.Distinct())
    {
        int balance = options.CurrencyBalance;
        assets.Add(new LoadAsset { AssetId = currencyAssetId, Balance = balance });
    }
    ...
    {
        charTokenAsset.Balance = charTokensOverride ?? options.CharTokenBalance;
    }
    else if (options.CharTokenAssetId > 0)
    {
        // CharTokens asset wasn't in the currency list; add it explicitly.
        assets.Add(new LoadAsset { AssetId = options.CharTokenAssetId, Balance = charTokensOverride ?? options.CharTokenBalance });
    }
...
static CharacterPriceEntry[] ParseCharacterPricesCsv(string? value)
{
    if (string.IsNullOrWhiteSpace(value))
    {
        return [];
...
static PerModeSettingsEntry[] ParsePerModeSettingsJson(string? value)
{
    if (string.IsNullOrWhiteSpace(value))
    {
        return [];
    ...
    try
    {
        return JsonSerializer.Deserialize<PerModeSettingsEntry[]>(value, JsonContract.Options) ?? [];
    }
    catch
    {
        return [];
...
static AdminCommandResult AdminSetModifiers(string command, AdminCommandRequest request, string actor, ServerAdminService adminService, IOptions<CustomServerOptions> options)
{
    // IOptions (not IOptionsMonitor): must mutate the same instance the services bound.
    var opts = options.Value;
    int[] newIds;
...
static AdminCommandResult AdminListModifiers(string command, IOptions<CustomServerOptions> options)
{
    var opts = options.Value;
    var current = new HashSet<int>(opts.MatchDefaults.GameModifierIds);
    var list = GameModifierCatalog.KnownGameModifiers.Select(m => new { id = m.Id, name = m.Name, display = m.Display, enabled = current.Contains(m.Id) }).ToArray();
...
{
    // Marker so tests can locate this assembly's entry point via WebApplicationFactory<ApiEntryPoint>.
    // The top-level Program lives in the global namespace and collides with CustomClientProxy's Program,
    // so a uniquely-named public type in this namespace is used as TEntryPoint instead.
