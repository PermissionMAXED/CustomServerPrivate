...
export CustomServer__RequireGameServerBootstrap="${REQUIRE_GAME_BOOTSTRAP:-false}"
export CustomServer__GameServerReadyTimeoutSeconds="${GAME_READY_TIMEOUT:-30}"
...
export CustomServer__GameServerStartupStallGraceSeconds="${GAME_STARTUP_STALL_GRACE:-300}"
export CustomServer__GameServerStartupStallSeconds="${GAME_STARTUP_STALL:-300}"
export CustomServer__GameServerWrapperOnlyStartupStallGraceSeconds="${GAME_WRAPPER_ONLY_STALL_GRACE:-300}"
export CustomServer__GameServerWrapperOnlyStartupStallSeconds="${GAME_WRAPPER_ONLY_STALL:-300}"
export CustomServer__GameServerPrewarmOnStartup="${GAME_PREWARM_ON_STARTUP:-false}"
...
[ -n "$DEPLOYED_RELEASE" ] || DEPLOYED_RELEASE="backup-pre-amp-refactor-20260527-174239-46-g1ed6721-dirty"
export BAPCUSTOM_RELEASE_LABEL="${BAPCUSTOM_RELEASE_LABEL:-$DEPLOYED_RELEASE}"

echo "[amp-start] release=${BAPCUSTOM_RELEASE_LABEL} git=${BAPCUSTOM_GIT_COMMIT:-1ed67211e039} branch=${BAPCUSTOM_GIT_BRANCH:-main}"
...
# Wine prefix prewarm (quick, just initializes the wineprefix)
if [ "${BAPCUSTOM_PREWARM_WINE:-0}" != "0" ]; then
  echo "[amp-start] prewarm begin"
  ./start-match.sh --prewarm ./game/bapbap.exe -batchmode -nographics
  echo "[amp-start] prewarm complete"
else
  echo "[amp-start] wine prewarm skipped by BAPCUSTOM_PREWARM_WINE=0"
fi

# Unity prewarm DISABLED by default (nographics causes AppUI freeze under Wine)
# Enable with BAPCUSTOM_PREWARM_UNITY=1 env var
if [ "${BAPCUSTOM_PREWARM_UNITY:-0}" != "0" ]; then
